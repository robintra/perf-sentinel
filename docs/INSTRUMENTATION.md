...
Every operator surface names the effective namespace: the text report and `diff` blocks print a `Namespace:` row, the TUI detail pane prints one too (`-` when the finding carries none), and the correlations panel qualifies each side as `service@namespace`. `diff` also keys findings on it, so the same problem in two deployments no longer folds into one row. The HTML report retains and displays both namespace attributes. Its single namespace filter uses `k8s.namespace.name` when present, then falls back to `service.namespace`. With neither attribute the finding has no namespace chip. This display grouping does not change the acknowledgment signature, so one acknowledgment can still cover the same problem across namespaces. Jaeger reads the values from process tags (with span-tag fallback), and Zipkin reads them from span tags.

# Schema reference: perf-sentinel-report v1.7
...
v1.1 adds the `canonical_waste` and `operational_waste` tiers to `aggregate`. v1.2 adds `aggregate.temporal_coverage` (a measurement-continuity signal), `scope_manifest.coverage_basis` (a provenance marker), and the reserved `integrity.cross_period_log` hook. v1.3 adds `methodology.standard_crosswalk` (an interpretive ESRS E1 datapoint crosswalk) and per-pattern `applications[].anti_patterns[].rgesn_criteria` (RGESN 2024 criteria). v1.4 adds `aggregate.database_waste` (the database-side avoidable energy and carbon at both thresholds, with its provenance models, an informational lower bound kept out of every total). v1.5 adds messaging support: two `PatternName` values (`n_plus_one_messaging`, `slow_messaging`) and `aggregate.messaging_waste`, the broker-side twin of `database_waste` (equally informational and equally excluded from every total, though which way it errs depends on its `model` tag, see `docs/LIMITATIONS.md`), emitted once broker publish spans are ingested. Both waste blocks also gain `declared_windows` and `declared_energy_kwh`, which hold the share of the figure an operator declared rather than measured, so a provisioned-cluster declaration is never published inside `measured_energy_kwh`. They are omitted when zero, which is always the case on the database block. v1.6 makes the carbon parameters visible, through four `methodology.calibration_inputs` fields: `carbon_methodologies` (the SCI tags observed on the period's windows, which say whether network transport entered the numerator), `embodied_gco2_total` (the SCI `M` term summed over the period), `embodied_gco2_per_request` (that total divided by `total_requests`, so it is comparable across reports and independent of traffic volume) and `scoring_coefficients` (the coefficients the windows were scored with, as `key=value` strings, two entries for one key meaning it changed mid-period). The settings behind them, starting with `[green] embodied_carbon_per_request_gco2` and `[green] network_energy_per_byte_kwh`, scale published figures and appeared nowhere in the report, so halving a coefficient left no trace. All three are read off the archived windows rather than off the configuration of whichever machine runs `disclose`, which is the only correct source when a period was produced elsewhere, and several `carbon_methodologies` entries mean the period spans legacy and current numerator methodologies. It also splits the published total: `aggregate.carbon_breakdown` gives the operational, embodied and transport terms, which sum to `total_carbon_kgco2eq`. Only the operational term carries an avoidable share, the one the waste tiers quantify, while amortised hardware and network transport cannot be reduced by fixing an anti-pattern, so reading the total alone mixes what an operator can act on with what they cannot. The block also carries `database_kgco2eq_out_of_total` and `messaging_kgco2eq_out_of_total` for measured or declared external-scope figures. Estimated fallback carbon is already included in the service total and is excluded here. Both sit outside the sum and outside `total_carbon_kgco2eq`, since a database cgroup and a broker are a different scope from the instrumented services, and folding them in would mix perimeters. They are published here so every carbon figure a run produced can be read in one place. Every term of the split is omitted when its value is zero, so an unmeasured term never reads as measured at zero, and the methodology still records that transport is part of the current numerator. Transport stays the least certain of the three when present: published figures span two orders of magnitude, so its coefficient is fixed since 0.9.25 (0.04 kWh/GB, no longer configurable, every disclosure scales the same traffic identically) and the term is also published under the sourced coefficient bracket, `transport_kgco2eq_low` (0.001 kWh/GB, Cloud Carbon Footprint) and `transport_kgco2eq_high` (0.059 kWh/GB, Sustainable Web Design Model v4 network segment, a ceiling). v1.6 also fills `integrity.trace_integrity_chain`, reserved and always null until now. The daemon hash-chains every archived window as it writes it, and `disclose` walks that chain while aggregating, publishing how many windows verified, how many carry no chain (written before this existed) and how many breaks were found. A break means a window was edited, removed or reordered after it was written. Breaks found in the same files but outside the period are counted apart, under `breaks_outside_period`, since one rolling archive can span years and this report only answers for its own period. The report is still produced and says so, rather than refusing to exist, so a truncated archive or a rotation still yields an honest partial report. Two things the chain does not do. It does not see a clean truncation of an archive's tail, which leaves a shorter self-consistent chain that no field inside the file can contradict. And it does not attest the source: whoever controls the daemon and its files can regenerate a consistent chain, and the field sits under the `content_hash` precisely so a break count cannot be rewritten to zero under a valid signature. Closing that last gap needs an external anchor for the chain head, which is what `integrity.cross_period_log` stays reserved for. v1.7 completes the chain verdict with two `trace_integrity_chain` fields derived from the cumulative `drops` counter each archive line carries since v1.7: `windows_dropped`, the windows the daemon produced but could not archive over the period (a full channel, an exited writer, a serialization or write failure), and `drop_counter_resets`, how many times the counter went backwards inside the period, one per daemon restart, each making `windows_dropped` a lower bound over the gap it spans. The figure says nothing about windows never produced (a stopped daemon): `aggregate.temporal_coverage` bounds those. Both fields are omitted when no archived line carried the counter, so an archive written before v1.7 never reads as zero drops. The disclosure remains a directional estimate, not regulatory-grade. The schema accepts `perf-sentinel-report/v1.0` through `v1.7`, and every added field defaults when absent, so older readers and reports remain valid and the `content_hash` of an older report is unchanged when re-hashed on a newer binary.

## Top-level keys

| key               | type          | required | notes                                                                                                               |
|-------------------|---------------|----------|---------------------------------------------------------------------------------------------------------------------|
| `schema_version`  | string (enum) | yes      | `"perf-sentinel-report/v1.7"` (also accepts `"…/v1.6"`, `"…/v1.5"`, `"…/v1.4"`, `"…/v1.3"`, `"…/v1.2"`, `"…/v1.1"`, `"…/v1.0"`) |

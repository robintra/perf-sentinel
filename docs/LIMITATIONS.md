...
- [Automated mysql-stat ingestion from Prometheus](#automated-mysql-stat-ingestion-from-prometheus): the collector is off by default, and each counter it does not publish leaves its column at zero.
...
## Messaging: producer side only, no consumer analysis

Broker spans are admitted from the OTel messaging convention (`messaging.system`, `messaging.destination.name`), one convention that in principle covers Kafka, RabbitMQ, Pulsar, SQS, NATS and JMS alike. Only the `PRODUCER` span is admitted, as an outbound call whose target is the destination.

Admission is decided in a fixed order: a SQL statement wins, then an outbound URL (`http.url`, `url.full`) on any span that is not `SERVER`, then an RPC callee on a `CLIENT` span, then messaging. An emitter that sets `url.full` on its publish span, which some AWS SDK versions do for SQS, therefore lands on the HTTP path instead, where the queue URL goes through the path normalizer and its account id is masked as `{id}`. The messaging path is verified against hand-built spans for the shape the convention describes, not against captures from every broker's agent, so read the list above as the conventions covered rather than a per-agent guarantee. If a broker's spans are not appearing as messaging findings, check whether the agent also sets `url.full` or `db.system` on them. A `CONSUMER` span (`process`) and a `CLIENT` messaging span (`receive`, `settle`) describe work done on a delivered message rather than a call the service made, and a polling consumer would flood the occurrence detectors with publishes it never issued. Those spans are dropped under the `not_io` reason. One exception: a `CLIENT` poll span that also carries `rpc.*`, which the AWS SDK sets on SQS `ReceiveMessage`, matches the RPC branch above and is admitted as an outbound call named after the API method. A consumer-only service therefore reads as having no messaging I/O, but may still show HTTP findings for its polling loop.

A publish carries no bind parameters, so the N+1 rule for messaging is the occurrence count alone. N publishes to one destination within the detection window are reported as `n_plus_one_messaging` with a distinct-params count of 0, since the variation lives in the payload and payloads are never read. This is more eager than the SQL and HTTP rules, whose parameter variance can separate a loop from a legitimate repeat. A service that batches deliberately at the application level will be flagged, and the suggestion to use the broker's batch API is the intended reading. Raise `n_plus_one_threshold` if the noise outweighs it.

There is no `redundant_messaging` finding type. Redundant detection groups on `(type, template, params)`, and with params always empty that key would collapse to the destination alone, so any two publishes to one destination would be reported as duplicates with a "cache or deduplicate" suggestion. Two messages are never the same message, so publish spans are skipped by that detector entirely. A repeated publish surfaces as `n_plus_one_messaging` or not at all.

One assumption behind both rules does not hold uniformly across brokers. The pipeline counts one span as one network round-trip, which is true for SQS `SendMessage` and for a RabbitMQ `basic_publish`, but not for Kafka. `KafkaProducer.send()` buffers in memory and the client flushes according to `batch.size` and `linger.ms`, so five publish spans can be a single write on the wire. On Kafka the occurrence count, the avoidable-I/O count and the resulting waste ratio are therefore upper bounds, and "send them as one batch" may describe an optimization the client already performs. `slow_messaging` inherits the same caveat: a Kafka producer span often measures enqueueing rather than broker acknowledgment.

Destinations are not normalized. A topic or queue name is already a template, so it is compared verbatim rather than routed through the HTTP path normalizer, which would mask numeric segments as `{id}` and strip the account id out of an SQS queue ARN. The trade-off is the opposite one: a deployment that names a topic per tenant produces one template per tenant, so a publish loop is split across groups and may fall under the threshold. Under-grouping is the safe side of this error, since it never merges two tenants into one finding.

The carbon model gives a publish the unweighted per-operation coefficient. The HTTP payload-size tiers are not reused. Their 10 KB and 1 MB boundaries encode a web payload distribution, and 1 MB is Kafka's default `max.message.bytes`, a protocol ceiling rather than a large message. Applying them would assert a near-constant discount that no measurement supports. `messaging.message.body.size` is still ingested for the day a measured coefficient exists.

A separate path does put a broker energy figure in the report, and it does not go through a coefficient at all. Declaring `[green.alumet.broker]` measures the broker cgroup, and `[green.broker_static]` derives watts from a declared cluster. Either way the energy is multiplied by the messaging waste ratio and reported as `messaging_waste`, excluded from every total, with a `model` tag that says where it came from. Read that tag before reading the number, because the three sources do not err in the same direction. `alumet_rapl` measures the broker cgroup through Alumet, whose default attribution formula covers CPU only, so it under-counts. `broker_specpower` bounds the declared vCPUs at full load, using a SPECpower table that covers CPU and baseboard and not storage, network or PSU. It is a ceiling on compute rather than on the cluster's wall power, and a storage-bound broker can draw more than it. It also does not move when the application starts batching, and a long idle stretch is truncated to one hour of billing, so it under-counts there. `estimated` re-presents a share of the totals the spans already produced. See `docs/CONFIGURATION.md`. Network transport energy is not estimated for messaging at all, because the hostname extraction behind it only understands `http://` and `https://` targets.

Two bounds on the destination itself. On a RabbitMQ **default exchange** `messaging.destination.name` is the empty string, so the target falls back to the span name and every routing key collapses into one template. Twelve publishes across three keys were measured as a single finding named `rabbitmq publish`, with nothing in the template to act on. `messaging.rabbitmq.destination.routing_key` is not read. And when a consumer trace carries several sibling `receive` spans, the producer link attributed to the work is the first one in batch order, which is deterministic but arbitrary, since no span says which message caused which query.

Three of the strongest determinants of real messaging energy are invisible from a `PRODUCER` span, so they are structurally outside what this tool can explain. The first is where the broker cluster sits on its utilisation curve. Measured power rises with throughput only up to roughly 20 % of a Kafka cluster's maximum, then flattens, so the marginal energy of one more publish depends on cluster load rather than on the publish itself. The second is the replication factor, which decides how many physical appends one logical publish becomes. The third is the distribution topology, partitions, brokers and consumer groups. None of the three appears in a producer-side span, and no amount of trace analysis recovers them. Attributing energy per publish is therefore an amortization of mostly fixed infrastructure power, presented as such, not a measurement of the work one message caused. See `docs/design/05-GREENOPS-AND-CARBON.md` for the measurements behind this.

Because publish spans now enter the pipeline, they also count in `total_io_ops`, so the I/O intensity score of an endpoint that mixes SQL and publishes is higher than the same traffic scored by 0.9.22. That is a widening of the measurement perimeter, not an application regression, and it matters when comparing disclosure periods across the upgrade.
...
`CONSUMER` ancestor must arrive in the same OTLP request and service index. The
default `BatchSpanProcessor` enqueues a parent span when it ends, after its
children, so a handler whose children end just before a scheduled flush ships
them in one batch and the consumer parent in the next. The link is then absent,
which is why the same handler can show the line on one run and not the next.
...
The SQL normalizer uses a homemade regex-based tokenizer rather than a full SQL parser. This trade-off keeps the binary small, avoids heavy deps and lets the tokenizer work across dialects.
...
| WASTE_HIGH 0.30   | Matches the default `io_waste_ratio_max`. The gate is user policy and the interpretation is a fixed heuristic. They stay independent so a relaxed gate does not silently mute the signal |
| WASTE_CRITICAL 0.50 | At least half of analyzed I/O is avoidable waste |

### Stability contract

Enum values (`healthy`, `moderate`, `high`, `critical`) are stable across versions, so downstream consumers can branch on them. Numeric thresholds are versioned with the binary and may evolve. Consumers needing version-independent classification (e.g. a Grafana alert) should read the raw `io_intensity_score` / `io_waste_ratio` fields and apply their own bands.
...
entry point. With no frame either, the destination of the nearest CONSUMER span
does, as `<messaging.system> <destination>` (`rabbitmq crm.dossiers`),
`messaging.destination.template` before the name and the system spelling
lowercased. What the destination holds depends on the instrumentation. Under
its default messaging conventions, the OpenTelemetry Java agent's
...
origin and leaves `"unknown"`. An instrumentation may set neither flag (the
OpenTelemetry Java agent among them), which is why the server-generated names
are filtered as well. Jaeger and Zipkin follow the same order.
...
`TraceWindow`. Anonymous context is not retained across exports. This bounded
context lets a route or a consumer span that arrives in a later export repair
an earlier I/O event, the usual case for a consumer, which ends after the
children it wraps, without creating a synthetic event or incrementing I/O
...
detect+score run on a single dedicated analysis worker, decoupled from the ingestion / eviction `select!` loop by a bounded channel (1024 batches). The loop enqueues evicted and expired batches without blocking, so a slow analysis pass cannot stall ingestion or delay TTL eviction. The trade-off is that when analysis cannot keep up with sustained load, the queue fills and whole batches are **shed**. Shedding is explicit and metered, not silent:
...
Analysis shedding is separate from archive retention. The per-window disclosure archive (`daemon/archive.rs`, the NDJSON that `disclose` later aggregates) has its own bounded channel with an explicit drop-on-full policy. Under sustained load, or if the writer task falls behind on disk I/O, whole windows are dropped from the archive even when their findings were analyzed and served on the live endpoints. Every drop is logged and counted on `perf_sentinel_archive_windows_dropped_total`, and that counter is the only witness after the fact. The hash chain advances only on successful writes, so a dropped window leaves no hole in `seq` and hash verification cannot reveal it. The counter itself has two blind spots. A writer task that dies discards its queued backlog, up to 256 windows, with no increment. A hard kill (SIGKILL, OOM) that lands before the drain finishes loses windows the same silent way. A graceful shutdown does drain and join the archive channel, exactly as it drains the analysis worker, so the backlog is flushed on a clean exit. What a clean exit cannot restore are the windows already dropped before it, and a write that fails during that final flush is still lost. The archive is best-effort (public transparency, not regulatory-grade), so treat it as a sampled record rather than a complete ledger, and read a nonzero drop rate as an archive that under-reports the period.
...
This is a **log-symmetric multiplicative interval**, not an arithmetic ±50% window. The geometric mean of `low` and `high` equals `mid`, while the arithmetic mean does not. The 2× framing reflects the order-of-magnitude uncertainty of the I/O proxy model (ENERGY_PER_IO_OP_KWH is rougher than half). A symmetric ±50% window would understate the real model uncertainty. Read the bounds as "the true value is within a factor of 2 of `mid`, in either direction".
...
- `co2.avoidable.methodology = "sci_v1_operational_ratio"`: `operational × (avoidable_io_ops / accounted_io_ops)`, a region-blind global ratio that excludes embodied carbon.
...
The optional network transport energy term estimates the energy cost of moving bytes between regions. The coefficient (0.04 kWh/GB, fixed since 0.9.25 so every disclosure scales transport identically) is a conservative value below recent whole-network averages (Sustainable Web Design Model v4, 2024: 0.059 kWh/GB operational for networks) and an upper bound for cross-region server traffic, where inter-datacenter coefficients run as low as 0.001 kWh/GB (Cloud Carbon Footprint). The disclosure publishes the term under that sourced 0.001-0.059 bracket beside the mid value.
...
- **Distinct endpoints.** Every distinct endpoint (finding type, service, grouping, template) seen in the window is kept until the window forgets it, with no cap. A service whose templates do not normalize well (literals left in SQL or URLs) keeps each variant for the whole `window_minutes`.
- **Pairing reach.** The pairing deque spans the findings analysed in the last `lag_threshold_ms + 2 x trace_ttl_ms`, about a minute at the default 30 s TTL, and every incoming finding scans it. Its size and that scan scale with `trace_ttl_ms`, not with `window_minutes`.
...
Batch `analyze` always reports an empty correlations array. This is expected, not a bug.
...
- **Kill-switch.** Setting `[daemon] api_enabled = false` disables all `/api/*` routes while keeping OTLP ingestion and `/metrics` active. Use this when the daemon runs in an environment where even loopback exposure of findings is unacceptable. `/metrics` still exposes finding counts via `perf_sentinel_findings_total` and related counters, so the query API flag does not remove all observable output.
...
- **Hub delivery is bounded, coalesced, and non-durable.** When `[daemon.hub_export]` is enabled, repeated signatures replace their pending value. After Hub acknowledgment they are suppressed for one hour unless severity worsens. `max_pending` caps both pending and recently acknowledged signatures. Capacity eviction is visible through `perf_sentinel_hub_export_dropped_total`. A process restart clears anything not yet acknowledged by the Hub and forgets recent acknowledgments, so active signatures may be sent again harmlessly. The Hub's hourly poll is a recovery safety net, but `/api/findings` returns at most 1000 entries, so sustained production delivery should use the push exporter rather than polling alone.
- **Findings history in Grafana is as deep as the Hub, and day-granular.** The `History (Hub)` row of the findings dashboard reads PerfSentinelHub (0.3.0 and later), not the daemon: without a Hub the dashboard cannot look back, whatever the time picker says. The Hub records the days it observed a finding, on its own clock in UTC, starting the day it was upgraded, keeps them for `Hub:Retention` (180 days by default), and records no occurrence counts, which stay with `perf_sentinel_findings_total` in Prometheus. The row's acknowledgment columns need daemons from 0.24.0, whose `GET /api/acks?include_toml=true` the Hub mirrors.
...
- **TLS key file**: `[daemon] tls_key_path` permissions are checked at startup, and a world- or group-readable key emits a warning.

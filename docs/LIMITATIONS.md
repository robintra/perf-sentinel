...
A separate path does put a broker energy figure in the report, and it does not go through a coefficient at all. Declaring `[green.alumet.broker]` measures the broker cgroup, and `[green.broker_static]` derives watts from a declared cluster. Either way the energy is multiplied by the messaging waste ratio and reported as `messaging_waste`, a lower bound excluded from every total, with a `model` tag that distinguishes measured from declared from estimated. The declared path is an upper bound that does not react to batching, see `docs/CONFIGURATION.md`. Network transport energy is not estimated for messaging at all, because the hostname extraction behind it only understands `http://` and `https://` targets.

...
Starting with v0.7.0, every official perf-sentinel release binary
carries a SLSA L2 build provenance attestation. The attestation is
generated by GitHub Actions through
`slsa-framework/slsa-github-generator` (pinned by SHA, tag in
trailing comment) and published alongside the binary as a release
asset (`multiple.intoto.jsonl`).

Why L2 and not L3: the reusable generator workflow is capable of L3
when invoked from a hardened, audited runner, but we run on the
shared `ubuntu-latest` pool with default permissions narrowed at the
job level. SLSA L2 is the honest claim and is what we record in
`integrity.binary_attestation.slsa_level` of each periodic report
produced by such a binary.
...
A successful verification confirms that the binary was built from
the specified tag of this repository by GitHub Actions, not by a
third party. Combine with the `verify-hash` subcommand against a
periodic disclosure report to verify the full chain:

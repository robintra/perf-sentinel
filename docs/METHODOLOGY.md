...
- **E (Energy)** is the per-operation electricity, in kWh. perf-sentinel resolves it from the configured source at runtime: an I/O proxy (`io_proxy_v3`, around `1e-7` kWh per I/O op, directional only, optionally rescaled by operator-supplied calibration coefficients via `[green] calibration_file`), or one of the five measured backends, Alumet RAPL, Scaphandre RAPL, Kepler eBPF, Redfish BMC wall-plug, or cloud-provider CPU% mapped against SPECpower tables. When several cover the same service the daemon keeps the highest-fidelity reading, in that order, see the fidelity ladder in [docs/ENERGY.md](ENERGY.md#the-fidelity-ladder). The selected source is surfaced in `methodology.calibration.energy_source_models` so an auditor can verify which path produced E.
...
- Connection and session management statements (`COMMIT`, `SET`, `SELECT set_config(...)`, pool pings) never reach this counter: the repetition detectors skip them, so no finding attributes them. They remain in `total_io_ops`, since a pooled driver really does execute one per connection checkout, but they are not avoidable and no batching remediation applies. See `docs/LIMITATIONS.md`, section "Session commands and waste ratio", for the exclusion boundary.

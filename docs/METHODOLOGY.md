...
- **E (Energy)** is the per-operation electricity, in kWh. perf-sentinel resolves it from the configured source at runtime: an I/O proxy or one of the five measured backends. The proxy is `io_proxy_v3`, around `1e-7` kWh per I/O op, directional only, optionally rescaled by operator-supplied calibration coefficients via `[green] calibration_file`. The measured backends are Alumet RAPL, Scaphandre RAPL, Kepler eBPF, Redfish BMC wall-plug, and cloud-provider CPU% mapped against SPECpower tables. When several cover the same service the daemon keeps the highest-fidelity reading, in that order, see the fidelity ladder in [docs/ENERGY.md](ENERGY.md#the-fidelity-ladder). The selected source is surfaced in `methodology.calibration.energy_source_models` so an auditor can verify which path produced E.
...
**Known limitation: 2x uncertainty bracket.** The carbon estimate ships with an explicit `2x` multiplicative bracket. The bracket signals that the directional model (especially the I/O proxy and the static grid tables) is unsuitable for regulatory-grade emissions reporting. Tightening the bracket requires a measured E term (any backend of the [ENERGY.md fidelity ladder](ENERGY.md#the-fidelity-ladder): Alumet or Scaphandre RAPL, Kepler eBPF, Redfish BMC, cloud SPECpower) and live Electricity Maps for the I term. The full uncertainty discussion lives in [docs/LIMITATIONS.md](LIMITATIONS.md).
...
- Connection and session management statements (`COMMIT`, `SET`, `SELECT set_config(...)`, pool pings) never reach this counter: the repetition detectors skip them, so no finding attributes them. They remain in `total_io_ops`, since a pooled driver executes one per connection checkout, but they are not avoidable and no batching remediation applies. See `docs/LIMITATIONS.md`, section "Session commands and waste ratio", for the exclusion boundary.
...
- `total_messaging_io_ops` and `avoidable_messaging_io_ops`: the same pair for message publishes, applied to a broker energy reading rather than a database one: `broker_waste = broker_energy * avoidable_messaging_io_ops / total_messaging_io_ops`. The broker energy is measured (Alumet on its cgroup), declared (a provisioned cluster read from the embedded SPECpower table) or estimated from the modeled publish energy, and the `model` tag on the figure says which. Like the database figure it is kept out of every total. Unlike the database figure, the direction of the error depends on the source: a measurement under-counts (Alumet's default attribution covers CPU only), a declaration bounds compute at full load without storage or network. See `docs/LIMITATIONS.md`.
- `database_waste`: that same formula computed on every run, tagged by `model`. When `[green.alumet.database]` is declared and the daemon received a reading, the energy is the measured window energy of the database cgroup (`model = "alumet_rapl"`). In that case, the optional declared region converts the waste to gCO2 with the region's grid intensity (Electricity Maps real-time when available, embedded annual otherwise) and provider PUE, hourly profiles not applied. When no database is declared at all (batch runs, or no `[green.alumet.database]`), the figure falls back to an estimate: the modeled energy of the window's SQL spans (verb multipliers, calibration and regional intensities included) multiplied by the SQL waste ratio, tagged `model = "estimated"`. A declared database with no delivered reading emits nothing. Its energy carries over to the next delivered window, so the period total never double-counts. Measured is a CPU-only lower bound, additional to the totals. Estimated reuses the per-span energy exactly as scoring resolved it (measured per-service sources when available, the I/O proxy with its 2x bracket otherwise) and is a re-presented share of `energy_kwh`/`co2`, never additional. The disclosure carries it as a separate `aggregate.database_waste` block (v1.4) with both thresholds and the provenance models, still outside every total. See the "Alumet precision bounds" section of [docs/LIMITATIONS.md](LIMITATIONS.md).
...
When the operator wires one of the measured backends (Alumet, Scaphandre, Kepler, Redfish, or the cloud-energy SPECpower scraper), perf-sentinel substitutes a measured per-service energy and switches the model tag accordingly (`alumet_rapl`, `scaphandre_rapl`, `kepler_ebpf`, `redfish_bmc`, `cloud_specpower`). The methodology section of a disclosure surfaces `scaphandre_used` and `specpower_table_version` so consumers know which path produced the numbers.
...
- `per_service_energy_models` and `per_service_measured_ratio` (in both `GreenSummary` per window and `Aggregate` over the period) surface the per-service fidelity view: which energy model fed each service and what fraction of its spans got measured.
...
Every disclosure ships with a `2x` multiplicative bracket on the carbon estimate. The bracket signals that the output is directional and unsuitable for regulatory-grade emissions reporting (CSRD, GHG Protocol Scope 3). The `notes.disclaimers` block of a disclosure reiterates this in operator-readable English, including the v1.0-specific limitations above.

...
- **Energy is recomputed via the proxy.** `total_energy_kwh` in the aggregate is derived from `total_io_ops * ENERGY_PER_IO_OP_KWH` regardless of which energy model the daemon used to score the source windows. When a deployment runs with Scaphandre or cloud SPECpower, the per-window CO2 in the source archives is measured but the aggregate energy line stays at the proxy estimate. A future schema revision will carry an `energy_source_models: Vec<String>` field.
- **Per-service CO2 attribution is region-blind.** The aggregator distributes the per-window total CO2 proportionally to per-service I/O share. When two services run in regions with very different grid intensities, this misallocates emissions. The exact per-region rows are available in the source `Report.green_summary.regions`, but the v1.0 schema does not surface them.
...
- **`_unattributed` bucket.** Windows whose `Report.per_endpoint_io_ops` is empty land in the `_unattributed` service. `disclose --strict-attribution` refuses such windows. Findings from those windows are also bucketed under `_unattributed` so a service is never published with `efficiency_score = 100` and non-zero anti-patterns.

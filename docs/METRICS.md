...
| Metric                                               | Type      | Labels                        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|------------------------------------------------------|-----------|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `perf_sentinel_findings_total`                       | counter   | `type`, `severity`, `service` | Findings detected since daemon start. `type` mirrors the `Finding.finding_type` enum, `severity` is `critical` / `warning` / `info`, `service` (since 0.18.0) is the emitting service, capped at 128 distinct services per daemon run with the overflow folded into `service="_other"` so the totals stay exact. With `[daemon] per_service_labels = false` the label is present but empty. Carries OpenMetrics exemplars when a `trace_id` is available, for 15 minutes after the batch that recorded one. |
...
| `perf_sentinel_slow_duration_seconds`                | histogram | `type`, `service`             | Duration histogram for spans exceeding the slow threshold, by event `type` (`sql`, `http_out` or `messaging`) and, since 0.18.0, `service` (capped at 64 distinct services, overflow folds into `service="_other"`, empty with `[daemon] per_service_labels = false`). Not pre-warmed under the default `per_service_labels = true`: a service's series appear on its first slow span (pre-minting the only label known up front, `service="_other"`, would advertise an overflow that never happened), so an absent series means no slow span yet, not a stopped worker. Only the knob-off unlabeled series is pre-warmed at zero as in 0.17. Buckets: 0.1, 0.25, 0.5, 0.75, 1, 1.5, 2, 3, 5, 10, 30 seconds. Used by Grafana `histogram_quantile()` for accurate percentiles across sharded daemon instances.                                                                     |
...
| `perf_sentinel_service_io_ops_total`                 | counter | `service` | Per-service cumulative I/O ops (read by every measured-energy scraper for per-service energy attribution). Label cardinality is capped at 1024 distinct services per daemon run, new services beyond the cap are not attributed. A span carrying no service name resolves to `service="unknown"` at ingestion (all four paths), rather than minting an empty label, and `service="_other"` is reserved as the analysis-side fold bucket, so a real service by that name merges into it. |
...
| `perf_sentinel_service_avoidable_io_ops_total`       | counter | `service` | Per-service share of `perf_sentinel_avoidable_io_ops`, derived from findings with the same dedup rule (since 0.18.0). Services past the 128-service analysis cap fold into `service="_other"`, so the sum across services always equals the global counter. Divide by `perf_sentinel_service_analyzed_io_ops_total` for a per-service waste ratio. A finding whose spans come from several services is split between them, each charged its own repeats and the finding's service (the first span in ingestion order) one less for the necessary call, rather than credited whole to one service.                                                                                                                                      |
| `perf_sentinel_service_analyzed_io_ops_total`        | counter | `service` | Per-service I/O ops of analysed traces (since 0.18.0): the waste-ratio denominator, from the same scoring pass, cap and green gating as the avoidable counter, unlike the ingest-side `perf_sentinel_service_io_ops_total` (which counts before analysis, drops past 1024 services and includes batches later shed).                                                                                                                                                                    |

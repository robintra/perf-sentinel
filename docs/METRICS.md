...
| `perf_sentinel_stored_findings`                | gauge     | (none)             | Distinct finding signatures currently retained in the query store, coalesced entries since 0.9.29 (gauge since 0.8.8). Pair with `perf_sentinel_max_retained_findings` for a headroom ratio.                                                                                                                          |
...
| `perf_sentinel_max_retained_findings`          | gauge     | (none)             | Configured cap of distinct finding signatures in the query store (`[daemon] max_retained_findings`), set once at startup (since 0.8.8). Pair with `perf_sentinel_stored_findings`.                                                                                                       |

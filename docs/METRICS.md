...
| `perf_sentinel_service_avoidable_io_ops_total`       | counter | `service` | Per-service share of `perf_sentinel_avoidable_io_ops`, derived from findings with the same dedup rule (since 0.18.0). Services past the 128-service analysis cap fold into `service="_other"`, so the sum across services always equals the global counter. Divide by `perf_sentinel_service_analyzed_io_ops_total` for a per-service waste ratio.                                                                                                                                      |

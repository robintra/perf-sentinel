...
| `perf_sentinel_analysis_grouping_overflow_total`     | counter   | (none)                        | Analysis-side series attributions (findings, avoidable and analysed I/O ops) folded into `grouping="_other"` because the 16-grouping cap was reached (since 0.19.0). Independent of the service cap: a series can fold on either axis or both. An ongoing increase means per-grouping attribution is coarsening for newly seen groupings, the totals and the per-service sums stay exact. |
| `perf_sentinel_slow_duration_grouping_overflow_total` | counter   | (none)                        | Slow spans folded into a `grouping="_other"` histogram series because the 8-grouping histogram cap was reached (since 0.19.0). Same contract as the service counter above, with the lowest cap of the three because the histogram now costs 14 series per (type, service, grouping) triple. |
...
| `perf_sentinel_service_io_ops_grouping_overflow_total` | counter | (none)  | I/O ops whose `grouping` folded into `_other` on `perf_sentinel_service_io_ops_total` because the 32-grouping ingest cap was reached (since 0.19.0). Nothing is dropped: per-service totals stay exact, only the per-grouping split coarsens. |
...
| `perf_sentinel_analysis_grouping_overflow_total > 0`, `perf_sentinel_slow_duration_grouping_overflow_total > 0` or `perf_sentinel_service_io_ops_grouping_overflow_total > 0` | Aggregate or reduce grouping values, or shorten `[detection] grouping_attributes` (the 16-, 8- and 32-series caps are fixed) |

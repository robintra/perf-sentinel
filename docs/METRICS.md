...
| `perf_sentinel_slow_duration_service_overflow_total` | counter | (none)          | Slow spans folded into the `service="_other"` histogram series because the 64-service histogram cap was reached (since 0.18.0). Same contract as the analysis overflow counter, with a lower cap because a histogram costs 13 series per (type, service) pair.                                                                       |

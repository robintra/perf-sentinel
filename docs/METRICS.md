...
| `perf_sentinel_analysis_service_overflow_total`      | counter   | (none)                        | Analysis-side series attributions (findings, avoidable I/O) folded into `service="_other"` because the 128-service cap was reached (since 0.18.0). An ongoing increase means per-service attribution is coarsening for newly seen services; the totals stay exact.                                                                                                                                                                                        |
...
| `perf_sentinel_service_avoidable_io_ops_total`       | counter | `service` | Per-service share of `perf_sentinel_avoidable_io_ops`, derived from findings with the same dedup rule (since 0.18.0). Services past the 128-service analysis cap fold into `service="_other"`, so the sum across services always equals the global counter. Divide by `perf_sentinel_service_io_ops_total` for a per-service waste ratio. |

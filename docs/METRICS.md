...
| `perf_sentinel_hub_export_dropped_total`       | counter   | (none)             | Invalid pending Hub entries, entries evicted at `hub_export.max_pending`, or a finding too large for the bounded payload. Alert on `rate(...) > 0`.                                                                                                                |

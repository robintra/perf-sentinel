...
RPC spans (`rpc.system`, e.g. gRPC or Dubbo) are checked last, after SQL and HTTP: they carry no statement or URL, so their target is `rpc.service/rpc.method` (falling back to the span name), and they enter the pipeline as `EventType::HttpOut` so the topological and occurrence detectors treat them as outbound calls. Only `SpanKind::Client` is admitted: the `rpc.*` keys are set on the inbound SERVER handler span too, and admitting those would double-count every hop and invent self-directed edges. This admission-only reuse keeps the two-variant `EventType` and the HTTP normalize/sanitize path unchanged.

...
- *The declaration owns the stretch it billed.* From the tick a fallback bills until the series recovers, any series delta covering that wall clock is dropped via `discard_pending`, gated on an outage marker carried by `StaticBrokerState`. The marker is what makes this safe to apply on a stale tick as well as on the recovery tick: it is only set once a declaration has actually billed, so the first stale tick still delivers whatever the series banked while it was live. Discarding on every fallback tick regardless of the marker would wipe those joules.

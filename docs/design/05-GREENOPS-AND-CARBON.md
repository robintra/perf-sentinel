...
**Why credit through `avoidable_by_service()`?** Neither detector keys its group on the service, so a template shared by two services inside one trace is one finding, owned by the service of the first span. `pattern.occurrences_by_service` records how many spans each service issued when there is more than one, and the winner's split is credited per service: each its own repeats, the owner one less for the necessary call. The sum is still `occurrences - 1`, so the per-service counters keep adding up to the global one, and a service does not read 0 % or above 100 % on the dashboard's per-service waste ratio for a template it shares. The owner is the first span in ingestion order, not the earliest timestamp, a known ceiling.
...
The denominator `accounted_io_ops` excludes the synthetic `unknown` bucket so the ratio is consistent with `operational_gco2` (which also excludes it). This keeps the numerator and denominator on the same accounting basis.
...
- `"sci_v1_numerator+transport"`: used on `co2.total`. The `(E × I) + M + T` footprint summed over traces, including when `T` is zero.
...
This preserves the **relative scale** (a 50% waste reduction yields a 50% drop in avoidable CO₂) without requiring per-finding region attribution. The trade-off is that when avoidable ops are concentrated in a high-intensity region, this ratio slightly under-attributes the savings. The simplification is documented as a known limitation and tagged at the data layer via `methodology: "sci_v1_operational_ratio"`.
...
Spans with no resolvable region land in a synthetic `"unknown"` bucket: zero operational CO₂ contribution. The `regions[]` breakdown still shows the bucket so users see the orphan I/O ops (the visible signal for troubleshooting, with detailed `tracing::debug!` messages available via `RUST_LOG=debug`).
...
A symmetric ±50% window (giving high = 1.5 × mid) would understate this uncertainty. The 2× multiplicative framing states that the true value is within a factor of 2 of `mid`, in either direction.
...
| OVHcloud | 1.24  | [OVHcloud KPIs FY25](https://www.ovhcloud.com/sites/default/files/external_files/kpis_fy25.pdf) (group average, September 2024 to August 2025). Measured over twelve months and audited by Apave against ISO 30134-2, covering 88% of the owned-and-operated fleet. Per-datacenter values span 1.19 (Strasbourg, Erith, Limburg) to 1.36 (Vint Hill). The table carries the group figure, as the three hyperscalers do                                        |
| Scaleway | 1.375 | [Scaleway Impact Report 2025](https://www-uploads.scaleway.com/Impact_Report2025_22ee3a8232.pdf) (2024 average over ten datacenters: four in France, three in the Netherlands, three in Poland). Per-datacenter values span 1.24 (WAW2) to 1.50 (WAW1, WAW3)                                                                                                                                                                                                  |
| OUTSCALE | 1.5   | **No published PUE.** 3DS OUTSCALE rents capacity rather than operating its own datacenters, so there is no fleet figure to cite and its regions fall back to the generic prior. Its partner Thesee advertises a 1.2 design PUE, which is neither a measured value nor OUTSCALE's own fleet, and the 1.64 figure that circulates traces back to a 2021 infographic that does not state it                                                                     |
...
The table holds one PUE per provider, not per datacenter, even where the provider publishes the finer grain. OVHcloud's 1.19 to 1.36 spread is an 8% swing around its group average, well inside the 2x bracket the whole estimate carries, so a per-region column would add a maintenance surface without moving a figure anyone reads.

### Carbon intensity data

Regional grid carbon intensities (gCO2eq/kWh) are sourced from [Electricity Maps](https://www.electricitymaps.com/) annual averages (2023-2024) and the [Cloud Carbon Footprint](https://www.cloudcarbonfootprint.org/) project. The table covers 17 AWS regions, 12 GCP regions, 6 Azure regions, 24 OVHcloud keys, 13 Scaleway keys, 5 OUTSCALE keys and 20 ISO country codes.

OVHcloud names one datacenter three ways depending on the API (the OpenStack Public Cloud region `GRA11`, the zone code `GRA`, the S3 location string `gra`), so several keys can point at the same grid. OUTSCALE keys carry an `outscale-` prefix because OUTSCALE reuses AWS region identifiers for different places: its `eu-west-2` is Paris where the AWS one is London, a factor of five on the same string. An OUTSCALE deployment therefore declares the prefixed key in `[green] default_region` or `[green.service_regions]`.
...
- `config/mod.rs`: `CloudEnergyConfig` and per-service `ServiceCloudConfig` (provider, region, instance_type, optional idle/max watts overrides).
- `table.rs`: embedded lookup table with idle and max watt values for ~390 instance types after the 2026-04-24 CCF refresh. All entries follow a single homogeneous methodology: `idle_watts = vCPU * idle_per_vCPU` and `max_watts = vCPU * max_per_vCPU`, with coefficients sourced per provider from `ccf-coefficients` 2026-04-24 (`coefficients-{aws,gcp,azure}-use.csv`). No baseboard overhead is reconstructed: the AWS-specific baseboard column was dropped by CCF in 2026-04-24 and is not added back. The 5 percent rule splits modern entries into two groups. Entries are re-aligned to CCF when the SPECpower direct compute diverged (Sapphire Rapids on AWS `m7i`/`c7i`/`r7i` and GCP `c3`, EPYC Genoa on AWS `m7a`/`c7a` and GCP `c3d`/`n2d`, Graviton 2/3/3E/4 mapped to CCF EPYC 2nd Gen proxy, EPYC Turin on AWS `m8a`/`c8a`, Emerald Rapids on GCP `c4`). They stay on `SPECpower_ssj 2008` direct compute 2024 Q1 - 2026 Q2 when within 5 percent or absent from the provider CSV (AWS Milan `m6a`/`c6a`, GCP Turin `c4d`, GCP Ampere Altra `t2a`, Azure Sapphire Rapids, Azure Emerald Rapids, Azure Genoa, Azure Cobalt 100, Sierra Forest). New AWS families added by this refresh: `m8a` / `c8a` (Turin), `m8i` / `c8i` (Emerald Rapids), `r7a` (Genoa memory-optimized). New GCP family: `c4a` (Axion ARM Neoverse V2, proxied to AWS Graviton 4). See `docs/LIMITATIONS.md`.
...
What that declared figure bounds is narrower than it reads. `SPECpower` covers CPU and baseboard, so it ceilings the declared vCPUs' compute and not the cluster's wall power, and a storage-bound broker can draw more. It also does not move when the application starts batching, which is a weakness for a figure meant to show remediation working. `MAX_BILLABLE_MS` truncates an idle stretch to one hour, so the declared figure under-counts there. All three are stated in `docs/LIMITATIONS.md`.

**Arbitration is the subtle part.** A measurement outranks a declaration, but "the measurement is live" is not the tick-level question it looks like.
...
- *Banked energy is real.* When the series goes quiet, whatever it banked while live is delivered before the declaration takes the window. A label that was never seen has nothing banked, which lets a typo fall through to the fallback.
- *The declaration owns the stretch it billed.* From the tick a fallback bills until the series recovers, any series delta covering that wall clock is dropped via `discard_pending`, gated on an outage marker carried by `StaticBrokerState`. The marker makes this safe to apply on a stale tick as well as on the recovery tick: it is only set once a declaration has billed, so the first stale tick still delivers whatever the series banked while it was live. Discarding on every fallback tick regardless of the marker would wipe those joules.

**The figure carries its provenance.** `DbEnergyContext.model` is filled by whichever source populated the window, because nothing downstream can infer it. Its `Default` is the weakest of the three tags (`estimated`), so a caller that forgets to state provenance cannot claim a measurement. The periodic disclosure keeps three buckets rather than two: folding a declared cluster into `measured_energy_kwh` would publish an operator's statement about provisioned hardware as a reading of the broker.
...
**Dashboard.** The Regions table in the GreenOps tab carries a sixth column `Estimated`. Three visual states: an orange `Estimated` badge when `intensity_estimated == true` (hover surfaces a tooltip with the `intensity_estimation_method`), a green `Measured` badge when `intensity_estimated == false`, and a neutral dash for rows whose `intensity_source` is not `real_time`. Annual / hourly / monthly_hourly profiles carry no estimation metadata, so the field stays `None` end-to-end. Both badges reuse the existing palette CSS variables (`--color-background-warning`, `--color-text-warning`, `--color-background-success`, `--color-text-success`) so dark and light themes adapt automatically.
...
**Messaging spans are unweighted (1.0x).** A publish is not given the HTTP tiers, even though `messaging.message.body.size` is ingested into the same `response_size_bytes` field. The tier boundaries encode a web payload distribution: 1 MB is Kafka's default `max.message.bytes`, a protocol ceiling rather than a large message, so under a default broker configuration nearly every message would land in the 0.8x tier. The result would be a near-constant 20% discount rather than a size-sensitive model. None of the three sources below measured a broker, so borrowing their ratios would extend their authority to a domain they never covered. The size attribute is ingested so a measured coefficient can be calibrated later through `calibrate` against an Alumet run on a broker bench, the same path that produced the SQL ratios.

**Why no per-publish coefficient can be a measurement.** The stronger reason is not the absence of a published figure but that a broker's power does not track its message rate. An ACM/SPEC ICPE 2026 study measured Kafka and Redpanda with Scaphandre over RAPL (3 brokers, replication factor 3, 4 KiB messages, 60 partitions per broker, on NVMe, SSD and HDD clusters). It reports that "Kafka exhibits a linear increase in power consumption up to about 20% of maximum throughput, after which it remains relatively stable regardless of additional load", attributing it to fixed thread pools, against roughly 50 % for Redpanda. The marginal energy of one publish therefore falls towards zero past that knee, and any per-operation coefficient is an amortized allocation of mostly fixed power rather than a measurement of marginal work. An older wattmeter study points the same way at the storage layer: on the Filebench Web workload, XFS sustains far more operations than Ext2 for only 29 % more power, roughly 6.75x less energy per operation for identical application work (Sehgal, Tarasov, Zadok, "Evaluating Performance and Energy in File System Server Workloads", USENIX FAST 2010). Energy per operation is dominated by fixed power and by the software stack, not by the content of the operation. This bounds what the whole proxy model can claim, SQL and HTTP coefficients included, and it is why the messaging coefficient asserts nothing.

Two Kafka defaults also break the naive "one publish equals one durable write" reading, in opposite directions. `flush.messages` defaults to `Long.MAX_VALUE` and the project itself recommends against setting it ("we recommend you not set this and use replication for durability and allow the operating system's background flush capabilities as it is more efficient"), so billing one fsync per publish is wrong. Conversely, the typical durable setup (replication factor 3, `min.insync.replicas=2`, `acks=all`) yields about one append per replica, and that fan-out is driven by `replication.factor`, not by `acks`.

**Standards footing.** The Green Software Foundation SCI specification (text v1.1.0, not the paid ISO/IEC 21031:2024 publication, which was not consulted) permits a modelled value: "In situations where there is a lack of access, capability, or rights to the necessary real-world data, the SCI allows for data generated through modeling, using best estimates instead." That is a conditional fallback tied to the data being inaccessible, not a free choice, and no GSF sub-specification covers broker or messaging workloads.
...
The default coefficient is `4e-11 kWh/byte` (0.04 kWh/GB), a conservative default below recent whole-network averages. The Sustainable Web Design Model v4 (2024) uses 0.059 kWh/GB operational (plus 0.013 embodied) for networks, a figure that includes access networks which cross-region server traffic does not traverse. Inter-datacenter coefficients run as low as 0.001 kWh/GB (the Cloud Carbon Footprint networking coefficient, applied to gigabytes in their estimator). Mytton, Lunden and Malmodin (2024, "Network energy use not directly proportional to data volume", Journal of Industrial Ecology 28(4):966-980, doi:10.1111/jiec.13512) demonstrate that the kWh/GB model is only valid for retrospective allocation: network equipment has significant fixed baseload power, so energy does not scale linearly with data volume (their Table 2 shows the Shift Project 2019 fixed-network model at 0.31-0.53 kWh/GB against Telefonica's actual 0.07 in 2020). The default is therefore an upper bound for cross-region server traffic. Since 0.9.25 the coefficient is fixed rather than configurable, so every disclosure scales transport identically, and the disclosure publishes the term under the sourced bracket too: 0.001 kWh/GB low (Cloud Carbon Footprint) and 0.059 kWh/GB high (SWDM v4 network segment), as `transport_kgco2eq_low` / `transport_kgco2eq_high`.
...
**Config.** No `[green]` key gates the transport term any more. `include_network_transport` and `network_energy_per_byte_kwh` are both deprecated and ignored since 0.9.25, parsed only to warn: gating the computation made two reports non-comparable, a configurable coefficient let two disclosures scale the same traffic differently, a zero silently erased the term, and a display toggle on a figure that is always disclosed had no remaining justification.
...
The design is read-heavy and write-rare:
...
A `confidence` field is stamped on every `Finding` in the JSON and SARIF report, indicating the source context of the detection. The value is set by the pipeline caller: `pipeline::analyze_with_traces` always sets `CiBatch` in batch mode, and `daemon::process_traces` derives it from `config.daemon_environment` in streaming mode. Detectors themselves never reason about confidence. They emit `Confidence::default()` and the caller overrides it.

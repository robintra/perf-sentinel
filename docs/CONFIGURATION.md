...
| `embodied_carbon_per_request_gco2` | float   | `0.001`  | SCI v1.0 `M` term: hardware manufacturing emissions amortized per request (per trace), in gCO₂eq. Region-independent. Must be > 0 since 0.9.25: no hardware has zero embodied carbon, and a zeroed coefficient would erase the M term from the disclosure. The applied value is published in `calibration_inputs`                                                                                                                                                                                                                                                                                                                                   |

...
| `api_key`       | string  | *(absent)*           | **Required when enabled**, and a config error otherwise, because the route writes. Constant-time compared against `X-API-Key`, 12 characters minimum, and `GET` also accepts `[daemon] read_api_key`. `PERF_SENTINEL_INCIDENTS_API_KEY` wins over this value |

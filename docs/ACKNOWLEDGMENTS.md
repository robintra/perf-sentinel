...
- `sanitized_endpoint` is `source_endpoint` with `/` and spaces replaced by `_` so the result splits cleanly on `:`.
  ...
  Migration note (0.9.22): entry points with no HTTP attribute (scheduled jobs, message consumers) now report the code frame (`com.foo.PurgeJob.execute`) instead of the literal `unknown`. Their signature changes, so an existing ack on an `unknown` endpoint must be re-captured. This also splits apart two jobs in one service that issued the same statement: they shared a single signature before, so acking one silently hid the other. Endpoints resolved from `http.route`, `http.url` or `url.full` are unchanged.

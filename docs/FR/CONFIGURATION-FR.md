...
| `memory_high_water_pct`   | entier   | `0`                         | Contrôle d'admission par pression mémoire, en pourcentage de la limite mémoire cgroup v2. Quand `memory.current / memory.max` franchit ce seuil, l'ingest OTLP est rejeté avec un statut retryable (compté sur `perf_sentinel_otlp_rejected_total{reason="memory_pressure"}`) jusqu'à ce que l'usage repasse dessous, bornant la RSS indépendamment de la profondeur de queue. `0` désactive le garde-fou (défaut). Linux/cgroup-v2 uniquement, inerte ailleurs. Réglez 80-85 pour garder de la marge au-dessus du régime permanent. Plage : 0 à 100         |

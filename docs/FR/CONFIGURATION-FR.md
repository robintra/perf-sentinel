...
| `window_minutes`     | entier   | `10`    | Fenêtre glissante en minutes sur laquelle les co-occurrences et les occurrences des sources sont comptées. Plage 1 à 10080 (7 jours). La mémoire ne croît pas avec elle : l'appariement ne garde qu'environ `lag_threshold_ms + 2 x trace_ttl_ms` de findings, et la fenêtre ne fixe que la période couverte par les compteurs par paire et par endpoint |

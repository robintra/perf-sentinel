...
| `include_network_transport`        | booléen  | `false`   | Affichage seul depuis la 0.9.25 : `false` masque le terme de transport dans le rapport CLI et le TUI, il ne le retire jamais des chiffres. Le terme est calculé, archivé et publié à chaque run, donc deux périodes restent comparables quel que soit le réglage. Il requiert `response_size_bytes` sur les spans HTTP (attribut OTel `http.response.body.size`) et la région cible mappée via `[green.service_regions]`. Les appels intra-région sont exclus. Le CO₂ transport apparaît comme `transport_gco2` dans le rapport JSON |
| `network_energy_per_byte_kwh`      | flottant | `4e-11`   | Énergie par octet pour le transport réseau (kWh/octet). Défaut 0.04 kWh/Go, une borne haute prudente pour le trafic serveur inter-régions (voir LIMITATIONS-FR.md, transport réseau). Appliqué à chaque appel HTTP inter-région portant une taille de réponse                                                                                                                                                                                                                                                                        |

...
| `network_energy_per_byte_kwh`      | flottant | `4e-11`   | Énergie par octet pour le transport réseau (kWh/octet). Défaut 0.04 kWh/Go, une borne haute prudente pour le trafic serveur inter-régions (voir LIMITATIONS-FR.md, transport réseau). Utilisé uniquement quand `include_network_transport = true`                                                                                                                                                                                              |
...
**Pourquoi `energy_interval_secs` existe.** C'est le champ à ne pas rater. L'exporteur d'Alumet publie chaque mesure comme une **jauge Prometheus contenant la dernière valeur flushée**, et `rapl_consumed_energy` est un `CounterDiff` : les joules brûlés pendant un `poll_interval` de la source, ni un compteur cumulatif ni une puissance. perf-sentinel divise par cet intervalle pour retrouver des watts. L'intervalle n'apparaît nulle part sur le fil, il doit donc être déclaré ici et correspondre à la configuration côté Alumet. **Un écart met l'énergie et le carbone à une échelle linéairement fausse, en silence** : déclarer `1.0` alors qu'Alumet échantillonne à `5s` surestime l'énergie d'un facteur 5, sans aucun avertissement. Voir `docs/FR/LIMITATIONS-FR.md#limites-de-précision-alumet`. Le démon affiche la valeur utilisée dans la ligne de log `Alumet scraper started`.
...
| `sampling_rate`           | flottant | `1.0`                       | Fraction des traces à analyser (0.0 à 1.0). Réduire en dessous de 1.0 pour diminuer la charge dans les environnements à fort trafic. Les traces sont gardées ou jetées entières sur un hachage du trace id, donc les détecteurs par trace restent corrects sur ce qui reste, mais tous les agrégats (ratio de gaspillage I/O, IIS, chiffres GreenOps et carbone, compteurs Prometheus) décrivent alors cette fraction du trafic sans être remis à l'échelle. Sous 1.0, le daemon émet une entrée `tuning` dans `Report.warning_details` qui le signale. Un sampling fait par un collector en amont a le même effet et ne peut pas être détecté, voir [HELM-DEPLOYMENT-FR.md](HELM-DEPLOYMENT-FR.md#sampling-du-collector-et-ce-qui-atteint-le-daemon)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

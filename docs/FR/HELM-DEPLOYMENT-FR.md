...
**Dimensionnez le PVC pour l'archive.** Aux valeurs par défaut, l'archive par fenêtre garde un fichier actif plus `max_files = 12` fichiers tournés de `max_size_mb = 100` chacun, soit environ 1,3 Go, alors que la demande par défaut du chart est de `1Gi`. Un volume plein n'arrête pas le daemon : l'écrivain de l'archive abandonne les fenêtres qu'il ne peut pas écrire et les compte dans `perf_sentinel_archive_windows_dropped_total{reason="write_error"}`. Passez `workload.statefulset.persistence.size` à `2Gi` ou plus, ou posez `persistence.manageDaemonPaths: false` et baissez `[daemon.archive] max_files` ou `max_size_mb` dans `config.toml`.

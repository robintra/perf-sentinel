...
Vérification de conformité au 2026-06-09 :

- **GitHub Actions** : 100 % des lignes `uses:` à travers les 11
  workflows de `.github/workflows/` sont épinglées à un commit SHA de
  40 caractères, avec le tag lisible en commentaire de fin de ligne.
- **Dockerfile** : l'image de production est `FROM scratch`, sans
  image de base externe à épingler. La seule action Docker invoquée
  depuis la CI (`zricethezav/gitleaks` dans `ci.yml`) est épinglée par
  digest.
- **Dépendances Cargo** : `Cargo.lock` est commité et suivi. Le
...
- **Permissions** : chaque workflow déclare `permissions:` au
  ...
  larges activés uniquement par job quand c'est nécessaire (release,
  packages, attestations).
- **Dependabot** : configuré pour `github-actions` dans
  `.github/dependabot.yml`, planification hebdomadaire le lundi,
  groupé par propriétaire upstream pour garder le diff cohérent.
...
de 40 caractères. Le tag semver va dans un commentaire de fin de
ligne pour que les relecteurs puissent lire la version d'un coup
d'œil :
...
à tout moment, et tous les workflows qui épinglaient `@v6`
exécutent immédiatement le nouveau code. Un SHA est
content-addressable : le réécrire nécessite une collision SHA-1, ce
qui n'est à la portée d'aucun attaquant connu.

### Images Docker

Quand un Dockerfile ou un workflow référence une image externe,
épinglez le digest du contenu :
...
rien à épingler dans l'image elle-même. Le binaire copié dedans
(`build/linux-${TARGETARCH}/perf-sentinel`) est construit depuis ce
dépôt même, avec `Cargo.lock` qui pilote l'ensemble de ses
dépendances.
...
n'activent des scopes additionnels que lorsque c'est nécessaire :
...
`permissions: write-all` au niveau workflow dans le dépôt.
...
Les dépendances Cargo sont exclues de Dependabot : la combinaison
`Cargo.lock` plus `cargo audit` quotidien couvre déjà l'angle
sécurité, et le volume de mises à jour de patch que Dependabot
générerait sur un workspace de 200+ crates est mal rentabilisé pour
un projet de cette taille. Les mises à jour Cargo sont gérées
...
Dependabot gère les mises à jour de routine. Pour une mise à jour à
la main (correctif de sécurité hors cycle hebdomadaire, ou nouvelle
action que Dependabot n'a pas encore prise en charge), résolvez le
SHA via l'API GitHub :
...
corresponde au nouveau tag. Un SHA avec un commentaire périmé est
...
2. **Triage** : lisez l'advisory, lancez `cargo tree -i <crate>` pour
   confirmer si la version affectée est compilée dans le binaire (le
   paragraphe `RUSTSEC-2026-0097` dans `audit.toml` est l'exemple
   canonique de la profondeur d'analyse attendue).

3. **Remédiation** : mettez à jour la dépendance dans `Cargo.toml` si
   le correctif est upstream, lancez `cargo update -p <crate>`,
   vérifiez avec `cargo audit`, ouvrez une PR avec préfixe
   `chore(deps)`.

4. **Acquittement** : si le chemin de code affecté n'est pas exercé,
   ...
   un réexamen. Ne pas ignorer silencieusement.

5. **Divulgation** : voir `SECURITY.md` pour le processus complet de
   divulgation coordonnée et la matrice des versions prises en charge.
...
- **Cosign** est l'outil CLI exécuté localement (ou par GitHub Actions en CI). Il ouvre un flux OIDC dans le navigateur, signe l'artefact, et envoie la signature à Sigstore.
...
depuis le 24 février 2025. Au moment de la migration, il n'avait
publié aucune release depuis 15 mois, et toutes ses actions internes
étaient encore sur Node.js 20 alors que les runners GitHub-hosted
...
- Nouvelle ligne `uses:` ? Doit épingler un SHA de 40 caractères, tag
  en commentaire de fin de ligne.
- Nouvelle ligne `FROM` dans un Dockerfile ? Doit épingler
...
- Nouveau workflow ? Bloc `permissions:` au niveau job, par défaut
  `contents: read`, scopes plus larges activés uniquement quand
  c'est nécessaire.
- `permissions: write-all` au niveau workflow ? À rejeter. Utiliser
  des scopes au niveau job à la place.

Les commandes de vérification ci-dessus peuvent tourner localement
avant de pousser pour s'assurer que la PR est propre.

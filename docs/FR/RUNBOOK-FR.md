...
3. **Traces pathologiques.** Une seule trace avec des milliers de spans consomme de la RAM. `max_events_per_trace` (défaut 1000) plafonne, vérifiez qu'il n'a pas été augmenté. Depuis 0.11.2 il plafonne indépendamment trois collections par trace, l'anneau d'événements, les contextes d'endpoint entrant retenus et l'index d'ancêtres, la même valeur retient donc une enveloppe plus grande qu'en 0.11.1. Le texte SQL surdimensionné venant d'un émetteur hostile ou verbeux est aussi borné par champ à l'ingestion (64 KiB par cible), mais 1000 événements de ce type dans une trace s'additionnent : baissez `max_events_per_trace` ou `max_active_traces` si un émetteur déviant est suspecté.

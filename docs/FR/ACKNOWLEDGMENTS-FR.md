...
- `sanitized_endpoint` est `source_endpoint` avec `/` et espaces remplacés par `_` pour que le résultat se split proprement sur `:`.
  ...
  Note de migration (0.11.2) : l'attribution d'endpoint OTLP sélectionne désormais la route la plus externe d'une chaîne contiguë du même service, et le daemon conserve un contexte parent borné entre les requêtes d'export. Un finding qui utilisait auparavant une route interne ou `unknown` change donc de signature. Re-capturez les acquittements et baselines de rapports persistés concernés avec 0.11.2 ; les corpus de traces ré-analysés des deux côtés par le même binaire ne churnent pas.

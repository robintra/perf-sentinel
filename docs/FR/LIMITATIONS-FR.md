...
Une voie distincte met malgré tout un chiffre d'énergie de broker dans le rapport, et elle ne passe par aucun coefficient. Déclarer `[green.alumet.broker]` mesure le cgroup du broker, et `[green.broker_static]` dérive des watts depuis un cluster déclaré. Dans les deux cas l'énergie est multipliée par le ratio de gaspillage messaging et rapportée en `messaging_waste`, une borne basse exclue de tous les totaux, avec une étiquette `model` qui distingue mesuré, déclaré et estimé. La voie déclarée est un majorant qui ne réagit pas au batching, voir `docs/FR/CONFIGURATION-FR.md`. L'énergie de transport réseau n'est pas estimée du tout pour le messaging, car l'extraction de nom d'hôte qui la pilote ne comprend que les cibles `http://` et `https://`.

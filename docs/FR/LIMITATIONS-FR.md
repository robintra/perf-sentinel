...
- [Nombre maximum d'événements par trace](#nombre-maximum-dévénements-par-trace) : plafond du ring buffer par trace.
...
- [Limites de longueur des champs à l'ingestion](#limites-de-longueur-des-champs-à-lingestion) : plafonds en octets appliqués à la frontière d'ingestion.
...
- [Ingestion automatisée mysql-stat depuis Prometheus](#ingestion-automatisée-mysql-stat-depuis-prometheus) : collecteur désactivé par défaut, et chaque compteur qu'il ne publie pas laisse sa colonne à zéro.
- [Secrets et credentials](#secrets-et-credentials) : pattern env-var-prioritaire pour les scrapers.
- [API Electricity Maps](#api-electricity-maps) : gestion de la clé d'API et réserves.
...
Chaque finding dérive d'un span normalisé. perf-sentinel lit une liste fermée d'attributs porteurs (le texte de la requête `db.statement` / `db.query.text`, l'URL cible `http.url` / `url.full`, plus les attributs d'enrichissement listés dans [Attributs de span requis](./INSTRUMENTATION-FR.md#attributs-de-span-requis)). Un span qui n'en porte aucun n'est pas une opération d'I/O et est ignoré. Un span SQL qui *est* une opération d'I/O mais arrive sans texte de requête, ou un span HTTP sans URL, est lui aussi ignoré : il n'y a rien à normaliser, donc aucun finding ne peut être produit. Depuis 0.11.2, le kind du span écarte un cas de plus même quand l'attribut est présent. L'URL d'un span `SERVER` décrit la requête à laquelle le service a répondu et non un appel qu'il a émis, donc elle ne produit aucun événement HTTP sortant. Elle fournit toujours l'endpoint entrant auquel les autres findings sont rattachés. La détection est bornée par la qualité de l'instrumentation en amont, de la même manière que tout outil purement logiciel est borné par sa source de mesure.
...
perf-sentinel modélise trois types d'I/O, le SQL relationnel, les appels sortants (HTTP et RPC comme gRPC) et les publications de messages. Un span dont le `db.system` désigne un datastore non-SQL (`redis`, `memcached`, `mongodb`, `cassandra`, `dynamodb`, `couchbase`, `couchdb`, `elasticsearch`, `opensearch`, `neo4j`, `hbase`, `geode`, `influxdb`) est écarté à l'ingestion. Son `db.statement` n'est pas du SQL relationnel et le tokenizer SQL le déformerait (un `GET user:123` Redis n'est pas un template de requête). L'écart est décidé sur le seul `db.system`, donc un span non-SQL est écarté qu'il porte ou non un statement ou une URL, de façon cohérente sur les chemins OTLP, Jaeger et Zipkin. L'écarter est le choix de réduction du risque : cela évite de faux findings N+1 et redundant sur le trafic cache ou document. Un `db.system` absent ou désignant un moteur SQL (`postgresql`, `mysql`, `mssql`, `oracle`, `clickhouse`, `cockroachdb`, ...) est toujours traité comme du SQL, donc aucun trafic relationnel n'est écarté par erreur.

Comme le span n'entre jamais dans le pipeline, un appel non-SQL écarté est aussi invisible aux détecteurs structurels (fanout excessif, appels sérialisés) : une requête qui éclate en de nombreux appels cache ou document ne lèvera pas ces findings. Sur le chemin OTLP, l'écart est compté sous la raison dédiée `non_sql_datastore` dans `perf_sentinel_otlp_spans_filtered_total`, gardée distincte de `not_io` pour qu'une flotte purement cache ne déclenche pas l'avertissement daemon de rétention nulle, qui ne compte que les raisons de manque d'instrumentation.

## Messaging : côté producteur seulement, pas d'analyse des consommateurs

Les spans de broker sont admis à partir de la convention OTel messaging (`messaging.system`, `messaging.destination.name`), une seule convention qui couvre en principe indifféremment Kafka, RabbitMQ, Pulsar, SQS, NATS et JMS. Seul le span `PRODUCER` est admis, comme un appel sortant dont la cible est la destination.

L'admission suit un ordre fixe : un statement SQL gagne, puis une URL sortante (`http.url`, `url.full`) sur tout span qui n'est pas `SERVER`, puis un appelé RPC sur un span `CLIENT`, puis le messaging. Un émetteur qui pose `url.full` sur son span de publication, ce que font certaines versions du SDK AWS pour SQS, atterrit donc sur le chemin HTTP, où l'URL de queue passe par le normaliseur de chemins et son identifiant de compte est masqué en `{id}`. Le chemin messaging est vérifié contre des spans construits à la main pour la forme que décrit la convention, pas contre des captures de l'agent de chaque broker : lisez donc la liste ci-dessus comme les conventions couvertes, pas comme une garantie par agent. Si les spans d'un broker n'apparaissent pas en findings messaging, vérifiez si l'agent pose aussi `url.full` ou `db.system` dessus. Un span `CONSUMER` (`process`) et un span messaging `CLIENT` (`receive`, `settle`) décrivent un travail effectué sur un message reçu plutôt qu'un appel émis par le service, et un consommateur qui fait du polling noierait les détecteurs d'occurrences sous des publications qu'il n'a jamais faites. Ces spans sont écartés sous la raison `not_io`. Une exception : un span `CLIENT` de poll qui porte aussi `rpc.*`, ce que le SDK AWS pose sur le `ReceiveMessage` SQS, correspond à la branche RPC au-dessus et est admis comme appel sortant nommé d'après la méthode d'API. Un service purement consommateur se lit donc comme n'ayant aucune I/O messaging, mais peut tout de même montrer des findings HTTP pour sa boucle de polling.

Une publication ne porte aucun paramètre bindé, donc la règle N+1 pour le messaging est le seul compte d'occurrences. N publications vers une même destination dans la fenêtre de détection sont rapportées en `n_plus_one_messaging` avec un compte de paramètres distincts de 0, puisque la variation vit dans la charge utile et que les charges utiles ne sont jamais lues. C'est plus large que les règles SQL et HTTP, dont la variance des paramètres permet de distinguer une boucle d'une répétition légitime. Un service qui groupe volontairement au niveau applicatif sera signalé, et la suggestion d'utiliser l'API de batch du broker est la lecture attendue. Relevez `n_plus_one_threshold` si le bruit l'emporte.

Il n'existe pas de type `redundant_messaging`. La détection de redondance groupe sur `(type, template, params)`, et avec des params toujours vides cette clé se réduirait à la seule destination : deux publications vers une même destination seraient rapportées comme des doublons avec une suggestion "mettre en cache ou dédupliquer". Deux messages ne sont jamais le même message, donc les spans de publication sont entièrement ignorés par ce détecteur. Une publication répétée ressort en `n_plus_one_messaging` ou pas du tout.

Une hypothèse sous-jacente aux deux règles ne tient pas uniformément selon les brokers. Le pipeline compte un span pour un aller-retour réseau, ce qui est vrai pour un `SendMessage` SQS et pour un `basic_publish` RabbitMQ, mais pas pour Kafka. `KafkaProducer.send()` accumule en mémoire et le client écrit selon `batch.size` et `linger.ms`, donc cinq spans de publication peuvent être une seule écriture sur le réseau. Sur Kafka, le compte d'occurrences, le compte d'I/O évitables et le ratio de gaspillage qui en découle sont donc des majorants, et "envoyez-les en un seul lot" peut décrire une optimisation que le client applique déjà. `slow_messaging` hérite de la même réserve : un span producteur Kafka mesure souvent l'enfilement plutôt que l'acquittement du broker.

Les destinations ne sont pas normalisées. Un nom de topic ou de queue est déjà un template, il est donc comparé tel quel plutôt que passé par le normaliseur de chemins HTTP, qui masquerait les segments numériques en `{id}` et retirerait l'identifiant de compte d'un ARN de queue SQS. Le compromis est inverse : un déploiement qui nomme un topic par tenant produit un template par tenant, donc une boucle de publication est éclatée entre plusieurs groupes et peut passer sous le seuil. Le sous-groupement est le bon côté de cette erreur, puisqu'il ne fusionne jamais deux tenants dans un même finding.

Le modèle carbone donne à une publication le coefficient par opération non pondéré. Les paliers de taille HTTP ne sont pas réutilisés. Leurs bornes à 10 Ko et 1 Mo encodent une distribution de charges utiles web, et 1 Mo est le `max.message.bytes` par défaut de Kafka, un plafond protocolaire plutôt qu'un gros message. Les appliquer affirmerait une remise quasi constante qu'aucune mesure ne fonde. `messaging.message.body.size` reste ingéré pour le jour où un coefficient mesuré existera.

Une voie distincte met malgré tout un chiffre d'énergie de broker dans le rapport, et elle ne passe par aucun coefficient. Déclarer `[green.alumet.broker]` mesure le cgroup du broker, et `[green.broker_static]` dérive des watts depuis un cluster déclaré. Dans les deux cas l'énergie est multipliée par le ratio de gaspillage messaging et rapportée en `messaging_waste`, exclue de tous les totaux, avec une étiquette `model` qui dit d'où elle vient. Lisez cette étiquette avant de lire le nombre, car les trois sources ne se trompent pas dans le même sens. `alumet_rapl` mesure le cgroup du broker via Alumet, dont la formule d'attribution par défaut ne couvre que le CPU, donc il sous-compte. `broker_specpower` borne les vCPU déclarés à pleine charge, à partir d'une table SPECpower qui couvre le CPU et la carte mère mais ni le stockage, ni le réseau, ni l'alimentation. C'est donc un plafond sur le calcul et non sur la consommation murale du cluster, et un broker limité par le stockage peut consommer davantage. Ce chiffre ne bouge pas non plus quand l'application se met à grouper ses publications, et une longue période d'inactivité est tronquée à une heure de facturation, donc il sous-compte là. `estimated` re-présente une part des totaux que les spans ont déjà produits. Voir `docs/FR/CONFIGURATION-FR.md`. L'énergie de transport réseau n'est pas estimée du tout pour le messaging, car l'extraction de nom d'hôte qui la pilote ne comprend que les cibles `http://` et `https://`.
...
## Messaging : traces producteur et consommateur reliées, pas fusionnées

Un broker relie un producteur à son consommateur par un span link OpenTelemetry plutôt que par une relation parent-enfant, et le consommateur démarre le plus souvent sa propre trace. perf-sentinel lit ce lien, depuis l'ancêtre `CONSUMER` le plus proche d'un span d'I/O, et le porte en `link_trace_id` sur les événements du handler consommateur. L'arbre `explain` le rend sous forme d'une ligne `triggered by trace <id>`, en CLI, dans `/api/explain/{trace_id}` et dans la TUI. Le dashboard HTML ne le porte pas, car ses spans embarqués n'ont pas ce champ.

Les deux traces ne sont **pas** fusionnées. Fusionner poserait sur les findings côté consommateur un identifiant de trace qui ne contient pas ces spans une fois collé dans Jaeger ou Tempo. Cela ferait aussi déclencher `serialized_calls` sur des spans consommateur dont l'ordonnancement appartient au broker et non à l'appelant, et gonflerait `chatty_service` sur toute chaîne asynchrone. En mode daemon, la fenêtre du producteur est de plus évincée après `trace_ttl_ms` (30 s par défaut), bien en deçà d'un lag de broker normal, donc une fusion n'attraperait jamais que les chaînes les plus rapides.
...
requête OTLP et le même index de service. Le `BatchSpanProcessor` par défaut
...
et le parent consommateur dans le suivant. Le lien est alors absent, ce qui
explique qu'un même handler affiche la ligne sur un run et pas sur le suivant.
...
Les ORM qui utilisent des paramètres nommés (Entity Framework Core avec `@__param_0`, Hibernate avec `?1`) produisent des spans SQL où les valeurs des paramètres ne sont pas visibles dans l'attribut `db.statement`/`db.query.text`. perf-sentinel voit le template avec les placeholders mais pas les valeurs réelles.

Cela signifie que les patterns N+1 (même requête, valeurs différentes) peuvent être classifiés comme `redundant_sql` (même requête, mêmes params visibles) au lieu de `n_plus_one_sql` (même requête, params différents). Les deux findings identifient correctement le pattern de requêtes répétées et la suggestion de regrouper ou de mettre en cache reste valide.
...
La même limitation s'applique au RPC (gRPC, Dubbo). La charge utile d'une requête gRPC vit dans le corps du message protobuf, pas dans un attribut de span, donc N appels distincts à une même méthode partagent un unique template à paramètres vides et sont lus comme `redundant_http` plutôt que `n_plus_one_http`. Les spans RPC sont aussi modélisés comme des appels HTTP sortants (ils ne portent ni statement ni URL), donc leurs findings sortent sous les types `_http` avec une remédiation teintée HTTP. Seul le côté sortant `SpanKind::Client` est admis (le span du handler entrant porte les mêmes clés `rpc.*` mais représente du travail entrant). Voir [INSTRUMENTATION-FR.md](./INSTRUMENTATION-FR.md#attributs-de-span-requis).

## Redaction de la query string HTTP et visibilité des N+1

La détection des N+1 HTTP dépend de la visibilité du paramètre variable dans le span. Une boucle N+1 qui fait varier un segment de path (`GET /api/orders/1`, `/api/orders/2`, ...) se normalise en `GET /api/orders/{id}` avec des params extraits distincts, et est détectée. Une boucle N+1 qui fait varier un paramètre de query (`GET /api/mock?seq=1`, `?seq=2`, ...) n'est détectée que si la query string arrive intacte jusqu'au span.

Certaines instrumentations masquent la query string avant l'export. OpenTelemetry .NET `System.Net.Http` la masque en `?*` par défaut (désactivable avec `OTEL_DOTNET_EXPERIMENTAL_HTTPCLIENT_DISABLE_URL_QUERY_REDACTION=true`). Quand la query est masquée, chaque appel de la boucle porte un `url.full` identique au byte près, donc perf-sentinel voit le pattern comme `redundant_http` (même URL répétée) et non `n_plus_one_http` (même URL, paramètre différent). Le paramètre variable a été détruit en amont, donc aucun consommateur de traces (Jaeger, Tempo, ou n'importe quel backend OTLP) ne peut le récupérer, pas seulement perf-sentinel.

Les deux verdicts identifient le pattern d'appels répétés et la suggestion de regrouper les appels reste valide. Pour obtenir `n_plus_one_http` spécifiquement sur .NET, désactivez le masquage de la query via la variable d'environnement ci-dessus, ou modélisez l'identifiant variable comme un segment de path plutôt qu'un paramètre de query.

## Compromis du regroupement par host HTTP

Le template HTTP sortant garde le host de l'appelé (`GET user-svc/api/x`), donc deux appels au même chemin sur des backends différents restent des groupes séparés au lieu de fusionner en un faux `redundant_http`. Le coût inverse est qu'un vrai N+1 peut se scinder si un même backend est adressé avec une orthographe de host incohérente dans une trace. Les IP littérales IPv4/IPv6 sont retirées (les replicas load-balancés continuent de se dédupliquer), le point final DNS est canonicalisé (`svc.` == `svc`) et le host est mis en minuscules. Un nom court et sa forme pleinement qualifiée (`user-svc` vs `user-svc.default.svc.cluster.local`) ne sont en revanche pas réconciliés, donc une boucle qui mélange les deux orthographes contre un même backend peut se scinder en groupes sous le seuil et passer inaperçue. De même, une boucle par item qui éclate vers des backends aux noms distincts sur le même chemin (`GET shard-1/lookup`, `GET shard-2/lookup`, ...) est traitée comme des opérations distinctes plutôt qu'un N+1, puisque ces appels ne peuvent pas être regroupés en un seul endpoint. Émettre une orthographe de host cohérente par appelé garde le regroupement N+1 intact.
...
Le ratio de gaspillage mesure combien d'I/O pourraient être éliminées (N+1, redondant), tandis que les findings lents mettent en évidence des opérations nécessitant une optimisation (indexation, cache) plutôt qu'une élimination.
...
Elles restent des opérations d'I/O et demeurent dans `total_io_ops`, dans l'attribution par service et dans le total carbone, parce que ce travail a bien lieu. Elles sortent en revanche de `avoidable_io_ops`, dérivé des findings. Un déploiement qui monte de version voit donc baisser `io_waste_ratio`, ainsi que `estimated_optimization_potential_kgco2eq` et l'énergie évitable publiée, sans que l'application ait changé. Un seuil `[thresholds] io_waste_ratio_max` calibré avant ce changement devient plus permissif que prévu et doit être recalibré.

L'exclusion est étroite par construction : elle ne couvre que ce qu'un driver émet une fois par acquisition de connexion. Les statements émis une fois par ligne ou par entité restent visibles, puisque les regrouper est un vrai correctif. `SELECT 1` n'est pas traité comme un ping de pool (il se normalise vers le même template que la sélection de n'importe quel littéral). Ce n'est pas non plus le cas de `SHOW COLUMNS` (introspection de schéma, une par entité), de `SELECT @@IDENTITY` (clés générées, une par insertion) ni du bloc anonyme Oracle `BEGIN pkg.proc(?); END;` (un appel de procédure stockée, et un N+1 de ces appels est réel).

## Interprétation des scores

La CLI affiche un qualificatif `(healthy / moderate / high / critical)` à côté de `io_intensity_score` et `io_waste_ratio` et la même classification est émise dans le rapport JSON sous forme de champs frères `io_intensity_band` et `io_waste_ratio_band`. Les tables de référence sont dans le README principal.
...
le plus proche qui le nomme, sous la forme `<messaging.system> <destination>`
...
service qui lit un seul exchange par ces derniers uniquement entre de nouveau
en collision sous ce nom. Une destination marquée temporaire ou anonyme, une
file nommée par le serveur (`amq.gen-*`, `spring.gen-*`), un substitut entre
chevrons tel que `<default>` (qu'une instrumentation émet pour un exchange par
défaut) ou une destination contenant `?`, `#` ou `@` ne nomme aucune origine
stable et laisse `"unknown"`. Une instrumentation peut ne poser aucun des deux
indicateurs (l'agent Java OpenTelemetry entre autres), raison pour laquelle les
...
L'échantillonnage en amont s'applique **avant** perf-sentinel, dans le SDK ou le collecteur. Il est distinct du réglage `sampling_rate` propre au daemon, traité plus bas.
...
- Pour les quality gates CI, utilisez le mode batch (`perf-sentinel analyze`) sur des traces intégralement capturées. Une gate ne doit pas décider sur 1% du trafic.
...
Pour les traces avec un très grand nombre d'événements, augmentez `max_events_per_trace` ou cherchez pourquoi une seule trace génère autant d'opérations.

## Traces longues et éviction TTL en mode daemon

La fenêtre de détection en streaming évince une trace lorsqu'elle est restée inactive pendant `trace_ttl_ms` (défaut 30s). "Inactive" signifie qu'aucun événement de span pour ce `trace_id` n'a été ingéré dans le TTL. Le TTL actif est réinitialisé à chaque ingestion de span, donc une trace qui émet un span toutes les <30s reste vivante indéfiniment.

Mais une trace qui émet des spans creux et espacés (par exemple un job batch long qui émet un span toutes les 60s, ou un websocket en long polling) sera évincée entre les rafales. Un span tardif portant le même `trace_id` qui arrive après l'éviction crée un **nouveau** bucket de trace. Les événements précédents sont perdus. Les détections pilotées par seuil qui s'appuient sur des spans co-localisés dans une même trace (`n_plus_one`, `chatty_service`, `excessive_fanout`, `pool_saturation`, `serialized_calls`) sous-rapportent silencieusement parce que chaque fragment passe sous le seuil par trace.

Atténuations, par ordre de précision :

- **Augmentez `trace_ttl_ms`** si vous connaissez l'écart maximum attendu entre rafales (`[daemon] trace_ttl_ms = 120000` pour 2 minutes). La mémoire croît avec `max_active_traces`, pas avec le TTL : un TTL plus long ne coûte rien tant que le profil de trafic ne dépasse pas le plafond LRU.
...
detect+score tournent sur un unique worker d'analyse dédié, découplé de la boucle `select!` d'ingestion / éviction par un canal borné (1024 lots). La boucle enfile sans bloquer les lots évincés et expirés, donc une passe d'analyse lente ne peut pas bloquer l'ingestion ni retarder l'éviction TTL. En contrepartie, lorsque l'analyse ne suit pas sous charge soutenue, la file se remplit et des lots entiers sont **délestés**. Le délestage est explicite et métré, pas silencieux :
...
Un lot délesté est totalement écarté de la détection : ses findings ne sont jamais émis et le corrélateur cross-trace ne le voit jamais. Comme perf-sentinel fait remonter des motifs *récurrents*, un N+1 ou un chemin bavard délesté est normalement redétecté à la requête suivante une fois le worker rattrapé. Un délestage soutenu signale un daemon sous-dimensionné pour le volume de traces : répartissez la charge sur plusieurs instances (shard par `trace_id`), gardez de la marge via `sampling_rate`, ou réduisez le coût par trace en amont.

Le délestage répond à la *surcharge*, pas à une *panne*. Si le worker d'analyse lui-même s'arrête (par exemple un détecteur qui panique sur une trace pathologique), le daemon ne reste pas debout à n'analyser plus rien : il sort en erreur pour qu'un superviseur (Kubernetes, systemd) redémarre le process. C'est le même comportement fail-loud que l'ancienne détection en ligne, où une panique faisait planter tout le daemon. Tout lot enfilé dans la brève fenêtre avant la sortie est compté comme délesté plutôt que perdu silencieusement.
...
**Un arrêt gracieux draine, un kill non gracieux non.** Sur un arrêt propre, le daemon draine sa fenêtre à travers la détection avant de quitter. SIGINT (Ctrl+C) et, sous Unix, SIGTERM déclenchent tous deux ce drain, donc une terminaison de pod Kubernetes normale (rolling update, scale-down) draine la fenêtre en vol au lieu de la jeter. Une mort *non gracieuse* la perd quand même : SIGKILL (le kill forcé du kubelet après la période de grâce de terminaison), un OOM kill ou un crash du processus sautent le drain et jettent les traces en vol, jusqu'à une fenêtre entière, sans reprise.

L'impact pratique est faible. Ce qui est en vol à cet instant, ce sont des traces incomplètes (elles n'ont pas atteint leur TTL, donc elles reçoivent peut-être encore des spans). perf-sentinel fait aussi remonter des patterns *récurrents* : un N+1 ou un chemin bavard que la fenêtre jetée aurait signalé réapparaît à la requête suivante et est capturé par le nouveau processus en quelques secondes. Les données de trace ne sont pas perdues non plus : elles vivent en amont dans votre collecteur ou votre store de traces. Les acknowledgments runtime sont sur disque et survivent (voir [mode StatefulSet](./HELM-DEPLOYMENT-FR.md#statefulset)). Le seul endroit où un trou est visible est l'archive NDJSON par fenêtre (opt-in), qui manque la fenêtre en vol au moment du kill. Si cela compte, gardez `trace_ttl_ms` court, ou exécutez les gates en mode batch où il n'y a pas de fenêtre à perdre.
...
`[daemon] max_payload_size` (défaut 1 Mio) plafonne les batches NDJSON individuels soumis au socket JSON. Une seule connexion peut transmettre plusieurs batches avant de se fermer et le daemon tolère jusqu'à **16× `max_payload_size`** par connexion avant de tronquer le flux. Avec les valeurs par défaut, cela veut dire qu'une connexion peut transférer jusqu'à 16 Mio de données de traces.

Ce facteur prévoit le cas des clients qui émettent beaucoup de petits batches sur une connexion longue durée (par exemple un sidecar qui vide une file d'attente bufferisée après un flush), sans exposer le daemon à un épuisement mémoire provoqué par un attaquant. Un client qui a besoin de plus de 16× la taille de batch configurée doit ouvrir une nouvelle connexion. Le plafond ne peut pas être désactivé.
...
Les deux subcommands acceptent un flag optionnel `--auth-header "Name: Value"` qui attache un header personnalisé à chaque requête backend. Utilisable pour Bearer tokens, Basic Auth ou headers de clé d'API personnalisés. La valeur parsée est marquée `sensitive`, donc hyper la masque dans les sorties de debug et l'exclut des tables HPACK HTTP/2, et le subcommand ne journalise jamais la valeur. Exemples :
...
Réserves communes aux deux flags :
...
- `co2.avoidable.methodology = "sci_v1_operational_ratio"` : `operational × (avoidable_io_ops / accounted_io_ops)`, un ratio global aveugle à la région qui exclut le carbone embarqué.
...
- **Comparer les exécutions** avant/après optimisation pour valider qu'un correctif réduit les I/O.
...
- Remplacer des données d'énergie mesurées (RAPL, Scaphandre, wattmètres in-process).
...
Quand les spans OTel portent l'attribut de ressource `cloud.region`, perf-sentinel répartit automatiquement les ops I/O par région et applique le bon coefficient d'intensité réseau. La chaîne de repli est :

1. `event.cloud_region` depuis l'attribut OTel.
2. `[green.service_regions]` mapping config par service.
3. `[green] default_region`.

Les ops I/O sans région résolvable atterrissent dans un bucket synthétique `"unknown"` et contribuent à zéro CO₂ opérationnel (un `tracing::warn!` est émis). Le carbone embarqué est tout de même émis car les émissions matérielles sont indépendantes de la région.
...
Les alias de codes pays (`fr`, `de`, `gb`, `ie`, `se`, `no`, `jp`, `br`, etc.) et synonymes fournisseurs cloud (`westeurope`, `northeurope`, `uksouth`, `francecentral`, `gra11`, `fr-par`, `waw1`, `nl-ams`, `bhs5`, `outscale-eu-west-2`, etc.) sont pris en charge et résolvent vers le même profil. Une région sans alias retombe sur la valeur annuelle plate, c'est pourquoi chaque clé ajoutée a besoin de sa propre ligne d'alias au lieu d'en hériter de son pays.

Quand `[green] use_hourly_profiles = true` (le défaut), l'étape de scoring utilise l'intensité spécifique à l'heure (et au mois quand disponible) pour chaque span basée sur son horodatage UTC. Les régions sans profil utilisent toujours la valeur annuelle plate. Les rapports sont tagués `model = "io_proxy_v3"` (mois x heure), `"io_proxy_v2"` (horaire annuel plat) ou `"io_proxy_v1"` (annuel) et chaque ligne de ventilation par région porte un champ `intensity_source` (`"annual"`, `"hourly"` ou `"monthly_hourly"`).
...
**Préférez Alumet pour les nouveaux déploiements.** [Limites de précision Alumet](#limites-de-précision-alumet) ci-dessus explique pourquoi `alumet_rapl` surclasse `scaphandre_rapl` : même source RAPL, échantillonnage moins erroné comme le caractérise le papier de Raffin et al., et attribution par cgroup. Le rythme upstream va dans le même sens : au moment de l'écriture, la dernière release Scaphandre est la v1.0.2 (février 2025) et l'issue de suivi ARM ci-dessous est ouverte depuis 2020, alors qu'Alumet est en développement actif. L'intégration Scaphandre reste prise en charge pour les déploiements existants.

**Version upstream.** Le parser ne dépend pas de la version : il consomme l'exposition Prometheus standard de `scaph_process_power_consumption_microwatts` avec les labels `exe` et `cmdline`, stable à travers les releases upstream. Le job CI wire-conformance (`.github/workflows/upstream-wire-conformance.yml`) épingle **Scaphandre v1.0.2** par SHA256 de l'artefact `.deb` upstream comme version de référence validée. Les autres releases récentes sont attendues comme fonctionnelles. Un renommage upstream de la métrique ou des labels déclencherait au runtime le filet warn-once "zero-sample" et l'assertion wire-conformance en CI.

**Exigences plateforme.** Scaphandre fonctionne sur :

- **Linux uniquement** (pas Windows, pas macOS, pas BSD).
- **CPU x86_64 Intel ou AMD avec prise en charge de RAPL** : la plupart des puces serveur et desktop récentes, mais notamment **PAS ARM64**. Apple Silicon, Ampere, Graviton et instances cloud ARM similaires ne peuvent pas utiliser cette intégration.
...
- **Linux uniquement**, toute architecture CPU prise en charge par Kepler (x86_64 et ARM64).
...
perf-sentinel embarque une intégration opt-in avec le standard BMC [Redfish](https://www.dmtf.org/standards/redfish) pour les lectures de puissance murale sur bare-metal. Quand `[green.redfish]` est configuré avec un ou plusieurs endpoints de châssis, le daemon `watch` interroge la ressource `/Power` de chaque châssis pour `PowerConsumedWatts`. Il distribue ensuite les joules au niveau du châssis sur les services mappés proportionnellement à leurs opérations, et publie les coefficients par service tagués `model = "redfish_bmc"`.

**Exigences plateforme.**

- **Nœuds bare-metal avec un BMC prenant en charge Redfish 1.0+.** Dell iDRAC, HPE iLO, Lenovo XCC, Supermicro X11+ conviennent, comme la référence OpenBMC. Les VMs cloud n'exposent pas de BMC et ne peuvent pas utiliser cette intégration.
- **HTTPS joignable depuis le daemon vers le BMC.** La plupart des BMCs présentent un certificat auto-signé par défaut. **La prise en charge d'un bundle CA fourni par l'opérateur est réservée à une version ultérieure.** Dans cette release, définir `ca_bundle_path` empêche le scraper de démarrer avec une erreur claire. Les opérateurs avec des certificats BMC auto-signés doivent placer le BMC derrière un reverse proxy qui présente un certificat signé publiquement (ou utiliser HTTP sur un segment réseau de confiance).
- **Authentification Basic.** L'authentification Session-token Redfish (POST `/SessionService/Sessions`) n'est pas encore prise en charge. Le champ `auth_header` porte une ligne Basic au format curl.
...
- **Mode batch non pris en charge.** La commande `analyze` ne lance pas le corrélateur. La corrélation cross-trace est intrinsèquement une préoccupation du streaming.
...
- **Endpoints distincts.** Chaque endpoint distinct (type de finding, service, regroupement, template) vu dans la fenêtre est conservé jusqu'à ce que la fenêtre l'oublie, sans plafond. Un service dont les templates se normalisent mal (littéraux restés dans le SQL ou les URL) garde chaque variante pendant tout `window_minutes`.
- **Portée d'appariement.** Le deque d'appariement couvre les findings analysés dans les derniers `lag_threshold_ms + 2 x trace_ttl_ms`, environ une minute avec le TTL par défaut de 30 s, et chaque finding entrant le parcourt. Sa taille et ce parcours croissent avec `trace_ttl_ms`, pas avec `window_minutes`.
- **Traces longues.** Les findings s'apparient sur leurs propres horodatages de premier span, mais un finding ne peut rencontrer que ceux analysés à environ `lag_threshold_ms + 2 x trace_ttl_ms` de lui. Une trace qui reste ouverte plus d'environ un `trace_ttl_ms` arrive tard à l'analyse et peut manquer des paires avec les findings de traces plus courtes.
- **Horodatages.** Un finding dont le `first_timestamp` est absent ou non UTC est placé à l'instant où le daemon l'a analysé. Un décalage d'horloge entre hôtes fausse le délai mesuré et peut inverser source et cible.
...
Le mode batch `analyze` renvoie toujours un tableau de corrélations vide. C'est attendu, pas un bug.
...
- **L'historique des findings dans Grafana va aussi loin que le Hub, à la granularité du jour.** La rangée `History (Hub)` du dashboard findings lit PerfSentinelHub (0.3.0 et suivants), pas le daemon : sans Hub le dashboard ne peut pas remonter dans le temps, quoi que dise le sélecteur. Le Hub enregistre les jours où il a observé un finding, à son horloge en UTC, à partir du jour de sa mise à niveau, les garde pendant `Hub:Retention` (180 jours par défaut), et n'enregistre aucun compte d'occurrences, qui restent dans `perf_sentinel_findings_total` côté Prometheus. Les colonnes d'acquittement de la rangée demandent des daemons à partir de 0.24.0, dont le Hub reflète le `GET /api/acks?include_toml=true`.
...
- Une série de compteur que l'exporteur ne publie pas laisse sa colonne à `0` plutôt qu'un chiffre inventé, et journalise un `WARN` qui la nomme. Sans le compteur d'appels en particulier, `mean_exec_time` répète `total_exec_time` et deux classements perdent leur signal.
...
- **Clé API Electricity Maps** : variable `PERF_SENTINEL_EMAPS_TOKEN`. Un `[green.electricity_maps] api_key` dans le fichier de config fonctionne mais émet un avertissement au chargement, car les fichiers de config versionnés sont une source fréquente de fuites de credentials.
- **Connection string PostgreSQL** pour `pg-stat --connection-string` : variable `PERF_SENTINEL_PG_CONNECTION`. Passer une connection string avec mot de passe en clair sur la CLI fonctionne aussi mais émet un avertissement (recommandé : `.pgpass` en production).
...
- **Fichier de clé TLS** : les permissions de `[daemon] tls_key_path` sont vérifiées au démarrage, et une clé lisible par le groupe ou les autres émet un avertissement.

Le daemon n'écrit jamais de secrets sur stdout/stderr : tous les chemins d'erreur des scrapers utilisent `redact_endpoint` pour retirer les userinfo de toute URL avant de journaliser.

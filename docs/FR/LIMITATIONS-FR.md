...
Chaque finding dérive d'un span normalisé. perf-sentinel lit une liste fermée d'attributs porteurs (le texte de la requête `db.statement` / `db.query.text`, l'URL cible `http.url` / `url.full`, plus les attributs d'enrichissement listés dans [Attributs de span requis](./INSTRUMENTATION-FR.md#attributs-de-span-requis)). Un span qui n'en porte aucun n'est pas une opération d'I/O et est ignoré. Un span SQL qui *est* une opération d'I/O mais arrive sans texte de requête, ou un span HTTP sans URL, est lui aussi ignoré : il n'y a rien à normaliser, donc aucun finding ne peut être produit. Depuis 0.11.2, le kind du span écarte un cas de plus même quand l'attribut est présent : l'URL d'un span `SERVER` décrit la requête à laquelle le service a répondu, pas un appel qu'il a émis, elle ne produit donc aucun événement HTTP sortant, tout en fournissant toujours l'endpoint entrant auquel les autres findings sont rattachés. La détection est bornée par la qualité de l'instrumentation en amont, de la même manière que tout outil purement logiciel est borné par sa source de mesure.

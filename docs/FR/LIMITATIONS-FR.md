...
**Deux millésimes, deux méthodologies.** La table embarquée mélange deux snapshots. **Entrées legacy** (Cascade Lake, Skylake, Ice Lake, EPYC 1ère-3ème Gen, ~190 instances) : Cloud Carbon Footprint coefficients 2023-05-01, AWS direct depuis `aws-instances.csv` avec overhead baseboard et mémoire inclus, GCP et Azure en `vCPU * coefficient_per_vCPU`. **Entrées modernes** (Sapphire Rapids, Emerald Rapids, Granite Rapids, Genoa, Milan, Turin, Graviton 3/4, Cobalt 100, Sierra Forest, ~130 instances) dérivées directement des résultats trimestriels `SPECpower_ssj 2008` 2024 Q1 - 2026 Q2, méthode per-vCPU pour tous les providers. **Conséquence** : un `m7i.large` moderne lit plus bas qu'un `m5.large` legacy de même nombre de vCPU, parce qu'aucun overhead AWS-specifique n'est ajouté. Les deux valeurs sont sourçables et défendables, mais les comparaisons absolues entre millésimes sont à interpréter comme "moderne est au moins aussi efficace" plutôt qu'un ratio propre.

**Graviton 3/4 et Cobalt 100 sont estimés, pas mesurés.** AWS ne soumet pas Graviton à SPECpower, Microsoft ne soumet pas Cobalt 100. Les valeurs de la table sont bornées par Ampere Altra Q80-30 (Neoverse N1, SPECpower 2024 Q1, 0.67/1.75 W/vCPU comme plancher conservateur) et Sapphire Rapids moins 25% par claim AWS public d'efficacité (0.53/2.63 W/vCPU comme plafond). Graviton 3 = 0.53/2.63, Graviton 4 = Graviton 3 moins 30% par claim AWS re:Invent 2023 = 0.37/1.84. Cobalt 100 (Neoverse N2) se positionne entre Altra (N1) et Graviton 3 (V1) puisque N2 est efficiency-focused avec un IPC plus bas que V1, donc la table utilise le midpoint 0.60/2.20 W/vCPU en attendant des données SPECpower Cobalt directes. Ces valeurs ARM portent une couche d'incertitude supplémentaire : prévoir **+/-40% plutôt que +/-30%** pour les entrées Graviton, Cobalt 100 et Ampere Altra.

**Coefficient Genoa basé sur n=1.** Le coefficient AMD Genoa (0.40/2.05 W/vCPU) pilote 26 SKUs (AWS m7a/c7a, GCP c3d/n2d, Azure Standard_Dadsv6) mais est dérivé d'un seul résultat SPECpower 2024 Q1 sur EPYC 9654 (top-bin 96 cœurs). Les binnings mid-range (9354, 9474F, 9554) tournent typiquement 5-15% moins efficacement que le 9654 à iso-charge. Prévoir **+/-40% plutôt que +/-30% pour les instances Genoa** jusqu'à ce que d'autres soumissions SPECpower élargissent l'échantillon.

**SKUs memory-optimized sans DRAM premium (gap legacy).** GCP `n2-highmem-*` et Azure `Standard_E*` réutilisent le même coefficient per-vCPU que `n2-standard-*` / `Standard_D*`, sans premium DRAM. AWS `r5` et `r6i` appliquent un facteur 1.15x sur max watts mais pas sur idle. Le DRAM réel contribue ~3-5W par DIMM 64GB en charge et ~30-40% de ça à idle. L'omission est faible en absolu mais biaise vers le bas pour les services memory-bound. Limitation legacy pré-existante, pas introduite par le refresh 2024-2026.

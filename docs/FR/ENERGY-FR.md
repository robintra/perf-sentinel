...
- **Le PUE** multiplie l'énergie pour rendre compte des surcoûts du centre de données (refroidissement, distribution électrique) : 1,09 pour GCP, 1,15 pour AWS, 1,17 pour Azure, 1,24 pour OVHcloud, 1,375 pour Scaleway, 1,5 pour OUTSCALE (qui n'en publie aucun) et pour une infrastructure inconnue.
...
Chaque ligne de ce tableau remplace ou affine la ligne au-dessus. Vous pouvez vous arrêter à n'importe quel barreau, et les rapports indiquent toujours le barreau où vous êtes.
...
Une contrainte s'applique à tous les barreaux RAPL : les compteurs matériels ne voient que le CPU et la DRAM, soit environ la moitié aux deux tiers de ce que le serveur tire à la prise. Seul Redfish voit le reste.

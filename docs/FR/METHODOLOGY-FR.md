...
- `database_waste` : le daemon calcule cette même formule directement quand `[green.alumet.database]` est déclaré, avec l'énergie de fenêtre mesurée du cgroup de base de données déclaré. La région déclarée optionnelle convertit le gaspillage en gCO2 avec l'intensité réseau de la région (temps réel Electricity Maps si disponible, annuelle embarquée sinon) et le PUE du fournisseur. Les profils horaires ne s'appliquent pas à ce chiffre, il peut donc différer du carbone par service modulé à l'heure dans le même rapport. Le chiffre est une borne basse CPU seulement avec un ratio par comptage, exclu de `energy_kwh`, de `co2` et de la publication, voir la section [Limites de précision Alumet](LIMITATIONS-FR.md#limites-de-précision-alumet) de docs/FR/LIMITATIONS-FR.md.

...
- **E (Énergie)** est l'électricité consommée par opération, en kWh. perf-sentinel résout ce terme depuis la source configurée au runtime : un proxy I/O (`io_proxy_v3`, environ `1e-7` kWh par opération d'I/O, directionnel uniquement, optionnellement recalibré par des coefficients fournis par l'opérateur via `[green] calibration_file`), ou un des cinq backends mesurés, Alumet RAPL, Scaphandre RAPL, Kepler eBPF, Redfish BMC (prise murale), ou le pourcentage CPU fourni par le cloud projeté sur des tables SPECpower. Quand plusieurs couvrent le même service, le daemon garde la lecture de plus haute fidélité, dans cet ordre, voir l'échelle de fidélité dans [docs/FR/ENERGY-FR.md](ENERGY-FR.md#léchelle-de-fidélité). La source retenue est exposée dans `methodology.calibration.energy_source_models` pour qu'un auditeur vérifie quel chemin a produit E.
- **I (Intensité du réseau)** est le carbone émis par kWh par le réseau électrique local, en gCO2eq/kWh. perf-sentinel embarque une table statique (couvrant toutes les régions cloud majeures et les principaux réseaux nationaux) et accepte un override en direct via l'API Electricity Maps quand `[green.electricity_maps]` est configuré. Les lignes sur réseau national sont régénérées deux fois par an depuis les données annuelles Ember (base génération), via une PR de revue ouverte par le workflow `refresh-datasets`. Les lignes subnationales (Amérique du Nord, Brésil BR-CS) restent maintenues à la main depuis des sources zonales en base consommation. La source est exposée dans `methodology.calibration.carbon_intensity_source` comme une des valeurs `static_tables`, `electricity_maps`, ou `mixed`.
...
**Limite connue : intervalle d'incertitude `2x`.** L'estimation carbone est livrée avec une fourchette multiplicative `2x` explicite. C'est un signal délibéré que le modèle directionnel (en particulier le proxy I/O et les tables statiques) ne convient pas au reporting d'émissions à valeur réglementaire. Resserrer la fourchette demande un terme E mesuré (n'importe quel backend de l'[échelle de fidélité d'ENERGY-FR.md](ENERGY-FR.md#léchelle-de-fidélité) : Alumet ou Scaphandre RAPL, Kepler eBPF, Redfish BMC, SPECpower cloud) et Electricity Maps en direct pour le terme I. La discussion complète sur l'incertitude vit dans [docs/FR/LIMITATIONS-FR.md](LIMITATIONS-FR.md).
...
- **Ember** est le think tank énergie qui publie des données électriques annuelles ouvertes (CC-BY-4.0), dont l'intensité CO2 de la production par pays. Les lignes sur réseau national de la table statique embarquée en sont régénérées. [Données](https://ember-energy.org/data/).
...
- `avoidable_io_ops` : nombre de spans d'I/O attribués à des anti-patterns évitables. Les patterns évitables sont N+1 SQL, N+1 HTTP, N+1 messaging, redundant SQL et redundant HTTP, tous énumérés par `FindingType::is_avoidable_io()`. Les deux premiers et les deux derniers sont aussi listés dans `core_patterns_required` de toute divulgation officielle, le N+1 messaging non, car un bus de messages n'est pas assez universel pour être exigé de toute organisation qui publie.
- Les commandes de gestion de connexion et de session (`COMMIT`, `SET`, `SELECT set_config(...)`, pings de pool) n'atteignent jamais ce compteur : les détecteurs de répétition les ignorent, aucun finding ne les attribue donc. Elles restent dans `total_io_ops`, puisqu'un driver poolé en exécute réellement une par acquisition de connexion, mais elles ne sont pas évitables et aucune remédiation par batch ne s'y applique. Voir la section Commandes de session et ratio de gaspillage de `docs/FR/LIMITATIONS-FR.md` pour la frontière de l'exclusion.
...
- `total_messaging_io_ops` et `avoidable_messaging_io_ops` : la même paire pour les publications de messages, appliquée à une lecture d'énergie de broker plutôt que de base de données : `gaspillage_broker = energie_broker * avoidable_messaging_io_ops / total_messaging_io_ops`. L'énergie du broker est mesurée (Alumet sur son cgroup), déclarée (un cluster provisionné lu dans la table SPECpower embarquée) ou estimée depuis l'énergie modélisée des publications, et l'étiquette `model` de la figure dit laquelle. Comme la figure base de données, elle est tenue hors de tous les totaux, mais contrairement à elle le sens de l'erreur dépend de la source : une mesure sous-compte (l'attribution par défaut d'Alumet ne couvre que le CPU), une déclaration borne le calcul à pleine charge sans le stockage ni le réseau. Voir `docs/FR/LIMITATIONS-FR.md`.
- `database_waste` : cette même formule calculée à chaque exécution, étiquetée par `model`. Quand `[green.alumet.database]` est déclaré et que le daemon a reçu une lecture, l'énergie est l'énergie de fenêtre mesurée du cgroup de la base (`model = "alumet_rapl"`) et la région déclarée optionnelle convertit le gaspillage en gCO2 avec l'intensité de la région (temps réel Electricity Maps si disponible, annuelle embarquée sinon) et le PUE du fournisseur, profils horaires non appliqués. Quand aucune base n'est déclarée du tout (exécutions batch, ou pas de `[green.alumet.database]`), le chiffre retombe sur une estimation : l'énergie modélisée des spans SQL de la fenêtre (multiplicateurs par verbe, calibration et intensités régionales inclus) multipliée par le ratio de gaspillage SQL, étiquetée `model = "estimated"`. Une base déclarée sans lecture reçue n'émet rien, son énergie se reporte sur la prochaine fenêtre livrée, le total de période ne double-compte donc jamais. Le mesuré est une borne basse CPU seulement, réellement additionnelle aux totaux. L'estimé réutilise l'énergie par span telle que le scoring l'a résolue (sources mesurées par service quand elles existent, proxy I/O avec son encadrement 2x sinon) et est une part re-présentée de `energy_kwh`/`co2`, jamais additionnelle. La divulgation le porte comme bloc séparé `aggregate.database_waste` (v1.4) avec les deux seuils et les modèles de provenance, toujours hors de tous les totaux. Voir la section [Limites de précision Alumet](LIMITATIONS-FR.md#limites-de-précision-alumet) de docs/FR/LIMITATIONS-FR.md.
...
Lorsque l'opérateur branche un des backends mesurés (Alumet, Scaphandre, Kepler, Redfish, ou le scraper cloud SPECpower), perf-sentinel substitue une énergie mesurée par service et bascule le tag de modèle en conséquence (`alumet_rapl`, `scaphandre_rapl`, `kepler_ebpf`, `redfish_bmc`, `cloud_specpower`). La section méthodologie d'un rapport expose `scaphandre_used` et `specpower_table_version` pour que les consommateurs sachent quel chemin a produit les chiffres.
...
perf-sentinel embarque une table d'intensité réseau statique et accepte une surcharge temps réel via l'API Electricity Maps quand `[green.electricity_maps]` est configurée. Les lignes sur réseau national sont rafraîchies deux fois par an depuis les données annuelles Ember (base génération, granularité nationale, dernière année disponible par pays), via une PR de revue. Les lignes subnationales (Amérique du Nord, Brésil BR-CS) restent maintenues à la main depuis des sources zonales en base consommation, et les profils horaires sont renormalisés dès qu'une valeur annuelle bouge de plus de 5 pour cent. Les comptabilités génération et consommation divergent surtout sur les réseaux bas carbone fortement interconnectés (les lignes nordiques ont été multipliées par plusieurs au changement de source). La fourchette d'incertitude 2x documentée reste la réserve qui prime dans tous les cas. Le champ `methodology.calibration.carbon_intensity_source` d'un rapport vaut `electricity_maps`, `static_tables` ou `mixed` pour qu'un auditeur puisse vérifier quel chemin a produit le CO2 opérationnel.
...
|---|---|---|
| `n_plus_one_sql`, `n_plus_one_http`, `n_plus_one_messaging` | 7.1, 6.1 | Cache serveur pour les données les plus utilisées, budget de requêtes par écran |
...
| `slow_sql`, `slow_http`, `slow_messaging` | (aucun) | Le RGESN n'a pas de critère sur la latence d'une opération unique. La famille 9 "Algorithmie" cible les charges de machine learning, pas la latence des requêtes. |

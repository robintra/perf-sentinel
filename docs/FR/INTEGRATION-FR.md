...
**Réserve sur N+1 contre requêtes redondantes.** dd-trace pré-obfusque le SQL (les littéraux sont déjà `?`), donc les paramètres par requête qui distinguent un N+1 d'une requête légitimement répétée ont disparu avant que perf-sentinel ne les voie. En mode de détection `auto` par défaut, un vrai N+1 à timing uniforme peut apparaître comme `redundant_sql` plutôt que `n_plus_one_sql`. Réglez `[detection] sanitizer_aware_classification = "strict"` pour récupérer les cas à forte occurrence (15 requêtes identiques ou plus dans une trace). Le scoring de gaspillage et de carbone est identique pour les deux types de finding.

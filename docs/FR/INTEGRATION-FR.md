...
- `--max-traces-embedded <N>` : cap sur les traces embarquées pour l'onglet Explain. Sans valeur, la sortie est ajustée automatiquement pour viser une taille HTML d'environ 5 Mo en coupant les traces à plus faible IIS. Un bandeau dans l'onglet Findings remonte le ratio tronqué quand la coupe s'applique, et la CLI loggue une ligne `info!` sur stderr (`Embedded N of M traces ... trimmed for file size`) pour que les logs de build CI portent le même signal. Sur une entrée qui est déjà un Report JSON, les arbres gardés sont ceux des findings de tête, donc `--sort` ci-dessous choisit lesquels.
- `--sort <CLE>` : `severity` ou `impact`, mêmes clés que `analyze --sort`. Réordonne les findings et, avec `--max-traces-embedded`, décide quels arbres de spans survivent au cap.

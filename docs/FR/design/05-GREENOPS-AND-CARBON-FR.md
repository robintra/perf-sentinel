...
**Config.** `[green] include_network_transport = false` (défaut) n'agit plus que sur l'affichage depuis la 0.9.25 : il masque le terme dans le rapport CLI et le TUI, et ne change rien à ce qui est calculé, archivé ou publié. Il atteint ces sorties via `scoring_config.show_network_transport`. Conditionner le calcul rendait deux rapports non comparables sur un réglage qui n'y laissait aucune trace. Le coefficient est configurable via `[green] network_energy_per_byte_kwh`.

...
**Chemin snapshot du daemon.** L'endpoint `/api/export/report` du daemon retourne `GreenSummary::disabled(0)` (pas de scoring live sur ce chemin), mais le handler patche `scoring_config` à partir du `Config` du daemon pour que la chip d'audit apparaisse sur le snapshot quand même. Un opérateur qui pull le snapshot pendant qu'Electricity Maps est totalement configuré voit la chip, même si cet endpoint ne recalcule pas les chiffres carbone.

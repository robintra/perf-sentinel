...
4. `sequential_siblings_indexed` (mode Strict uniquement) : tous les spans partagent un même `parent_span_id` non vide et le groupe chaîne `prev.end_us <= next.start_us` après tri par timestamp de début. Les bornes sont calculées en microsecondes pour éviter la troncation silencieuse des durées sous-milliseconde. Substitue `has_orm_scope` sur les piles bare-driver (Vert.x reactive PG, pgx, asyncpg, sqlx, Prisma `queryRaw`) qui n'émettent jamais de scope ORM. La branche bare-driver exige toujours la variance temporelle élevée pour se déclencher.
5. `high_occurrence` (mode Strict, branche ORM uniquement) : quand `has_orm_scope` est vrai mais la variance temporelle est basse (le piège cache-warm observé sur EF Core + Npgsql et Hibernate L2 cache), un nombre d'occurrences élevé (>= 3 x `n_plus_one_threshold`, par défaut 15) est accepté comme signal corroboratif en remplacement de la variance. Les boucles de polling legacy sous le seuil (typiquement 5-10 appels par requête) restent classées en `redundant_sql`. La branche bare-driver (séquentiel sans ORM) conserve l'exigence stricte variance-seule.
...
- `strict` privilégie la précision : préserve les findings `redundant_sql` sur les requêtes identiques en cache de compte modéré (sous la barre `3 x threshold` cache-warm) parce que le signal de variance temporelle reste bas. Au-dessus de la barre (par défaut 15 occurrences), les groupes avec scope ORM se déclenchent même avec une variance basse, fermant le piège cache-warm observé sur EF Core + Npgsql et Hibernate L2 cache. Les piles bare-driver exigent la combinaison siblings séquentiels + variance quel que soit le nombre d'occurrences. Recommandé quand des findings `redundant_sql` exploitables ont de la valeur dans votre environnement.

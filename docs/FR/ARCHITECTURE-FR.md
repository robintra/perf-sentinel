...
| **score**        | `score/`          | Scoring GreenOps (`mod.rs`) : IIS par endpoint, ratio de gaspillage, top offenders, green_impact par finding. Carbone multi-région SCI v1.0 (`carbon.rs`) : CO₂ opérationnel par région via attribut OTel `cloud.region`, terme carbone embodié, rapport `co2` structuré avec intervalles de confiance (Phase 5a)            |

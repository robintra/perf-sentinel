...
| **score**        | `score/`          | Scoring GreenOps (`mod.rs`) : IIS par endpoint, ratio de gaspillage, top offenders, green_impact par finding. Carbone multi-région SCI v1.0 (`carbon.rs`) : CO₂ opérationnel par région via attribut OTel `cloud.region`, terme carbone embodié, rapport `co2` structuré avec intervalles de confiance, profils horaires pour FR/DE/GB/US-East, intégration optionnelle Scaphandre par-processus (`scaphandre.rs`), estimation d'énergie cloud via CPU% + interpolation SPECpower (`cloud_energy/`) |

...
- **Pattern** : le template SQL, HTTP ou messaging normalisé qui regroupe les events d'un finding (ex. `SELECT * FROM users WHERE id = ?`).
...
- **Mode CI** (`perf-sentinel analyze --ci`) : identique au batch, mais une violation du quality gate fait sortir le processus avec le code 1. Le champ `Confidence` de chaque finding est étiqueté `ci_batch` (confiance la plus faible : variété de trafic limitée).
- **Mode daemon** / **streaming** / **watch** (`perf-sentinel watch`) : processus de longue durée qui ingère les events OTLP/JSON en temps réel, avec éviction TTL et détection par trace. Les findings sont étiquetés `daemon_staging` ou `daemon_production` selon `[daemon] daemon_environment` (augmente la sévérité chez les consommateurs en aval comme perf-lint).

L'axe `Confidence` (`ci_batch` < `daemon_staging` < `daemon_production`) est étiqueté automatiquement par le runtime et exposé dans JSON, SARIF, HTML et l'API de requête du daemon.
...
- `GET /api/correlations` (corrélations inter-traces, plafonné à 1000)
- `GET /api/status` (uptime, traces actives, nombre de findings stockés)

Un `FindingsStore` (buffer circulaire) retient les findings récents pour l'API (dimensionné par `[daemon] max_retained_findings`, défaut 10k). La sous-commande compagnon `perf-sentinel query` restitue ces endpoints en sortie colorée dans le terminal. Conditionné par `[daemon] api_enabled` (défaut true). Voir [`docs/FR/LIMITATIONS-FR.md`](LIMITATIONS-FR.md#api-de-requêtage-du-daemon) pour le modèle de menace sans authentification.

## Responsabilités des modules

| Module              | Chemin               | Responsabilité                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|---------------------|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **event**           | `event.rs`           | Type central `SpanEvent` (variantes SQL, HTTP et messaging) avec horodatage, IDs trace/span, service, opération, cible, durée                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **ingest**          | `ingest/`            | Sources d'entrée : parseur JSON avec auto-détection du format (`json.rs`), import Jaeger JSON (`jaeger.rs`), import Zipkin JSON v2 (`zipkin.rs`), récepteur OTLP gRPC+HTTP (`otlp.rs`), dont la conversion recoud les requêtes SQL qu'une instrumentation en couches a scindées entre les spans qui portent le statement et ceux qui portent la durée (PHP Doctrine + PDO). Implémente le trait `IngestSource`. Parseur PostgreSQL `pg_stat_statements` (`pg_stat.rs`) et parseur des digests MySQL Performance Schema (`mysql_stat.rs`) pour l'analyse de hotspots. Récupérateurs de traces distants pour Grafana Tempo (`tempo.rs`) et l'API de requête Jaeger (`jaeger_query.rs`), avec helpers partagés pour l'en-tête d'authentification (`auth_header.rs`), les fenêtres de recherche en arrière (`lookback.rs`) et l'encodage d'URL (`url_enc.rs`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **detect**          | `detect/`            | Détection de patterns sur les traces corrélées. N+1 (`n_plus_one.rs`) : même template, paramètres différents, dans une fenêtre. Redondant (`redundant.rs`) : même template et paramètres. Lent (`slow.rs`) : durée au-dessus du seuil avec template récurrent. Fanout (`fanout.rs`) : span parent avec trop de spans enfants. Service bavard (`chatty.rs`) : trop d'appels HTTP sortants par trace. Saturation du pool (`pool_saturation.rs`) : spans SQL concurrents dépassant le seuil via algorithme de balayage. Appels sérialisés (`serialized.rs`) : appels frères séquentiels indépendants potentiellement parallélisables. Corrélateur inter-traces pour le mode daemon (`correlate_cross.rs`). Pistes de remédiation attachées aux findings (`suggestions.rs`). Classification sensible aux sanitizers (`sanitizer_aware.rs`) : reconnaît le SQL déjà paramétré pour que la détection N+1 écarte les cas sûrs (SQL uniquement aujourd'hui)                                                                                                                                                                                                                                                                                                                                                                                                    |
| **score**           | `score/`             | Scoring GreenOps (`mod.rs`) : IIS par endpoint, ratio de gaspillage, top offenders, green_impact par finding. Pipeline carbone réparti entre `carbon.rs` (table d'intensité du réseau embarquée, constantes SCI), `carbon_compute.rs` (boucle d'accumulation par span), `carbon_profiles.rs` (profils horaires du réseau) et `region_breakdown.rs` (regroupement par région, sélection du model-tag, finalisation du `CarbonReport`). Six backends de puissance et d'énergie opt-in, chacun un sous-module structuré : `alumet/`, `scaphandre/` et `kepler/` (RAPL), `redfish/` (télémétrie BMC), `cloud_energy/` (CPU% + interpolation SPECpower embarquée), `electricity_maps/` (intensité du réseau en direct, uniquement si configuré). Plomberie partagée des scrapers : `energy_state.rs` (cache de coefficients par service), `ops_snapshot_diff.rs` (deltas d'opérations par service et par fenêtre de scrape) et `prom_parser.rs` (parseur d'exposition Prometheus partagé par les scrapers Kepler et Alumet). Calcul de l'évitable à seuil canonique fixe contre la manipulation des divulgations (`canonical.rs`). Multi-région SCI v1.0 via attribut OTel `cloud.region`, intervalles de confiance, profils horaires du réseau pour 22 régions (mensuel x horaire complet pour FR/DE/GB/US-East, journalier représentatif pour 18 de plus) |
| **report**          | `report/`            | Formatage de sortie. Rapport JSON (`json.rs`), export SARIF v2.1.0 (`sarif.rs`), sortie CLI colorée (`mod.rs`), métriques Prometheus avec exemplars OpenMetrics (`metrics.rs`), dashboard HTML autonome en un fichier (`html/` + `html_template.html` embarqué) avec onglets Findings, Explain, pg_stat, mysql_stat, Diff, Correlations et Green. Interprétations en langage clair des findings (`interpret.rs`), avertissements de rapport (`warnings.rs`), et la pile de divulgation publique périodique (`periodic/` : schéma, agrégateur, validateur, configuration d'organisation, hacheur de contenu, attestation)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **daemon**          | `daemon/`            | Mode streaming : `mod.rs` héberge `DaemonError` et l'orchestrateur `run()`. Responsabilités réparties entre `event_loop.rs` (boucle tokio::select, éviction TraceWindow, worker d'analyse unique exécutant detect/score hors de la boucle), `listeners.rs` (spawn OTLP gRPC/HTTP, scrapers énergie optionnels), `tls.rs` (chargement certs, `MaybeTlsStream`, boucle HTTPS avec plafond de concurrence sur les handshakes), `json_socket.rs` (ingestion NDJSON Unix, cfg(unix)), `sampling.rs` (échantillonnage par hash de trace-id), `findings_store.rs` + `query_api.rs` (findings retenus et endpoints HTTP de requête), `ack.rs` (store d'acquittements et endpoints HTTP d'ack), `health.rs` (liveness et readiness), `archive.rs` (archive NDJSON `Report` par fenêtre consommée par `disclose`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **config**          | `config/`            | Parse `.perf-sentinel.toml` avec le format sectionné ([thresholds], [detection], [green], [daemon]). Les 8 clés top-level legacy acceptées en 0.5.x ont été retirées en 0.6.0 (voir `CONFIGURATION-FR.md` pour la table de migration)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **time**            | `time.rs`            | Helpers partagés de conversion d'horodatage (`nanos_to_iso8601`, `micros_to_iso8601`). Utilisé par l'ingestion OTLP, Jaeger et Zipkin                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **explain**         | `explain.rs`         | Visualiseur d'arbre de trace : construit l'arbre des spans à partir de `parent_span_id`, annote les findings en ligne (suggestion, correctif par framework, emplacement dans le code). Sortie texte et JSON                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `Finding`         | detect           | Anti-pattern détecté avec type, sévérité, détails du pattern, horodatages, green_impact et `confidence` (CI batch / daemon staging / daemon production) |
...
sentinel-cli (binaire)
  |
  +-- CLI clap : sous-commandes analyze / explain / diff / report / watch / query / ack / inspect / tempo / jaeger-query / pg-stat / mysql-stat / calibrate / disclose / verify-hash / hash-bake / demo / bench / man / completions
  |
...
Le crate CLI reste léger : il parse les arguments, charge la configuration et délègue aux fonctions de sentinel-core. Toute la logique métier réside dans sentinel-core.
...
Le binaire publié active les quatre. Les consommateurs de la bibliothèque sentinel-core partent de tout désactivé et activent au besoin. `tonic`, `axum`, `tower`, `prometheus` et `opentelemetry-proto` restent des dépendances inconditionnelles de sentinel-core car `report/metrics.rs` et `ingest/otlp/` utilisent leurs types même quand aucun écouteur ne tourne.

...
| **ingest**          | `ingest/`            | Sources d'entrée : parseur JSON avec auto-détection du format (`json.rs`), import Jaeger JSON (`jaeger.rs`), import Zipkin JSON v2 (`zipkin.rs`), récepteur OTLP gRPC+HTTP (`otlp.rs`). Implémente le trait `IngestSource`. Parseur PostgreSQL `pg_stat_statements` (`pg_stat.rs`) et parseur des digests MySQL Performance Schema (`mysql_stat.rs`) pour l'analyse de hotspots. Récupérateurs de traces distants pour Grafana Tempo (`tempo.rs`) et l'API de requête Jaeger (`jaeger_query.rs`), avec helpers partagés pour l'en-tête d'authentification (`auth_header.rs`), les fenêtres de recherche en arrière (`lookback.rs`) et l'encodage d'URL (`url_enc.rs`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

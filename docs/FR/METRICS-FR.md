...
| `perf_sentinel_slow_duration_grouping_overflow_total` | counter   | (aucun)                       | Spans lents repliés dans une série d'histogramme `grouping="_other"` parce que le plafond de 256 paires de l'histogramme était atteint (depuis 0.19.0). Même contrat que le counter service ci-dessus, avec le plafond le plus bas des trois car l'histogramme coûte désormais 14 séries par triplet (type, service, grouping). |
...
| `perf_sentinel_incidents_total`                | counter | `kind`             | Incidents enregistrés par `POST /api/incidents` (depuis 0.20.0), par genre déclaré : `oom_kill`, `memory_saturation`, `restart`, `deploy`, `other`. Compte des incidents, pas des livraisons de webhook : Alertmanager répète une alerte active à chaque `repeat_interval` et un repostage est le même incident, donc ce compteur ne peut pas monter tout seul. Pas de libellé `service`, un corps POST étant contrôlé par l'appelant là où l'exception sanctionnée couvre les services passés par les meters d'ingestion cappés. Les cinq genres sont pré-chauffés à zéro. Le tableau de bord Grafana d'aperçu lit la présence de cette métrique, à côté de `perf_sentinel_analysis_service_overflow_total`, pour le même bandeau de compatibilité. |
...
| `perf_sentinel_service_last_span_timestamp_seconds`    | gauge   | `service`             | Timestamp Unix du dernier span reçu pour ce service (depuis 0.20.0). Un timestamp plutôt qu'un âge, sur la convention de `process_start_time_seconds` : `time() - gauge` donne l'âge, et un trou de collecte ne peut pas le faire mentir. Il place un incident en face des findings de sa fenêtre, ce à quoi répond `until_ms` sur `/api/findings`, voir [QUERY-API-FR.md](QUERY-API-FR.md). À lire comme un signal de trafic, pas de vie : un crash, une mise à l'échelle à zéro, un déploiement progressif et un cron silencieux se lisent pareil, donc une alerte dessus a besoin d'un `for:` plus long que l'inactivité normale du service. Il est post-échantillonnage, donc un service à faible débit sous `sampling_rate < 1` peut sembler vieux tout en étant sain. La cardinalité suit le cap d'ingestion de 1024 services, une série n'existe que pour un service admis, et aucune série n'est jamais retirée : un service définitivement mort garde un timestamp figé, ce qui se lit comme un âge qui grandit.                                                                                                                                                                                            |

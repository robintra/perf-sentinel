...
| `perf_sentinel_stored_findings`                | gauge     | (aucun)            | Signatures de findings distinctes actuellement retenues dans le store de la query API, entrées coalescées depuis 0.9.29 (gauge depuis 0.8.8). À apparier avec `perf_sentinel_max_retained_findings` pour un ratio de headroom.                                                                                                                                    |
...
| `perf_sentinel_max_retained_findings`          | gauge     | (aucun)            | Plafond configuré de signatures de findings distinctes dans le store (`[daemon] max_retained_findings`), positionné une fois au démarrage (depuis 0.8.8). À apparier avec `perf_sentinel_stored_findings`.                                                                                                                    |
...
| `perf_sentinel_slow_duration_seconds`          | histogram | `type`             | Histogramme de durée pour les spans dépassant le seuil slow, par `type` (`sql`, `http_out` ou `messaging`). Buckets : 0.1, 0.25, 0.5, 0.75, 1, 1.5, 2, 3, 5, 10, 30 secondes. Utilisé par `histogram_quantile()` Grafana pour des percentiles précis sur des déploiements daemon shardés.                          |

...
négociation de contenu, et émet des exemplars quand des `trace_id` sont
disponibles côté findings.
...
Enregistrées uniquement sur Linux (les lectures `procfs` sous-jacentes
...
| `process_start_time_seconds`    | gauge   | Horodatage Unix du démarrage du processus. |
...
le parcours FD peut dominer entre 1 et 5 ms par scrape. Le verrou
...
à l'intervalle typique de 15 à 60 secondes, à surveiller pour des
intervalles plus serrés.

**Périmètre d'exposition.** Quand l'opérateur lie l'endpoint metrics
à `0.0.0.0` (défaut des Pods Kubernetes pour le scraping intra-cluster),
les metrics process exposent des signaux opérationnellement sensibles :
uptime via `process_start_time_seconds` (inférence de patch / redémarrage),
...
Pour les topologies de scraping à l'échelle du cluster, mettre `/metrics`
...
- `channel_full` (HTTP et gRPC) : le canal d'événements est saturé ou
  fermé et le daemon n'a pas pu mettre le batch en file. La mise en file
  attend jusqu'à 2 secondes avant de rejeter, les rafales courtes sont
  donc absorbées sans rejet tandis qu'une saturation soutenue ressort
  vite. La voie HTTP renvoie 503, la voie gRPC renvoie `UNAVAILABLE` en
  saturation (les deux sont rejouables selon la spec OTLP) et
...
- `memory_pressure` (HTTP, gRPC et la socket JSON Unix) : le working
  ...
  L'ingest est alors rejeté (HTTP 503, gRPC `UNAVAILABLE`, les deux
  rejouables) pour borner la RSS indépendamment de la profondeur de
  file, jusqu'à ce que l'usage retombe 5 points de pourcentage sous le
  ...
  compte des requêtes, pas des spans. Ne se déclenche jamais quand le
  garde-fou est désactivé (`memory_high_water_pct = 0`, défaut) ni sur
  un hôte sans limite mémoire cgroup v2.

Les 4 raisons sont pré-chauffées à 0 au démarrage pour que les
dashboards puissent tracer la ligne zéro avant le premier rejet.
...
surveiller les logs du proxy ou de la gateway en amont, ou câbler un
counter de rejet tower-http dans leur stack.

Les deux counters au niveau span exposent le taux de rétention du
filtre I/O (seuls les spans SQL, HTTP sortants et de publication vers
un broker sont analysables, voir
...
continuent de répondre en succès. Seule cette paire de counters
...
`perf_sentinel_otlp_spans_filtered_total`, pré-chauffées à 0 :
...
- `non_sql_datastore` : le span nomme un store non-SQL (Redis,
  MongoDB, ...) dans `db.system`. Hors du périmètre d'analyse, pas un
  manque d'instrumentation (voir [`LIMITATIONS-FR.md`](./LIMITATIONS-FR.md)).
...
| `perf_sentinel_stored_findings`                       | gauge     | (aucun)                                   | Findings actuellement retenus dans le ring buffer de la query API (depuis 0.8.8). À apparier avec `perf_sentinel_max_retained_findings` pour un ratio de marge.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `perf_sentinel_analysis_shed_batches_total`           | counter   | (aucun)                                   | Lots d'analyse délestés parce que la file du worker était pleine ou que le worker s'est arrêté. Chaque délestage est compté ici. Alertez plutôt sur `perf_sentinel_analysis_shed_traces_total`, un lot délesté pouvant porter une trace comme un millier.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `perf_sentinel_slow_window_keys_refused_total`        | counter   | (aucun)                                   | Spans lents non comptés par la fenêtre lente inter-batchs parce que leur clé (type, service, grouping, template) est nouvelle et que 1024 clés sont déjà suivies. Le premier refus journalise un avertissement une seule fois par processus. Voir `[detection] slow_query_window_minutes`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `perf_sentinel_analysis_grouping_overflow_total`      | counter   | (aucun)                                   | Attributions de séries côté analyse (findings, I/O évitables et analysées) repliées dans `grouping="_other"` parce que le plafond de 512 paires (service, grouping) admises était atteint (depuis 0.19.0). Une paire au-delà garde son service et ne replie que son regroupement. Une hausse continue signifie que l'attribution par regroupement se dégrade pour les regroupements nouvellement vus, les totaux et les sommes par service restent exacts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `perf_sentinel_slow_duration_grouping_overflow_total` | counter   | (aucun)                                   | Spans lents repliés dans une série d'histogramme `grouping="_other"` parce que le plafond de 256 paires de l'histogramme était atteint (depuis 0.19.0). Même contrat que le counter service ci-dessus, avec le plafond le plus bas des trois car l'histogramme coûte 14 séries par triplet (type, service, grouping).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
sont en lecture seule et ne sont pas comptés ici, car aucune
opération n'a lieu après le chargement initial.

| Metric                                         | Type    | Labels             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
|------------------------------------------------|---------|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `perf_sentinel_ack_operations_total`           | counter | `action`           | Opérations ack et unack réussies.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `perf_sentinel_incidents_total`                | counter | `kind`             | Incidents enregistrés par `POST /api/incidents` (depuis 0.20.0), par genre déclaré : `oom_kill`, `memory_saturation`, `restart`, `deploy`, `other`. Compte des incidents, pas des livraisons de webhook : Alertmanager répète une alerte active à chaque `repeat_interval` et un repostage est le même incident, donc ce compteur ne peut pas monter tout seul. Pas de libellé `service`, car un corps POST est contrôlé par l'appelant alors que l'exception admise couvre les services passés par les compteurs d'ingestion plafonnés. Les cinq genres sont pré-chauffés à zéro. Le tableau de bord Grafana d'aperçu lit la présence de cette métrique, à côté de `perf_sentinel_analysis_service_overflow_total`, pour le même bandeau de compatibilité.                                                                                                                                                               |
| `perf_sentinel_incidents_archive_failed_total` | counter | (aucun)            | Enregistrements d'incident que le writer de `[daemon.incidents] archive_path` a laissé tomber sur un canal plein ou n'a pas pu écrire, qu'ils viennent d'une livraison, d'une fermeture ou d'une passe de consolidation (depuis 0.20.0). L'incident est toujours dans le ring en mémoire, donc ceci compte de la durabilité perdue, pas de la donnée perdue. Une hausse continue signifie qu'un post-mortem après un redémarrage du daemon ne trouvera rien.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `perf_sentinel_incidents_rejected_total`       | counter | `reason`           | Requêtes et alertes que les routes d'incident ont refusées (depuis 0.20.0), par raison : `unauthorized` (un `POST` ou un `GET` sur `/api/incidents` qui ne présente ni la clé d'écriture ni, sur `GET`, `[daemon] read_api_key`), `no_service` (une alerte sans le libellé de service configuré), `unparsable_time` (une alerte dont le `startsAt` n'est pas du RFC 3339), `overflow` (les alertes au-delà des 1000 qu'une livraison peut porter). Le corps de réponse porte les mêmes comptes, mais Alertmanager le jette et ne réessaie jamais un 4xx (un groupe encore actif est renvoyé au `group_interval` suivant, donc le 401 se répète jusqu'à la correction de l'en-tête et la capture est alors ce que le ring détient encore). Un receiver avec le mauvais en-tête ou une règle avec le mauvais libellé perd chaque capture sans que rien d'autre ne bouge. Les quatre raisons sont pré-chauffées à zéro. |
...
- `file_too_large` (HTTP 507, `action=ack` uniquement) : l'ajout
  ferait passer le JSONL au-dessus de 64 Mio. Saturation par
  daemon, indique qu'une compaction est nécessaire au prochain
  redémarrage ou que la limite doit être relevée. Côté `unack` ce
  cas remonte sous `internal_error` (HTTP 500), car les endpoints
  ack ne différencient pas aujourd'hui la limite sur l'écriture unack.
- `entry_too_large` (HTTP 507, `action=ack` uniquement) : un seul
  enregistrement dépasse 4 Kio après sérialisation, typiquement parce
  que le champ `by` ou `reason` fourni par l'appelant est trop gros.
...
`absent()`. L'ensemble pré-chauffé compte 2 séries succès
...
`action=unack,reason=already_acked`) ne sont pas pré-chauffées, pour
éviter de fausses séries.
...
ou dans une alerte qui ne doit se déclencher que pour un backend en
service.
...
- `unreachable` : échec transport bas niveau (connexion refusée,
  échec DNS, erreur TLS handshake, hôte hors ligne). L'endpoint n'est
  pas joignable depuis le pod du daemon.
- `timeout` : le délai de 3 secondes sur l'appel HTTP par scrape a
...
- `invalid_utf8` : le corps de réponse n'est pas de l'UTF-8 valide.
  Scaphandre émet toujours du texte Prometheus ASCII-safe, donc ce cas
  signale presque toujours que l'endpoint n'est pas Scaphandre.
...
interrompent la tâche scraper au démarrage avant que le compteur soit
...
- `topk(1, increase(perf_sentinel_scaphandre_scrape_failed_total[1h]))` :
  ...
  de l'hôte, un `http_error` persistant vers un exporteur derrière un
...
**Piège de l'obsolescence sans échantillon**.
...
contient aucune série Kepler v2 correspondante. Le cas classique est une
...
Le même piège de l'obsolescence sans échantillon s'applique, et il est
plus probable ici que pour Kepler, car `metric_name` et `label_key` sont
fournis par l'opérateur (l'exporteur d'Alumet façonne les noms avec un
`prefix`/`suffix` configurable). Une coquille ou un renommage amont
produit des HTTP 200 sans échantillon correspondant et une jauge qui se
remet sans cesse à 0. Le daemon avertit après trois ticks
...
Deux messages de warn distincts existent, un par cause, chacun avec sa
...
| Metric                                                 | Type    | Labels                | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|--------------------------------------------------------|---------|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `perf_sentinel_io_waste_ratio`                         | gauge   | (aucun)               | Ratio de gaspillage I/O cumulatif (avoidable / total) depuis le démarrage. Utiliser `rate()` sur les counters sous-jacents pour des valeurs sur fenêtre.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `perf_sentinel_service_io_ops_total`                   | counter | `service`, `grouping` | Ops I/O cumulatives par service (lu par chaque scraper d'énergie mesurée pour l'attribution énergie par service). La cardinalité du label est plafonnée à 1024 services distincts par exécution du daemon, les nouveaux services au-delà du plafond ne sont pas attribués. Un span sans nom de service est résolu en `service="unknown"` à l'ingestion (sur les quatre chemins), plutôt que de créer un label vide, et `service="_other"` est réservé comme bac de repli côté analyse : un vrai service portant ce nom y est fusionné. Depuis 0.19.0 il porte aussi `grouping`, la première valeur de `[detection] grouping_attributes` du span (vide s'il n'en a aucune), plafonné à 4096 paires (service, grouping) admises par run. Une paire au-delà replie son regroupement dans `grouping="_other"`, contrairement à l'axe service du même counter, qui tronque. `grouping="_other"` est réservé de la même façon (un vrai regroupement portant ce nom fusionne dans le bac de repli sans faire bouger le counter de débordement). Les scrapers d'énergie le lisent replié en un total par service.                                                                                                   |
...
| `perf_sentinel_service_io_ops_grouping_overflow_total` | counter | (aucun)               | Ops I/O dont le `grouping` s'est replié dans `_other` sur `perf_sentinel_service_io_ops_total` parce que le plafond d'ingestion de 4096 paires était atteint (depuis 0.19.0). Rien n'est tronqué : les totaux par service restent exacts, seule la répartition par regroupement se dégrade.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `perf_sentinel_service_last_span_timestamp_seconds`    | gauge   | `service`             | Horodatage Unix du dernier span reçu pour ce service (depuis 0.20.0). Un horodatage plutôt qu'un âge, sur la convention de `process_start_time_seconds` : `time() - gauge` donne l'âge, et un trou de collecte ne peut pas le fausser. Il place un incident en face des findings de sa fenêtre, ce à quoi répond `until_ms` sur `/api/findings`, voir [QUERY-API-FR.md](QUERY-API-FR.md). À lire comme un signal de trafic, pas de vie : un crash, une mise à l'échelle à zéro, un déploiement progressif et un cron silencieux se lisent pareil, donc une alerte dessus a besoin d'un `for:` plus long que l'inactivité normale du service. Il est post-échantillonnage, donc un service à faible débit sous `sampling_rate < 1` peut sembler vieux tout en étant sain. La cardinalité suit le plafond d'ingestion de 1024 services, une série n'existe que pour un service admis, et aucune série n'est jamais retirée : un service définitivement mort garde un horodatage figé, ce qui se lit comme un âge qui grandit.                                                                                                                                                                                            |
| `perf_sentinel_service_avoidable_io_ops_total`         | counter | `service`, `grouping` | Part par service de `perf_sentinel_avoidable_io_ops`, dérivée des findings avec la même règle de dédoublonnage (depuis 0.18.0). Les services au-delà du plafond d'analyse de 128 se replient dans `service="_other"`, donc la somme sur les services égale toujours le counter global. Divisez par `perf_sentinel_service_analyzed_io_ops_total` pour un ratio de gaspillage par service. Un finding dont les spans viennent de plusieurs services est réparti entre eux, chacun portant ses propres répétitions et le service du finding (le premier span dans l'ordre d'ingestion) une de moins pour l'appel nécessaire, au lieu d'être crédité en entier à un seul service. Le label `grouping` (depuis 0.19.0) est celui du finding : un finding dont les spans viennent de plusieurs services impute la part de chacun sous ce seul regroupement. Cette imputation est exacte, car les détecteurs N+1 et redondance partitionnent leurs groupes par l'identité du regroupement, donc chaque span imputé par un finding le porte déjà et le ratio avec `perf_sentinel_service_analyzed_io_ops_total` ne peut pas dépasser 1. Ce que la répartition approxime, c'est quel service a fait l'unique appel nécessaire. |
| `perf_sentinel_service_analyzed_io_ops_total`          | counter | `service`, `grouping` | Ops I/O par service des traces analysées (depuis 0.18.0) : le dénominateur du ratio de gaspillage, issu de la même passe de scoring, du même plafond et du même gating green que le counter avoidable, contrairement à `perf_sentinel_service_io_ops_total` côté ingest (compté avant analyse, tronqué au-delà de 1024 services, incluant les lots délestés ensuite). Porte `grouping` depuis 0.19.0 sous le même plafond que le counter avoidable, donc le ratio par (service, grouping) ne mélange pas non plus deux populations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
de démarrage à froid. Lire `warnings` sur le `/api/export/report` d'un
daemon en marche renvoie une liste vide et se lit comme "aucun
...
"à un moment depuis le démarrage le canal a saturé, voici le nombre
pour le post-mortem". Acquitter des findings via l'API ack du daemon
n'efface aucun kind, car ils reflètent l'état du daemon, pas la sortie
de détection.
...
| `perf_sentinel_analysis_grouping_overflow_total > 0`, `perf_sentinel_slow_duration_grouping_overflow_total > 0` ou `perf_sentinel_service_io_ops_grouping_overflow_total > 0` | Agréger ou réduire les valeurs de regroupement, ou raccourcir `[detection] grouping_attributes` (les plafonds de 512, 256 et 4096 paires sont fixes) |
...
Les règles à compteurs sont collantes (les compteurs cumulatifs ne se
...
qu'un rapport échantillonné sous-estime ses comptes quelle que soit la
charge. Le message nomme les comptes et laisse les ratios de côté, car
un échantillonnage uniforme par trace touche numérateur et dénominateur
de la même façon. Le ratio de gaspillage I/O reste lisible, si bien que
le remettre à l'échelle produirait un nombre faux et non un nombre
...
dépassé. Le conseiller lit le snapshot de config pris au démarrage du
daemon, une indication reflète donc toujours les valeurs utilisées par
le processus en cours.

Un échantillonnage appliqué **avant** le daemon (un collector qui fait
tourner `tail_sampling` en amont) rétrécit les mêmes comptes et ne lève
aucun avertissement, parce qu'une trace conservée est indiscernable d'une
...
de regroupements, ou l'éviction du corrélateur : ce sont des états que le
daemon atteint en fonctionnant normalement, et chacun est un panneau du tableau
de bord. Les seuils portent un plancher pour la même raison : un `rate(...) > 0`
qui se déclenche sur un taux de perte d'une fraction de pourcent apprend au
lecteur à couper la règle.
...
correcte mais inutile sur une installation dont la flotte n'émet
légitimement aucun span d'I/O, et qu'une règle fausse dès le premier jour
finit coupée pour toujours. Activez-la une fois votre premier finding vu,
avec une fenêtre supérieure à votre plus longue période d'inactivité
légitime :
...
- Émetteur SARIF pour intégration CI : voir [SARIF-FR.md](SARIF-FR.md).

...
| `perf_sentinel_slow_duration_seconds`                | histogram | `type`, `service`             | Histogramme de durée pour les spans dépassant le seuil slow, par `type` (`sql`, `http_out` ou `messaging`) et, depuis 0.18.0, `service` (plafonné à 64 services distincts, débordement replié dans `service="_other"`, vide avec `[daemon] per_service_labels = false`). Rien n'est pré-chauffé avec `per_service_labels = true` (défaut) : les séries d'un service apparaissent à son premier span lent (pré-créer le seul label connu d'avance, `service="_other"`, afficherait un débordement qui n'a jamais eu lieu), donc une série absente signifie aucun span lent pour l'instant, pas un worker arrêté. Seule la série sans label du réglage désactivé est pré-chauffée à zéro comme en 0.17. Buckets : 0.1, 0.25, 0.5, 0.75, 1, 1.5, 2, 3, 5, 10, 30 secondes. Utilisé par `histogram_quantile()` Grafana pour des percentiles précis sur des déploiements daemon shardés.                                                                                     |
...
| `perf_sentinel_service_avoidable_io_ops_total`       | counter | `service` | Part par service de `perf_sentinel_avoidable_io_ops`, dérivée des findings avec la même règle de dédoublonnage (depuis 0.18.0). Les services au-delà du plafond d'analyse de 128 se replient dans `service="_other"`, donc la somme sur les services égale toujours le counter global. Divisez par `perf_sentinel_service_analyzed_io_ops_total` pour un waste ratio par service.                                                                                                                                                         |
| `perf_sentinel_service_analyzed_io_ops_total`        | counter | `service` | Ops I/O par service des traces analysées (depuis 0.18.0) : le dénominateur du waste ratio, issu de la même passe de scoring, du même plafond et du même gating green que le counter avoidable, contrairement à `perf_sentinel_service_io_ops_total` côté ingest (compté avant analyse, tronqué au-delà de 1024 services, incluant les lots délestés ensuite).                                                                                                                                                                             |

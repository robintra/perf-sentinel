...
A few terms recur across the docs and code, and the distinctions matter:
...
- **Pattern**: the normalized SQL, HTTP or messaging template that groups events for a finding (e.g. `SELECT * FROM users WHERE id = ?`).
...
|---------------------|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **event**           | `event.rs`           | Core `SpanEvent` type (SQL, HTTP and messaging variants) with timestamp, trace/span IDs, service, operation, target, duration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **score**           | `score/`             | GreenOps scoring (`mod.rs`): IIS per endpoint, waste ratio, top offenders, green_impact per finding. Carbon pipeline split across `carbon.rs` (embedded grid intensity table, SCI constants), `carbon_compute.rs` (per-span accumulation loop), `carbon_profiles.rs` (hourly grid profiles), `region_breakdown.rs` (region folding, model-tag selection, `CarbonReport` finalization). Six opt-in power and energy backends, each a structured submodule: `alumet/`, `scaphandre/` and `kepler/` (RAPL), `redfish/` (BMC telemetry), `cloud_energy/` (CPU% + embedded SPECpower interpolation), `electricity_maps/` (live grid intensity, only when configured). Shared scraper plumbing: `energy_state.rs` (per-service coefficient cache), `ops_snapshot_diff.rs` (per-service op deltas per scrape window) and `prom_parser.rs` (Prometheus exposition parser shared by the Kepler and Alumet scrapers). Fixed-threshold avoidable computation for disclosure anti-gaming (`canonical.rs`). Multi-region SCI v1.0 via OTel `cloud.region` attribute, confidence intervals, hourly grid profiles for 22 regions (full monthly x hourly for FR/DE/GB/US-East, representative daily for 18 more) |
...
| **capture**         | `capture.rs`         | CI-side OTLP receiver: mounts the same OTLP gRPC/HTTP listeners as the daemon against a raw sink, writing each request back out as one NDJSON line for `analyze --ci`. No correlation, detection or scoring, and no conversion either, so the file describes what the application sent. Backs the `capture` subcommand, which also wraps a test command and propagates its exit code                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
The CLI crate stays thin: it parses arguments, loads config and delegates to sentinel-core functions. All business logic lives in sentinel-core.
...
The published binary enables all four. Library consumers of sentinel-core start with everything off and opt in. `tonic`, `axum`, `tower`, `prometheus` and `opentelemetry-proto` remain unconditional dependencies of sentinel-core because `report/metrics.rs` and `ingest/otlp/` use their types even when no listener runs.

...
- Periodic disclosure schema v1.4: `aggregate.database_waste` sums the database-side avoidable energy and carbon over the period at both thresholds (operator and binary-pinned canonical, the same anti-manipulation construction as the waste tiers), with its provenance `models` and `windows_with_figure`. An informational lower bound kept out of every total. The per-window archive carries the matching `disclosure_waste.database` block. See `docs/SCHEMA.md`.

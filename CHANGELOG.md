...
- Cross-trace correlation confidence stays inside the `[0, 1]` the field documents. The correlator scored one co-occurrence per (source occurrence, target finding) couple, so a source seen twice and followed by three targets counted six against a denominator that counted the source twice, and a live report showed `conf 150%`. Each source occurrence now counts once per pair, tracked by a monotonic sequence number because every finding in a batch is stamped with the same timestamp and timestamps could not separate them. The published ratio is also clamped, since the pair counter accumulates for the life of the pair while the source total only spans the rolling window, so a long-lived pair can still outgrow its denominator.

...
- `green_summary.database_waste` now exists on every run: when no Alumet measurement of the database cgroup is available (batch runs, managed databases, no `[green.alumet.database]`), the figure is estimated from the modeled energy of the window's SQL spans (verb multipliers, calibration and regional intensities included) multiplied by the SQL waste ratio. The new `model` field carries the provenance (`alumet_rapl` = measured, `io_proxy_*` = estimated), surfaced on every display. Still excluded from `energy_kwh` and `co2`.

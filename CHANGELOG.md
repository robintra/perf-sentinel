...
- `POST /api/incidents` receives an Alertmanager webhook and freezes the findings of the window that preceded the incident, `GET /api/incidents` returns them, both behind the opt-in `[daemon.incidents]` section and its required `api_key`. The point is not to record that an incident happened, it is to capture the findings before the FIFO ring evicts them, which on a busy fleet takes minutes: by the time anyone opens a terminal the window is usually gone. The window is `[at_ms - lookback_ms, at_ms + 2 * trace_ttl_ms]`: a finding is stamped when its trace is analysed, one TTL after its last span, so the traces live at the crash land after `startsAt` and a window closed there would miss the findings a post-mortem wants most. The first freeze runs at reception and a settle pass re-resolves the same window one TTL after it closes, once the analysis that stamps the last traces has caught up, keeping the result when it is not shorter. Each incident carries the `oldest_finding_ms` read under the same lock as the fold, which is what separates a complete capture from one the ring had already eaten into. The Alertmanager envelope is the only accepted shape, because it is the only one an operator cannot produce otherwise (`webhook_config` has no body template) while any script emits it with curl. The service comes from a configurable label and an alert without it is refused, that label being the join key to the findings. The kind comes from a second label whose value must be one of five, never guessed from `alertname`. Reposting is idempotent on a content-derived id and never degrades: a repeat re-resolves a fixed window against a ring that only evicts, so the first capture is kept and a `resolved` delivery only closes it, and `perf_sentinel_incidents_total{kind}` counts incidents rather than webhook deliveries. `startsAt` is read as RFC 3339 with any offset, the way Go serializes it. This is about observed services, not the daemon's own cgroup, which stays `perf_sentinel_ingest_memory_pressure`. `[daemon.incidents] archive_path` appends every accepted delivery to a newline-delimited JSON file, off the reactor and never failing the webhook, because the ring dies with the daemon and a node-level memory event that kills the observed service often takes a co-located daemon with it, destroying the very record that would explain the outage. Append-only with the last record of an id winning, one open per delivery, with the guards the other appenders already have: a symlink is refused, a pre-existing file with group or world bits is refused since `0600` applies on creation only, and a crash-truncated last line is sealed. Deliberately without rotation: a few incidents a day against the analysis archive's several windows a second does not justify a bounded channel and a drop counter.

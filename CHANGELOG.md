...
- `analyze --format json` carries the findings' masked span trees the way `tempo` and `jaeger-query` now do, and all three only embed when the JSON sink will serialize them, sparing the text and SARIF paths the clones nobody reads. The embed also gained a 512 MiB safety ceiling that keeps the traces of the top-ranked findings and logs the drop, so a producer can no longer write a file its own `report --input` refuses at the 1 GiB read cap.

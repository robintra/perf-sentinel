...
- `pg-stat` and `mysql-stat` cross-reference statements against every SQL template observed in the traces instead of only the templates that produced a finding, so a healthy traced query finally gets its `[seen in traces]` marker, and both report the matched share (`trace_match`: templates and calls-weighted percentage, also shown on the dashboard panels). Worded "trace-matched", not "coverage": the database counters are cumulative since the last statistics reset.

...
- An acknowledgment that is still active but suppressed nothing in a run is reported, under the stable `unmatched_acknowledgment` warning kind. The file was write-mostly: nothing ever said an entry had become dead weight, so a finding accepted once stayed accepted long after it was fixed, and the accepted-problem list only grew. It also closes a hole no baseline comparison can: acknowledging a finding removes it from a baseline produced with filtering on, so the pull request that genuinely fixes it later has nothing left to make disappear and its Diff shows no resolution at all. The warning answers that directly, without a baseline and without needing both sides to have covered the same endpoints. It deliberately states two readings rather than one, since an entry carries a signature and no endpoint: the problem was fixed, or the scenario that produced it did not run. Claiming a fix would invite deleting an entry that still protects the gate, so the disambiguation is left to the run's per-endpoint I/O op counts or to re-running the campaign. An expired entry is inactive and is never reported this way, its finding is already back in the gate.

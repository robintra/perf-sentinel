...
- `[green] include_network_transport` no longer decides what the disclosure counts. The transport term is computed on every run and always published, and the setting only hides it in the dashboards, which is what `scoring_config.show_network_transport` now carries into each archived window for the TUI to honour. Two organisations whose reports disagreed on this setting were not comparable, and the setting left no trace in the published figures. The term stays inside `total_carbon_kgco2eq`, like the embodied term it resembles: irreducible, uncertain, and part of the perimeter a disclosure must not under-state. Consequence to expect once: a period scored with the setting off now reports a larger total than the same period would have under 0.9.24.

...
- Long normalized templates wrap instead of running off the right edge. A forty-column `UPDATE` rendered as one unbroken line in the detail pane, readable only by scrolling sideways. The block now breaks after each comma, a boundary a reader of SQL expects, using `<wbr>` rather than a zero-width space so the text copied out of the block is still the exact template.

...
- `source_endpoint` falls back to the code frame (`code.namespace` + `code.function`, or the fully-qualified `code.function.name`) when no HTTP attribute is found anywhere up that chain, instead of reporting the literal `"unknown"`. Entry points reached outside an inbound HTTP request, scheduled jobs and message consumers, previously produced findings that named no origin. Worse, since the ack signature is `type : service : endpoint : hash(template)`, two distinct jobs in one service issuing the same statement shared a signature, so acknowledging one silently hid the other. Applies to the OTLP, Jaeger and Zipkin ingestion paths, all three of which also learned the stable `code.function.name` / `code.file.path` / `code.line.number` spellings. The separator is `.` rather than the Java-idiomatic `#`, because `#` is truncated by the endpoint secret-stripping applied at the ingestion boundary; a `#` inside a frame is rewritten to `.` for the same reason.
- The code frame is used only when it names an origin. A blank attribute, one holding ASCII control characters (already dropped from the `code_*` fields by ingestion sanitization), or a bare unqualified function name such as `execute` all keep `"unknown"`, which is honest, rather than producing an empty endpoint, a leading-dot endpoint, or a generic one that collides exactly as `"unknown"` did.

### Migration

- Findings whose endpoint was previously `"unknown"` change signature, and that is a wider set than scheduled jobs alone: it includes HTTP findings whose route lived two or more levels above the leaf span, which is the common case on Spring, Quarkus and most layered stacks. Any entry in `.perf-sentinel-acknowledgments.toml` recorded against an `unknown` endpoint stops matching and must be re-recorded. Findings whose endpoint was already a route are unaffected. Run `analyze` once and copy the new signatures from the report.
- `diff` baselines captured before this release are affected the same way. `diff` keys finding identity on `(type, service, source_endpoint, template)`, so every finding that moves off `"unknown"` is reported once as resolved and once as new on the first run after upgrade, with no application change behind it. A CI job uploading `diff --format sarif` raises a fresh code-scanning alert for each. Re-capture the baseline against this release before comparing.

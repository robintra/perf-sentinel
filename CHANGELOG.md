...
- OVHcloud, Scaleway and 3DS OUTSCALE regions carry grid intensity and PUE, next to AWS, GCP and Azure. Until now a workload on any of the three landed in the synthetic `unknown` bucket and contributed **zero operational CO2**: not an absent figure, a wrong one, on three European sovereign clouds. 42 keys join the table, and the intensities are not hand-entered, they come from the same Ember refresh by ISO3 country code that already feeds the other providers, so the script stays the single source. OVHcloud names one datacenter three ways depending on which API answers (`GRA11` for the OpenStack Public Cloud region, `GRA` for the zone code, `gra` for the S3 location string) and all three are keyed, since which one reaches `cloud.region` depends on the SDK the caller instruments. OUTSCALE keys carry an `outscale-` prefix, because OUTSCALE reuses AWS region identifiers for different places: its `eu-west-2` is Paris where the AWS one is London, 42 against 221 gCO2/kWh on the same string, so an unprefixed row would have silently scored a French deployment on the British grid. That deployment declares the prefixed key in `[green] default_region`. PUE is per provider as it already was for the other three: 1.24 for OVHcloud (FY25 group average, measured over twelve months and audited by Apave against ISO 30134-2) and 1.375 for Scaleway (2024 average over ten datacenters). OUTSCALE publishes none, and rents capacity rather than operating its own datacenters, so its rows ride the generic prior rather than a number invented for them; the 1.2 its partner advertises is a design figure for one site, and the 1.64 that circulates traces back to a 2021 infographic that does not state it. Each region also gets its own hourly-profile alias, without which it would have resolved to the flat annual value while looking supported. The datacenter-level PUE both OVHcloud and Scaleway publish is deliberately not modeled: an 8% spread around a group average sits far inside the 2x bracket the whole estimate carries. The SPECpower path (`[green.cloud]`) still accepts only `aws`, `gcp` and `azure`, since the CCF coefficients publish nothing for these instance families and a fallback would be a fabricated wattage; those machines stay covered by Alumet or Scaphandre.

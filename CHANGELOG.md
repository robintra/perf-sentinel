...
- The periodic disclosure schema moves to `perf-sentinel-report/v1.5`, adding the three messaging pattern names to the `PatternName` enum. Purely additive: no field is added or removed, and re-hashing an existing v1.4 report still yields its original `content_hash`, since the hashed form carries the report's own `schema_version`. A report generated by this build declares v1.5, so it is not byte-identical to one generated by 0.9.22.

...
- The service meters share one admission policy (`CappedServices`): anonymous spans (Zipkin/Jaeger default to an empty name) now count under `service="unknown"` on the ingest-side `service_io_ops_total` too, instead of minting `service=""` (which honor-labels scrapes re-attribute to the scrape target), and `_other` is reserved everywhere: a real service named `_other` merges into the fold bucket without taking a cap slot. Library API change for embedders: `score::score_green` returns a fourth tuple element, the per-service split of the deduped avoidable ops.

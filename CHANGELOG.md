...
- The OTLP gRPC listener accepts gzip and deflate compressed exports. The OTel Collector's OTLP exporter compresses with gzip by default, and the listener declared no accepted encoding, so tonic answered `Unimplemented: Content is compressed with 'gzip' which isn't supported`. The Collector treats that status as permanent, which means it dropped every batch without retrying: a collector left at its defaults sent nothing through, while the pod stayed green, `/health` kept answering and every `/metrics` counter stayed at zero, so the failure only existed in the collector's own logs. The documented Collector recipe pointed at `:4317` and the instrumentation guide stated in the same breath that no `compression: none` override was needed, a claim that only ever held for the OTLP HTTP endpoint, which decompresses since it exists and is unaffected. That endpoint accepted gzip only, it now takes deflate as well, so the two endpoints accept the same encodings. The decoding size cap keeps applying to the decompressed message, so `[daemon] max_payload_size` still bounds a compressed payload and no window opens for a compression bomb. `perf-sentinel capture`, which receives from the same collectors, gets the same two encodings. `zstd` is deliberately left out: the collector only sends it when explicitly configured, and it would pull C bindings into a static musl binary. On 0.9.26 and earlier the workaround stays `compression: none` on the exporter, or pointing it at the HTTP endpoint.

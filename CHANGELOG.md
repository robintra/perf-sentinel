...
- `GET /api/findings` folds recurring findings into one row per problem. Detection is per trace, deliberately, so a recurring pattern re-emits an identical finding for every request that exhibits it: an operator polling the endpoint on a busy service read the same `redundant_sql` nineteen times in as many traces and reasonably concluded deduplication was broken, and the very first field tester did. The fold happens on the read path, keyed by the canonical acknowledgment signature, the product's existing "same problem" key. The stored buffer still holds every per-trace detection, which matters more than it looks: those instances carry the per-trace severity, they are what `/api/findings/{trace_id}` answers with for a trace whose spans have already aged out of the window, and FIFO pressure on them is the only thing that ever expires a fixed problem. Folding them away in the store instead would have downgraded a critical occurrence to whatever the next quiet trace produced, emptied that triage endpoint, and turned every count-based quality-gate rule into a distinct-problems rule. A folded entry carries two additive fields, `seen_count` and `first_seen_ms`, and the worst severity of the fold; `stored_at_ms` keeps its meaning. `limit` applies after the fold, so a pattern recurring on a hundred traces can no longer consume a whole page. `/api/findings/{trace_id}` and the exported quality gate deliberately read the unfolded instances.

- The recurrence count survives the export boundary. `/api/export/report` flattens stored entries into plain `Finding`s, so coalescing alone would have traded nineteen noisy rows for one row that no longer said it had happened nineteen times, on the dashboard operators actually read. `Report` gained `finding_occurrences`, a daemon-only list of `{ signature, seen_count, first_seen_ms }` for signatures seen more than once, following the `correlations` precedent rather than adding a daemon concept to the `Finding` type every detector constructs. The HTML dashboard joins it back by signature and labels the row with the number of traces, kept visually distinct from the existing `×N` metric, which counts spans inside a single trace. `perf-sentinel query --daemon <URL> findings` prints the same summary from `/api/findings`, which carries the tally directly. The quality gate in the same response is still evaluated on the unfolded per-trace detections, so its count-based rules keep counting occurrences.

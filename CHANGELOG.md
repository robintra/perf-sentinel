...
- `disclose` reports how many aggregated windows carried no canonical waste figure. A period mixing pre-canonical archives with newer ones passes `--intent official` validation because the canonical threshold is the maximum across windows, so the canonical tier under-reported by exactly the older windows' share, silently.

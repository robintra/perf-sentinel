...
- An acknowledgment that is still active but suppressed nothing in a run is reported, under the stable `unmatched_acknowledgment` warning kind. The file was write-mostly: nothing ever said an entry had become dead weight, so a finding accepted once stayed accepted long after it was fixed, and the accepted-problem list only grew. It also closes a hole no baseline comparison can: acknowledging a finding removes it from a baseline produced with filtering on, so the pull request that genuinely fixes it later has nothing left to make disappear and its Diff shows no resolution at all. The warning answers that directly, without a baseline and without needing both sides to have covered the same endpoints. What it can say depends on the entry. Two optional fields, `service` and `source_endpoint`, copied verbatim from the finding, let the warning check the run's per-endpoint I/O op counts: an exercised endpoint with no finding reads as "looks fixed, the entry can be removed", an absent one as "proves nothing, keep the entry". An entry without the fields keeps a deliberate double reading, fixed or simply not run, since its opaque signature cannot tell the two apart and claiming a fix would invite deleting an entry that still protects the gate. An expired entry is inactive and is never reported this way, its finding is already back in the gate.

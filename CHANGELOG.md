...
- The daemon findings store coalesces by signature instead of listing one row per trace. Detection is per trace, deliberately, so a recurring pattern re-emitted an identical finding for every request that exhibited it: an operator polling `/api/findings` on a busy endpoint read the same `redundant_sql` nineteen times in as many traces and reasonably concluded deduplication was broken, and the very first field tester did. The listing was the smaller half of the problem: the store was a plain ring buffer, so one hot pattern could fill all `max_retained_findings` slots (10 000 by default) and evict every rarer finding from the API. Entries are now keyed by the canonical acknowledgment signature, the product's existing "same problem" key: one entry per distinct problem, refreshed in place with its latest instance, plus `first_seen_ms` and `seen_count` fields on the wire (additive, `stored_at_ms` keeps meaning the latest store time). Recurrences of a known pattern no longer consume capacity, and eviction only triggers past `max_retained_findings` DISTINCT problems, oldest last-seen first. One behavioral edge moves with this: `GET /api/findings/{trace_id}` only matches a finding whose latest instance belongs to that trace, `/api/explain/{trace_id}` stays the exhaustive per-trace view. The `perf_sentinel_stored_findings` gauge now counts distinct signatures.

- The recurrence count survives the export boundary. `/api/export/report` flattens stored entries into plain `Finding`s, so coalescing alone would have traded nineteen noisy rows for one row that no longer said it had happened nineteen times, on the dashboard operators actually read. `Report` gained `finding_occurrences`, a daemon-only list of `{ signature, seen_count, first_seen_ms }` for signatures seen more than once, following the `correlations` precedent rather than adding a daemon concept to the `Finding` type every detector constructs. The HTML dashboard joins it back by signature and labels the row with the number of traces, kept visually distinct from the existing `×N` metric, which counts spans inside a single trace. `perf-sentinel query --daemon <URL> findings` prints the same summary from `/api/findings`, which carries the tally directly. The TUI's Config tab also stopped describing the store as a ring buffer evicting the oldest insertion.

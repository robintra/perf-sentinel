...
- `[green] embodied_carbon_per_request_gco2 = 0.0` is rejected at config load. No hardware has zero embodied carbon, and a zeroed coefficient erased the SCI `M` term from the disclosure while looking like a measurement. The coefficient stays configurable, hardware fleets genuinely differ and the applied value is published under `calibration_inputs`, but zero stops being a way to opt a term out of a public figure. An absent `embodied_kgco2eq` therefore has a single meaning: no green-scored window in the period.

...
- Three task-sized example configs sit next to the reference one and are usable as they are: `perf-sentinel-energy-alumet.toml` for measured energy on bare metal or a Kubernetes node, `perf-sentinel-energy-cloud.toml` for the modelled path on cloud VMs with no readable RAPL, and `perf-sentinel-carbon-live.toml` for live grid intensity. The reference file stays the inventory and now points at them. They are separate files rather than thematic fragments because only one config file is ever loaded, there is no include or merge, and energy and carbon both live under `[green]`, which TOML refuses to see declared twice: fragments would be neither usable alone nor concatenable. A test now loads every file under `examples/` through the parser, which is what nobody was doing when the Redfish and Scaphandre snippets shipped in a shape it rejects.
...
- The sections that expose the daemon or change what it reports carry an explicit `/!\` warning rather than a neutral note, including one that was not stated anywhere: `[daemon.ack]` is enabled by default and its routes accept any caller until `api_key` is set, which is harmless on the default loopback bind and is not on `0.0.0.0`. `examples/perf-sentinel-docker.toml`, which does bind `0.0.0.0`, gets the same warning and moves to `environment = "production"`, with a note that `"prod"` is rejected at load time.

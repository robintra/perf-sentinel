...
- The published carbon total is split into the three terms it is made of, under `aggregate.carbon_breakdown`. A disclosure exists to state avoidable waste, and the single total mixed what an anti-pattern fix can reduce with what it cannot: only the operational term carries the avoidable share the waste tiers quantify, while amortised hardware and cross-region network transport are irreducible by construction. Reading `total_carbon_kgco2eq` alone therefore overstated the addressable footprint by whatever those two terms weigh, with no way to tell. The three sum back to the total, and the transport term is omitted rather than zeroed when no window counted any: a zero would not distinguish the setting being off from it being on with no cross-region traffic, since the pipeline produces identical windows for both and tags them the same.  Network transport is flagged as the least certain of the three rather than given a fabricated interval: its coefficient defaults to an upper bound and published figures span two orders of magnitude, so the report's uniform 2x bracket understates it and it should be read as a ceiling, which is now written next to the field in both languages.

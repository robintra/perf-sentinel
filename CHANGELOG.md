...
- Archived windows are hash-chained as the daemon writes them, and `disclose` verifies the chain while aggregating. Each line records the hash of `{ts, report, prev, seq}`, the hash of its predecessor and its position in the file, so an edited, removed, reordered or truncated window is detectable: `prev` alone cannot see a run removed from the end of a file, which leaves a shorter but self-consistent chain, and `seq` is what closes that. A line whose `hash` field is deleted counts as a break rather than as a pre-chaining line, once the file's chain has started, otherwise stripping the field would have been a way to rewrite a window and still publish zero breaks. The chain head only advances after a line reaches the file and every line is flushed as it is written, so a disk error or an ungraceful kill surfaces as a dropped window rather than as an accusation of tampering. Startup reads back the tail of the archive instead of the whole file to resume the chain. Until now the integrity story stopped at the published document: the `content_hash` and the cosign signature proved a report had not moved since generation, while the NDJSON windows it was built from carried nothing at all, so editing one before running `disclose` produced a perfectly coherent report with a valid hash. Each line now records the hash of `{ts, report, prev}` and the hash of its predecessor, which makes an edited, removed or reordered window detectable. `integrity.trace_integrity_chain`, reserved and always null since v1.0, carries the verdict: windows verified, windows written before chaining existed, and breaks found. A break is published rather than fatal, so a truncated archive or an old rotation still yields an honest partial report, and the field sits under the `content_hash` precisely so a break count cannot be reset to zero under a valid signature. A restarted daemon reads the last hash back before appending, otherwise every restart would look like tampering. This is tamper-evidence after the fact, not proof of a sincere source: whoever controls the daemon and its files can regenerate a consistent chain, which is what `integrity.cross_period_log` stays reserved to address, and both documents say so.

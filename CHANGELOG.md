...
- `analyze --ci` can start failing on a bus-heavy service with no configuration change: N+1 and redundant messaging findings count as avoidable I/O, so they feed `io_waste_ratio` and the `io_waste_ratio_max` gate rule. No gate rule is dedicated to the messaging types, the ratio is the only path.

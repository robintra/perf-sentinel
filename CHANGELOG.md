...
- Configuration can now be split into deterministic fragments under `.perf-sentinel.d/`. Names must follow `NN-lowercase-name.toml`, priorities must be unique, tables merge recursively, later values win, and the main `.perf-sentinel.toml` loads last. The ready-to-copy examples are standardized as `30-green-alumet.toml`, `31-green-cloud.toml`, `32-green-scaphandre.toml`, `33-green-kepler.toml`, `34-green-redfish.toml`, `40-green-electricity-maps.toml` and `60-daemon-docker.toml`. Each remains usable alone with `--config`, while copying several into the fragment directory composes them without concatenating duplicate `[green]` tables.

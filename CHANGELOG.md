...
- The incident ring survives a daemon restart when `[daemon.incidents] archive_path` is set. The daemon wrote every incident, close and settle to the archive but never read it back, so `/api/incidents` and the Grafana incident panels started empty after every restart or upgrade. At startup, before the API serves, the archive is now streamed line by line, from its last 256 MiB at most so an unrotated file cannot hold the daemon past its liveness probe: it is replayed in the order it was written, so the last record of each id wins and the last `max_retained` incidents to arrive come back in the order the ring held them, lines that do not parse (a torn write sealed at open) are skipped with one warning, and nothing is appended back. The read uses the append side's symlink guards. `docs/CONFIGURATION.md`, `docs/QUERY-API.md`, `docs/RUNBOOK.md` and their French mirrors describe it.
- The incident panels of the findings dashboard reach the whole ring. They read `/api/incidents?limit=50`, so only the 50 most recent incidents were reachable. The `Incidents` table keeps pages of 50, since every incident carries its frozen findings and it fetches the page on each refresh, from the offset a new `Incident skip rows` variable picks (0 to 950, by 50). `Incident findings` asks for its one incident through the new `id` parameter of `GET /api/incidents`, so it no longer fetches a page of frozen findings to show one incident, and a clicked row stays found when a new incident shifts the page. A daemon older than 0.23.1 ignores `id`, and the table then reads the same page as the row clicked. Dashboard `version` 10. `docs/HELM-DEPLOYMENT.md` and its French mirror describe it.
- The example incident alerting rules (`examples/incident-alerts-prometheus-operator.yaml` and its VictoriaMetrics twin) keep only the services the daemon ingested over the last day, through an `and on (service)` join on `perf_sentinel_service_io_ops_total`. Their container selector also matches workloads the daemon does not trace, and each of those raised an incident with no finding, which cluttered the incident panels.

### Added

- `GET /api/incidents` takes an `id` parameter and answers with that one incident, or `[]` when the ring does not hold it, ignoring `service`, `namespace`, `offset` and `limit`. A client that holds an id, the findings dashboard's `Incident findings` table among them, no longer pages through incidents that each carry up to 1000 frozen findings to pick one out, and finds it wherever a newer incident pushed it. An older daemon ignores the parameter and answers with the page. `docs/QUERY-API.md` and its French mirror describe it.

### Fixed

- A finding whose only framework signal was an OpenTelemetry Java agent scope (`io.opentelemetry.jdbc`, `io.opentelemetry.apache-httpclient-5.0`, `io.opentelemetry.spring-webmvc-6.0`) and that carried no `code_location` got no `suggested_fix`, not even the `java_generic` one: the language-from-scope step knew the Go, npm, NuGet, Ruby and PHP prefixes but not the Java agent's own `io.opentelemetry.<library>` convention. Any `io.opentelemetry.` scope now identifies Java, after the PHP `io.opentelemetry.contrib.php.` prefix and after the framework scope rules, so `spring-data` still gives `java_jpa` and PHP keeps its scopes. The Java namespace rules then run on `code_location` when present, then the service-name rules (`helidon-mp-orders` still gives `java_helidon_mp`), and `java_generic` applies otherwise. `java_generic` also gains an `n_plus_one_sql` fix (one `JOIN` or `WHERE id IN (...)` query, `NamedParameterJdbcTemplate` or, on PostgreSQL, `= ANY(?)`), in line with the Go, Node, Ruby and PHP generics.
- Cross-trace slow findings never carried a `suggested_fix`, even with a `code_location` that resolves to `java_jpa`: only the per-trace detector output went through the suggestions step. The batch `detect_slow_cross_trace` findings and the daemon's cross-batch slow window findings are now enriched when they are built, so `analyze` output and `/api/findings` show the fix on them too.
- `serialized_calls`, `excessive_fanout`, `chatty_service` and `pool_saturation` findings carried no instrumentation scopes and no `code_location`, so no framework or language could be inferred and they never got a `suggested_fix`. Each now carries the scopes and `code_location` of one representative call it already references: the first call of the serialized sequence, the first child of the fan-out, the first outbound HTTP call of the chatty trace and the first SQL span of the saturated service. The CLI `Source:` line, the HTML report `source` line and SARIF `locations[]` point at that call. Finding signatures do not depend on either field, so existing acknowledgments still match.
- A finding whose detected framework had no fix for its type (`redundant_http` or `pool_saturation` under `java_jpa`, `pool_saturation` under `java_webflux`) got no `suggested_fix` at all, less than the language generic it would have received without the framework signal. The lookup now falls back to the framework's language generic (`java_jpa` to `java_generic`), and a framework-specific fix still wins when one exists. The messaging fixes, keyed by broker, are unchanged.
- The `info` severity of the findings dashboard's `Findings` and `Incident findings` tables, and the `empty ring` capture of its `Incidents` table, used Grafana's `text` color, which a color-text cell draws dark whatever the theme, so they were unreadable on a dark background. They are blue now.
- Two changes to one incident could reach the `[daemon.incidents] archive_path` file in the opposite order to the one the ring applied them: a new incident, a close and a settle each handed their record to the archive writer after releasing the ring's lock. A `resolved` delivery racing the settle pass could then leave the older record last, and the last record of an id is the one that counts, for the startup reload as for any reader of the file, so the incident came back open or without its settled findings. The record is now handed over before the ring unlocks, and the hand-off still never waits: a full channel drops it and counts it on `perf_sentinel_incidents_archive_failed_total`, as before.
...
- Every other place a person reads a time shows it in local time too, the viewer's browser time zone on the HTML report and the machine's on the terminal. On the report that is the `seen first` and `last` window under a finding, which dates both sides when they fall on different local days, the span times in the Explain tree, whose tooltip keeps the exact timestamp, and the `Expires` column of the live-mode acknowledgments table. On the terminal it is the `Window:` line of `analyze`, `report` and `diff` text output and of the `inspect` detail panel, the `Period:` line of `query correlations`, the `Expires:` line printed by `ack create` and the `AT` and `EXPIRES_AT` columns of `ack list`, each at the precision it had. The machine formats stay in UTC: JSON, SARIF, NDJSON, CSV exports, archives, periodic reports and the daemon API. On the terminal the zone is `TZ`, then the system's, and a `TZ` naming a zone the system database lacks resolves against an IANA database embedded in the binary (`chrono-tz`), so the `FROM scratch` container image, which ships none, prints `TZ=Europe/Paris` in Paris time instead of silently in UTC. `docs/CLI.md` and its French mirror describe it.
- The cross-trace correlator no longer keeps every finding occurrence of `[daemon.correlation] window_minutes`, so a 1440-minute window is practical. Pairing keeps only the findings analysed in the last `lag_threshold_ms + 2 x trace_ttl_ms`, about a minute of traffic at the default 30 s TTL, and the window only sets the span of the per-pair and per-endpoint counters. Each distinct endpoint, template included, is stored once and shared by the pairing deque and every pair that names it, and stays for as long as it is seen in the window, so memory grows with the number of distinct endpoints in the window rather than with occurrences. `window_minutes` is validated between 1 and 10080 at config load when correlation is enabled. The median lag reservoir keeps 64 samples per pair instead of 256, and when the `max_tracked_pairs` cap trips, eviction drops the lowest-count pairs first and the stalest among equal counts. `docs/CONFIGURATION.md`, `docs/design/04-DETECTION.md`, `docs/QUERY-API.md`, `docs/LIMITATIONS.md` and their French mirrors describe the correlator as it runs.
- Four dependency requirements move to their newest patch release: clap 4.6.7, clap_complete 4.6.11, ureq 3.4.2 and toml 1.1.6, with `Cargo.lock` refreshed. All semver-compatible. The fuzz workspace's lock file already carried toml 1.1.6 and does not use the other three.

### Added

- The daemon counts slow occurrences across analysis batches. A template slow in `slow_query_min_occurrences` separate episodes within `[detection] slow_query_window_minutes` (default 15, `0` disables, range 0 to 60) yields a `slow_*` finding with the same shape and signature as a batch cross-trace one, so it folds with them in the findings store. Episodes are counted per type, service, grouping and normalized template, and slow spans within max(60 s, 1.5 x `trace_ttl_ms`) of an episode's first span count as one episode, so an isolated slow span or a short lock stays in the duration histogram only. Spans already reported by a slow finding of their batch are not counted again, and a key reports at most once per window, on a batch that holds a fresh slow span of it. `pattern.occurrences` and the percentiles count episodes, one span each, and the finding's `trace_id` is the trace of the slowest span of the episode that fired it, analysed in that batch, so `/api/explain` finds it. At most 1024 keys are tracked, and the new counter `perf_sentinel_slow_window_keys_refused_total` counts the slow spans refused past that cap, with a warning logged once. `analyze` and the other batch commands are unchanged. `docs/CONFIGURATION.md`, `docs/design/04-DETECTION.md`, `docs/design/07-CLI-CONFIG-RELEASE.md` and `docs/METRICS.md`, with their French mirrors, describe the key, the window and the counter.
- The findings dashboard gains a `Finding type` variable that narrows the `Findings` table on the daemon through the `type` parameter `GET /api/findings` already accepts. The `Type` column header could only filter a page that `Max rows` had already cut, so a rare type never reached the table: `Slow SQL` and `Slow HTTP` findings, a fraction of a percent of a busy daemon's volume, sat outside the 1000 most recent rows. It lists the twelve finding types under the labels the `Type` column shows, one at a time, and its `All` sends a single space, which the API reads as no filter, like `Grouping` and `Service`. Dashboard `version` 9, and `docs/HELM-DEPLOYMENT.md` with its French mirror describe it.
- `GET /api/correlations` gains `source_sample_trace_id`, the trace of the source-side finding of the latest co-occurrence, next to `sample_trace_id` on the target side, so both sides of a pair open in Explain. On the HTML report the source side of a correlation card opens Explain on that trace when no finding of the report matches it, as the target side already did, and the correlations CSV export ends with `source_sample_trace_id` and `sample_trace_id` columns. The findings dashboard's `Correlations` table gains a `Source trace` column and its `Trace` column is renamed `Target trace`. Dashboard `version` 9, and `docs/QUERY-API.md` and `docs/HELM-DEPLOYMENT.md` with their French mirrors describe the field and the columns.
- `docs/INSTRUMENTATION.md` documents a third way to hand `analyze` a Java trace file: from agent 2.32.0, the first to bundle SDK 1.66, the `otlp_file/development` exporter of declarative configuration writes the file itself from the forked test JVM, one OTLP request per line, with no port and no `capture`. It names the three traps: the file opens in append mode, `OTEL_CONFIG_FILE` makes the agent ignore every `OTEL_*` variable, and an older agent ends in `BUILD SUCCESS` with no file. `docs/CI.md` and the Jenkins template name it next to `capture`, and the template no longer claims that the Java reference setup uses a file exporter. The French mirrors follow.

### Fixed

- Cross-trace correlation paired findings by the analysis tick that produced them instead of by their own timestamps. Every finding of a tick was stamped with the tick time, so only findings analysed in the same tick could pair, `median_lag_ms` was always 0, and which side was `source` and which `target` followed the order of the batch. Findings now pair on their `first_timestamp`, across ticks, the earlier one is the source and the lag is the gap between the two. Pair counts and source totals were counted over windows out of phase with each other, and a source that had left the window counted as 1, which inflated `confidence`. Both counts now share the same half-window buckets, a pair counts in the bucket of its source occurrence, and a pair whose source has no occurrence left in the window is not reported.
...
- Every panel description on the two Grafana dashboards is rewritten shorter and in paragraphs, so the help behind a panel's title reads as two to four short blocks rather than one wall of text, and names finding types by their labels (`N+1 SQL`, `Pool saturation`) instead of their identifiers. Overview dashboard `version` 9.
...
- The overview dashboard's yellow band is the darker `semi-dark-yellow` in the four places it appeared, the `I/O waste ratio` arc, the `Total findings` and `Active traces` stats and the headroom series. Grafana's plain yellow sits close enough to white that the light theme washed it out, and Grafana paints a gauge's value in the theme text colour whatever the field colour is, so the arc was the only thing carrying the band.

### Added

- The findings dashboard gains a `Correlations` table reading `GET /api/correlations`, one row per directional pair of findings that fired together across services while `[daemon.correlation]` is enabled: `Source service`, `Source type`, `Target service`, `Target type`, `Confidence`, `Co-occurrences`, `Source total`, `Median lag`, `Grouping`, the two operation templates, `First seen`, `Last seen` and the `Trace` of the most recent target-side finding. The daemon has computed these pairs since the correlator shipped, but nothing in Grafana read them, so an operator who enabled the section polled the route by hand to learn which N+1 drags which pool saturation behind it, and had to do so within one window of the last co-occurrence, since nothing persists a pair and a restart drops them all. The table honours neither `Grouping` nor `Service`, the route has no filter, and the column headers filter instead, like the acknowledgments and incidents tables. `Confidence` renders as a percentage and `Median lag` in milliseconds, `First seen` and `Last seen` are parsed from the route's ISO 8601 strings, and the columns keep their order through an organize transformation since the Infinity backend parser returns them alphabetically. Dashboard `version` 6, and `docs/HELM-DEPLOYMENT.md` with its French mirror name the table and what empties it.
- Both Grafana dashboards carry a `Compatibility` badge at the right end of their first panel line, saying when the daemon feeding them predates what their panels need. An operator who imported a dashboard once and never came back had no way to tell an empty panel from a daemon too old to fill it, and since the two dashboards fail differently they read the daemon's age differently. The overview one has no version to read, no perf-sentinel metric carrying one, so it grades the daemon by the presence of two metrics its panels depend on, `perf_sentinel_incidents_total` from 0.20.0 and `perf_sentinel_analysis_service_overflow_total` from 0.18.0, both published from daemon start whatever the configuration: orange below 0.20.0 where `Service silence` has nothing to draw, red below 0.18.0 where the `Service` and `Grouping` pickers still offer names the findings panels cannot honour, so a filtered view reads as a clean service. The findings one reads the `version` that `GET /api/status` has returned since 0.4.0 and turns red below 0.21.0, where it misreads rather than degrades: `grouping` and `offset` did not exist as query parameters and the API ignores what it does not know, so the `Grouping` pill names one deployment while the table lists every one of them and `Skip rows` returns the same page at every value. Both paint a neutral `Unknown` tile when nothing answers rather than a green one, and both fall to compatible on a daemon newer than they are, every metric and route change since 0.18.0 having been additive. Dashboard `version` 9 for the overview and 6 for the findings one, `docs/HELM-DEPLOYMENT.md` with its French mirror describes both banners, and `docs/RELEASE-PROCEDURE.md` with its own says when a release moves a floor.

### Fixed

- A trace rooted in a message consumer reported `unknown` as its endpoint whenever no application `code.*` frame sat on the chain above the I/O, which is the usual shape of a Spring AMQP or Kafka listener under the OpenTelemetry Java agent: the CONSUMER span is the root and the repository frames are siblings of the calls, not ancestors. Every finding of every consumer in a service then shared one endpoint, so acking one hid the others. The endpoint now falls back to the destination of the nearest CONSUMER span, `<messaging.system> <destination>`, reading `messaging.destination.template`, then `messaging.destination.name`, then the legacy `messaging.destination`, after the inbound route and the code frame, on OTLP, Jaeger and Zipkin. The nearest one, because when consumer spans nest the inner one is the listener that ran the work, where an outer `amqp-client` delivery span names only the exchange. A destination flagged temporary or anonymous, a server-named `amq.gen-*` or `spring.gen-*` queue, a placeholder in angle brackets such as `<default>`, which instrumentations emit for a default exchange, or one holding `?`, `#` or `@`, which the endpoint sanitizer would truncate, stays `unknown`, though an instrumentation may set neither flag, the OpenTelemetry Java agent among them, which is why the server-generated names are filtered as well. The daemon retains consumer destinations apart from route roots, under their own `max_events_per_trace` cap and with their parent links, because a CONSUMER span ends after the children it wraps and usually arrives in a later export, and reads them only where no route and no resolved ancestor answers. An endpoint an I/O span carries that no retained route confirmed, a frame or a destination, is cached unproven, so a listener calling its own API cannot outrank the route of the SERVER span below it, and at an ancestry cap of one a retained destination stops the sole retained route from being guessed.
- The daemon's ancestor walk stamped the entries above its outermost route with that route, so I/O spans on a sibling branch (a listener's own queries next to a self-call) inherited a route they never served. Only the entries below the outermost resolution are compressed now.
- The overview dashboard's `I/O waste ratio` gauge no longer reads a green `0.00` when it has nothing to divide. Its expression ended in `or vector(0)`, so a daemon publishing neither `perf_sentinel_service_avoidable_io_ops_total` nor `perf_sentinel_service_analyzed_io_ops_total`, which is every daemon below 0.18.0 and every daemon with `[green] enabled = false`, painted a large gauge at zero waste inside its green band over a fleet whose waste is simply unmeasured, and that is the artefact people screenshot into a status review. It reads `No data` there now. The dashboard's three other `or vector(0)` guards stay, they count findings and ingested operations where zero is a real and healthy value.

### Security

- **`rustls` bumped to 0.23.45 against RUSTSEC-2026-0285.** `rustls` reaches perf-sentinel transitively through `tokio-rustls` and `hyper-rustls`, which carry the daemon's own TLS listeners, and through `ureq`, behind the shared outbound HTTP client every scraper and the Tempo and Jaeger ingest paths use. A TLS 1.3 handshake message that followed a key-changing message inside the same record was accepted at the wrong encryption level, where RFC 8446 section 5.1 requires the connection to end on an `unexpected_message` alert. The transcript stays authenticated, so a network-position attacker cannot alter or complete a handshake through it, and the practical effect is that a peer could send in plaintext what should have been encrypted. Patch-level bump, semver-compatible, `cargo audit` green. The fuzz workspace's lock file does not carry `rustls` and is unchanged.
...
- `examples/incident-alerts-prometheus-operator.yaml` and `examples/incident-alerts-victoriametrics-operator.yaml` ship the alerting half of `POST /api/incidents`, which the daemon documented but nobody could copy. Four rules (`oom_kill`, `restart`, `memory_saturation`, `deploy`) carry the `service`, `perf_sentinel_kind` and `namespace` labels the route reads, and the receiver goes with them. `service` is derived fleet-wide from the pod name rather than written one rule at a time, the two cluster assumptions behind that derivation are stated at the top of the file, and the deploy rule keys on a new ReplicaSet rather than on the Deployment's generation, which the HPA also advances, so an autoscale is not posted as a release. Both files also name the namespace matcher their operator adds by default, `alertmanagerConfigMatcherStrategy` on one and `disableNamespaceMatcher` on the other, which matches nothing when the alerts carry the observed workload's namespace rather than the resource's own, and the prometheus-operator one names the route order that decides whether an earlier route swallows them first.
...
- The pinned toolchain moves from Rust 1.97.1 to 1.98.1 (`rust-toolchain.toml`, `rust-version`, the CI comments that name it and CONTRIBUTING.md), with clippy and rustfmt clean on the new release before any dependency moved. Every dependency requirement in the two `Cargo.toml` then goes to its newest release: clap 4.6.6, clap_complete 4.6.9, clap_mangen 0.3.3, uuid 1.26.1, hyper 1.11.1, ureq 3.4.1, libc 0.2.189, mimalloc 0.1.52, thiserror 2.0.20, toml 1.1.5, lru 0.18.4, http-body-util 0.1.5, tokio-rustls 0.26.5, tower-http 0.7.1, flate2 1.1.10, and the one major, dirs 7.0.0, whose only breaking change is `preference_dir` on Windows, which nothing here calls: the ack store reads `data_local_dir`, unchanged on every platform, and dirs 7 comes from the same publisher with the same single dependency, its repository having moved from GitHub to Codeberg. `Cargo.lock` is refreshed with it.
...
- A page of `GET /api/findings` no longer clones every folded row before cutting it. The fold cloned the first instance of each distinct signature, template and suggestion included, then applied the severity screen, the delta bound, `offset` and `limit` to those clones and dropped the rest, so a one-row read of a ring holding 100 000 instances folded into 4300 signatures paid for 4300 clones. It now accumulates the group metadata against a borrowed representative and materialises only the rows the page keeps, with the same representative (the worst-severity instance, the newest on ties), the same counts and the same order, which three new tests pin alongside the existing ones. Measured by the new `benches/findings_store.rs` on a 100 000-instance ring with production-sized strings (800-byte templates, one in twenty at 16 KB, 600-byte suggestions, 4300 distinct signatures): the first page of 1000 rows goes from 9.1 ms to 6.3 ms, a page at offset 3000 from 8.9 ms to 6.6 ms and a one-row read from 9.2 ms to 5.9 ms, all of it spent under the read lock the write path waits behind. What remains is the pass over the ring itself, hashing every instance's fold key. The store lives behind the `daemon` feature, which the core crate does not enable on its own, so the bench and the store's tests need `--features daemon`.
...
- `[daemon.archive] path` and the files it rotates into are created `0600`, where they were created at the process umask and landed world-readable at `0644` under the `0022` a container ships with. They hold the whole window `Report`, every finding, endpoint and normalized SQL shape, and the chart puts them on the same PVC as the ack store, which has refused a weak mode since it existed. The SQL is normalized, so no literal and no secret is in there, but the shape of a fleet is what a co-resident attacker reads next, and the same function already reasons about one when it closes the symlink race on rotation. Creation only, deliberately: a file already on disk keeps the mode its operator gave it, since tightening one the daemon may not own is exactly how the incident archive below failed to start. Run `chmod 600` on an existing archive to bring it in line.
...
- The findings dashboard gains a `Daemon status` line from `GET /api/status` (version, uptime, the three runtime gauges against their caps), an `Energy backends` table from `GET /api/energy`, and an `Include acked` variable with an `Acked via` column, since the API leaves acknowledged findings out by default (0.5.20) and a dashboard that never asks for them hides an acked critical with nothing saying so. It also declares the Infinity plugin in `__inputs` and `__requires`, so the import dialog binds a datasource instead of importing panels with nothing to query, and pins the findings table's column order through an `organize` transformation: the Infinity backend parser returns fields alphabetically, so the table opened on `Acked via` and `Endpoint` with `Severity` and `Type` pushed to the right. Its French mirror gains the `Grafana on the query API` section `docs/HELM-DEPLOYMENT.md` had carried alone.
- `GET /api/findings` accepts `since_ms`, a lower bound on `stored_at_ms` in Unix epoch milliseconds, inclusive. A collector polling a daemon had no way to ask for a delta and could only re-read the newest rows up to the 1000 cap, so on a fleet carrying more distinct signatures than that, a restart resynchronised the most recent ones alone. The bound applies after the signature fold, against the row's most recent detection, so `first_seen_ms` and `seen_count` keep reporting the whole retained history rather than the slice inside the window, for the same reason `severity` filters after the fold. It does not undo eviction: a signature the ring buffer already dropped is gone at any bound, which is what `max_retained_findings` governs. `/api/export/report` is untouched, it still snapshots the raw instances the quality gate counts. `docs/QUERY-API.md` and its French mirror also gain the `include_acked` row their parameter table had been missing.

- `GET /api/findings` accepts `until_ms`, the upper bound that closes a window, and `GET /api/status` reports `oldest_finding_ms`. Together they answer the post-mortem question perf-sentinel could not: what was already firing on a service in the five minutes before it was OOM-killed or restarted. `until_ms` makes the listing a window, from `since_ms` or the start of the buffer, folded over the detections inside it alone so `first_seen_ms` and `seen_count` describe the window: applied after the fold instead, an upper bound would keep only the groups that had gone quiet by then, and a chronic pattern running all week would match every incident window ever asked for. `since_ms` alone stays a delta poll with whole-buffer counts. `oldest_finding_ms` separates "nothing fired" from "the ring no longer reaches that far back", which were the same empty answer before. `docs/RUNBOOK.md` gains the recipe, and its existing archive recipe is corrected: it compared `.ts` as strings, and `"…T14:00:00.123Z"` sorts below `"…T14:00:00Z"` because `.` precedes `Z`, so it dropped the windows in the first second of the range and picked up the ones in the first second after it.

- `perf_sentinel_service_last_span_timestamp_seconds{service}` publishes when the daemon last received a span for each service, and the Grafana overview gains a `Service silence (time since last span)` panel over it. A Unix timestamp rather than an age, on the `process_start_time_seconds` convention, so `time() - gauge` is the age and a scrape gap cannot make it lie. It is what turns an alert into the window to hand to the `since_ms` / `until_ms` pair above. Two things it is deliberately not: a liveness signal, since a crash, a scale to zero, a rolling deploy and a quiet cron all read the same and an alert needs a `for:` longer than the service's normal idle gap, and a replacement for `increase(perf_sentinel_service_io_ops_total[10m]) == 0`, which already answers the common case. What it adds over that counter is an age unbounded by a range vector, and absence rather than zero after a daemon restart, where every counter resets and looks like a fleet-wide stop. Cardinality rides on the existing 1024-service ingest cap, and the stamp is taken once per batch rather than per event.

- `POST /api/incidents` receives an Alertmanager webhook and freezes the findings of the window that preceded the incident, `GET /api/incidents` returns them, both behind the opt-in `[daemon.incidents]` section and its required `api_key`. The point is not to record that an incident happened, it is to capture the findings before the FIFO ring evicts them, which on a busy fleet takes minutes: by the time anyone opens a terminal the window is usually gone. The window is `[at_ms - lookback_ms, at_ms + 2 * trace_ttl_ms]`: a finding is stamped when its trace is analysed, one TTL after its last span, so the traces live at the crash land after `startsAt` and a window closed there would miss the findings a post-mortem wants most. The first freeze runs at reception and a settle pass, one per delivery and bounded in flight, re-resolves the same window one TTL after it closes, once the analysis that stamps the last traces has caught up, and merges the result by signature so the record can grow but never lose a row, whatever the ring evicted in between. Each incident carries the `oldest_finding_ms` read under the same lock as the fold, which is what separates a complete capture from one the ring had already eaten into. The Alertmanager envelope is the only accepted shape, because it is the only one an operator cannot produce otherwise (`webhook_config` has no body template) while any script emits it with curl. The service comes from a configurable label and an alert without it is refused, that label being the join key to the findings. The kind comes from a second label whose value must be one of five, never guessed from `alertname`. A third label, `namespace_label` (default `namespace`, the one kube-prometheus alerts already carry), is optional and never a reason to refuse: its value is carried on the incident as `namespace`, joins the id only when present so a record without one keeps its id, and `GET /api/incidents` filters on it through a `namespace` parameter. It is a reading label, never a Prometheus label and never a bound on the freeze, which stays by service. A `deploy` is posted for the same reason as a `restart`: to freeze what was already firing before the rollout, and so a restart the rollout causes is not read as a crash. Reposting is idempotent on a content-derived id and never degrades: a repeat re-resolves a fixed window against a ring that only evicts, so the first capture is kept and a `resolved` delivery only closes it, and `perf_sentinel_incidents_total{kind}` counts incidents rather than webhook deliveries. `startsAt` is read as RFC 3339 with any offset, the way Go serializes it, label values are trimmed, an `endsAt` before `startsAt` is not an end, a delivery carries at most 1000 alerts, and the intake body tells `recorded` from `repeated`. `GET /api/incidents` pages with `offset`. This is about observed services, not the daemon's own cgroup, which stays `perf_sentinel_ingest_memory_pressure`. `[daemon.incidents] archive_path` appends every new incident, close and settle to a newline-delimited JSON file through one writer task opened at startup, so a bad path fails the daemon rather than the first incident, two records can never interleave, and a webhook handler dropped by a timeout has already handed its record over, because the ring dies with the daemon and a node-level memory event that kills the observed service often takes a co-located daemon with it, destroying the very record that would explain the outage. Append-only with the last record of an id winning, with the guards the other appenders already have: a symlink is refused, a pre-existing file with group or world bits is refused since `0600` applies on creation only, and a crash-truncated last line is sealed. Deliberately without rotation: a few incidents a day against the analysis archive's several windows a second does not justify a bounded channel and a drop counter.

- `perf_sentinel_incidents_rejected_total{reason}` counts what the incident routes refused: a request without the key, an alert without the service label, an unparsable `startsAt`, the alerts past the per-delivery cap. The intake body already says so, but Alertmanager discards it and never retries a 4xx (a group still firing is re-sent at the next `group_interval`, so the 401 repeats until the header is fixed and the capture is then whatever the ring still holds), so a receiver sending the wrong header or a rule with the wrong label lost every capture with nothing on a dashboard to show it.
- `[daemon] read_api_key`, with `PERF_SENTINEL_READ_API_KEY` winning over it, opens the two `GET`s a write key gates, `/api/acks` and `/api/incidents`, without the power to write: it never satisfies a `POST` or `DELETE`, never adds a gate where no write key is set, and must differ from both write keys, since a read key equal to a write key is that key. It exists so that Grafana and the Hub, which each carry one header value, never hold the key that can ack a finding or fabricate an incident, a hole the shipped findings dashboard already had on `/api/acks`. `/api/config` summarises it to `read_api_key_set` and reports `incidents_enabled`. The CORS wildcard refusal, which knew the ack key alone, now covers both write keys, so `[daemon.cors] allowed_origins = ["*"]` next to `[daemon.incidents] api_key` fails at load where it used to pass (a lone read key is never compared, so it is not a replay target). The environment overrides now apply even when no config file is present, where they were silently skipped: a container started with the keys in its environment and no mounted config ran with its write routes open and `/api/config` said so to nobody. Two claims that were already false are corrected on the way: `docs/CONFIGURATION.md` said perf-sentinel reads no environment variable, and `docs/RUNBOOK.md` said `GET /api/acks` stays unauthenticated.
- `perf-sentinel query monitor` gains an Incidents tab, its sixth, and `perf-sentinel query incidents` lists the same records from a terminal. `POST /api/incidents` freezes the findings of the window before a restart or a memory event, but the only way to read them back was a `curl` with the key by hand, and the monitor an operator already keeps open beside the pager showed nothing of it. The tab polls the 20 newest incidents from `GET /api/incidents` on every tick and lays them out like every other tab, one scrollable page: a bold row per incident with its start as local time, the service as `ns/service` when the alert carried a namespace (the `kubectl` form, which `query incidents` prints too), the kind, `firing` or `ended`, the finding count and a capture marker read from `oldest_finding_ms` against `window_from_ms` (`complete`, `partial` when the ring had already evicted part of the window so the list is short of what fired, `empty ring`), the alert's detail below it, then one indented row per finding with its type, severity, endpoint, window tally and whether it fired `before` or `after` the restart, from `first_seen_ms` against `at_ms`, that marker leading the row and the capture marker on its own line so an 80-column terminal never clips the two readings the tab exists for. It is the one tab that needs a key, so `query monitor` gains `--api-key-file` (`PERF_SENTINEL_DAEMON_API_KEY` wins over it, the same resolution as `query inspect`) and the read-only `[daemon] read_api_key` suffices, so the monitor never holds a key that can `POST`. The daemon's refusals stay hints on the tab rather than a `[STALE]` monitor: a 401 names the flag and the variable that pass the key and says the read key is enough, a 503 names `[daemon.incidents] enabled = false`, a 404 names a daemon older than 0.20.0, and a transient failure keeps the last list on screen, since the `[STALE]` marker tracks the report poll alone. The Config tab reads the two new `/api/config` booleans, `read_api_key` as set or unset and `incidents` as enabled or disabled, and the header now sheds its daemon URL, then the cadence, then the age when the terminal is narrow, so six labels and the stale marker still fit 80 columns. `query incidents` takes `--service`, `--namespace`, `--offset`, `--limit` (default 50, the daemon caps at 100), `--format text|json` and the same `--api-key-file`, prints one header block per incident (kind, service, start and end as local time, the window, the capture marker, how many findings fired only after the restart, the detail and the id) followed by its findings in the `query findings` layout, and exits 1 with the same three named causes instead of the generic unreachable-daemon hint. The demo tape seeds one Alertmanager envelope and tours the new tab with the read key. `query findings` inherits the same guard on the way: a `2xx` body it cannot parse now exits 1 naming the route, where it printed `No findings from daemon.` and exited 0, so a proxy's error page or a detector a newer daemon added no longer reads as a clean run in CI.
- The findings dashboard gains an `Incidents` table and an `Incident findings` table over `GET /api/incidents`, so the window an Alertmanager webhook froze is readable where the operator already looks rather than through a `curl` carrying a key. `Incidents` lists what was posted, newest first, with the namespace when the alert carried one, the capture window, the number of findings frozen for it and a `Capture` reading of `oldest_finding_ms` against the window's start (`complete` when the ring still reached below it, `partial` when part of the window was already evicted, `empty ring` when nothing was retained), and clicking a row sets the new `Incident` textbox variable through a data link that carries the dashboard's other variables and the time range, upon which `Incident findings` shows that incident's rows, folded over the window alone, with `First seen` in place of `Acked via` so a row that fired only after the restart is visible as such. Both tables read the same `limit=50` page so a clicked id is always found, both ignore the time picker since the route has no time filter, and both need a key: `examples/grafana-infinity-datasource.yaml` now sends `[daemon] read_api_key` as `X-API-Key` (`auth_method: apiKey`, the value expanded from `PERF_SENTINEL_READ_API_KEY` in Grafana's own environment), never the `[daemon.incidents]` write key, and that one header also opens `GET /api/acks` for the acknowledgments table when `[daemon.ack] api_key` is set while the ungated routes ignore it. The rows are built by JSONata in Infinity's backend parser, with `$map` rather than `$.{...}`, which the plugin's jsonata-go does not iterate, and a ternary returning `[]` so an empty selection reads No data instead of an error. `docs/HELM-DEPLOYMENT.md` and its French mirror describe the two panels, the read key in the datasource and the time-picker caveat.
...
- The Grafana dashboard's `Findings distribution by type`, `Top 10 finding types by severity` and `Findings detail by type and severity` listed every type whose series existed anywhere in the range, including the ones a replaced instance had emitted before the range and never during it: after a deploy, `slow_sql` sat in the pie's legend at zero for three hours and read as a detector that had gone quiet. The three queries now end in `> 0`, so a type absent from the legend is one with no increase over the range, and the pie's description says so.
- The findings dashboard's setup notes left three things to guess, all of which fail quietly. `examples/grafana-infinity-datasource.yaml` and the Helm guide told the reader to scope the dashboard's folder to the people allowed to see SQL templates and endpoint names, which only works in a folder of its own: dropped beside the cluster and database dashboards, restricting it restricts those too. They named the datasource port as 4318 rather than as the chart's `service.ports.otlpHttp.port`, which the chart cross-checks against `[daemon] listen_port_http` while this file sits outside that check, so a moved port leaves the panels hanging then answering 503 with nothing naming the cause. And neither said which Infinity version to install: the `2.0.0` in the dashboard's `__requires` is the floor the import dialog checks, not a statement about later majors, and the columns come from the backend parser, so a major bump can empty a table without erroring anywhere.
...
- The daemon's three appenders, the analysis archive, the incident archive and the ack store, open their file with an open that never follows the last path component: `O_NOFOLLOW` on Unix, where the analysis archive had the symlink check alone and a link planted between the check and the open was written through, and `FILE_FLAG_OPEN_REPARSE_POINT` on Windows, where the handle is then refused when it carries a reparse point. The ack store's symlink refusal was Unix-only, so a Windows daemon followed a symlinked `[daemon.ack] storage_path` in silence and now refuses it at startup. On Windows the refusal covers every reparse point, a cloud sync placeholder, a `compact /exe` file or a deduplicated file included, so point the three paths at a plain local file. The ack compaction creates its temporary file exclusively after removing any leftover instead of truncating whatever sat at the path.

## [0.19.0] - 2026-09-03

### Changed

- **`perf_sentinel_findings_total`, `perf_sentinel_slow_duration_seconds`, `perf_sentinel_service_io_ops_total`, `perf_sentinel_service_avoidable_io_ops_total` and `perf_sentinel_service_analyzed_io_ops_total` gain a `grouping` label next to `service`. Breaking for any dashboard or alert that matches these series without aggregating, the way 0.18.0 was for `service`.** The value is the finding's effective grouping, the first attribute present from `[detection] grouping_attributes` (`k8s.namespace.name`, then `service.namespace`, by default), so on Kubernetes it is the namespace the analysed traffic runs in, and an empty string when the span carried none, which Prometheus stores as no label at all. The label carries the value alone: two spans grouped by different configured attributes with the same value share one series where the findings keep them apart, so configure a single key when that matters. A `checkout` deployed in two namespaces was one series in 0.18.0 and is two now, which is the split the shipped dashboard could not make: its `Daemon namespace` variable selects the install, not the workloads. The label is not named `namespace` on purpose. Prometheus Operator attaches a `namespace` target label, and the chart's `serviceMonitor.honorLabels: true` (chart 0.17.1) would let the daemon's win, breaking that variable and every `namespace=~"$namespace"` filter on the dashboard. The cardinality rule holds through per-run caps on admitted (service, grouping) pairs, a second gate after the service caps: 512 on findings and the analysis-side I/O counters, 256 on the histogram, 4096 on the ingest-side I/O counter. A pair past its cap keeps its service and folds only its grouping into `grouping="_other"`, so `sum by (service)` over the new label equals the 0.18.0 per-service series exactly, `sum()` the pre-0.18 one, and the caps count pairs, so what folds follows services x groupings: 11 services in 20 namespaces is 220 pairs and never folds, 100 services in 10 namespaces is 1000 pairs and folds every pair past the 512th, where a cap on grouping values would have folded nothing. Capping pairs keeps the bound proportional to the series that actually exist. A query like `perf_sentinel_findings_total{severity="critical"}` now returns one series per (service, grouping) and an unaggregated alert on it fires per pair, wrap it in `sum()` or `sum by (service)` to keep the old behavior. A finding whose spans come from more than one service charges every service's avoidable share under the finding's grouping, which is exact: the detectors partition their groups by the grouping identity, so every span a finding charges already carries it and the per-(service, grouping) waste ratio cannot exceed 1. OpenMetrics exemplars follow the split, the worst-trace map is keyed on (type, severity, service, grouping). `[daemon] per_grouping_labels = false` restores the 0.18.0 shape (the label stays declared but empty on every series, which PromQL treats as absent), and unlike `per_service_labels` it governs the three I/O counters too. An install already running `per_service_labels = false` needs it as well to keep the histogram's pre-warmed unlabeled series: since 0.19.0 that series is minted at startup only when both knobs are off, so on the default it appears with the first slow span instead.
- **Breaking, `perf-sentinel-core` only.** `DaemonConfig` gains `per_grouping_labels`, `GreenSummary.avoidable_per_service` is keyed by `(service, grouping)` instead of the service alone, and `GreenSummary` gains an in-process `analyzed_per_service` with the same key. Neither struct carries `#[non_exhaustive]`, so a struct literal in a downstream crate needs the new fields and a reader of the map matches on the pair. `MetricsState`'s public vectors change arity (`findings_total` four labels, the three `service_*` counters two, `slow_duration_seconds` three), so a downstream `with_label_values` at the 0.18 arity panics at runtime. `DetectConfig`, `score_green`, `snapshot_service_io_ops`, `PerEndpointIoOps`, `Pattern.occurrences_by_service` and the acknowledgment signatures are unchanged.

### Added

- `[daemon] per_grouping_labels`, default `true`, the `grouping` twin of `per_service_labels`: `false` makes the label empty on every series of the five families above, restoring the 0.18.0 shape. Reported on `/api/config` next to `per_service_labels`. Three overflow counters keep the caps observable, `perf_sentinel_service_io_ops_grouping_overflow_total`, `perf_sentinel_analysis_grouping_overflow_total` and `perf_sentinel_slow_duration_grouping_overflow_total`, all counting attributions folded into `grouping="_other"`, not data lost, and the tuning advisor names the three caps when any of them moves. None ships as an alert, for the same reason as the service overflow counters in 0.18.0.
- The Grafana dashboard gains a `Grouping` variable ahead of `Service`, so the service list narrows to the groupings picked, and `grouping=~"$grouping"` reaches all eleven panels of the row now titled `Analysis (filtered by grouping and service)`. Its description says what `Daemon namespace` does not do, and that `grouping="_other"` is the fold bucket.

This release bumps the minor rather than the patch: `perf-sentinel-core` is published on crates.io and two public structs change shape, which under SemVer's `0.x` rules lands on the minor, the rule 0.10.0 and 0.18.0 followed.

## [0.18.0] - 2026-09-02

### Changed

- `DetectConfig` gains `sanitizer_aware_min_cv`. It is a public struct without `#[non_exhaustive]`, so a struct literal in a downstream crate needs the new field. A report written before the field reads back as `0.5`, the value its run hard-wired.

- **`perf_sentinel_findings_total` gains a `service` label, `perf_sentinel_slow_duration_seconds` a `service` label next to `type`. Breaking for any dashboard or alert that matches these series without aggregating.** The shipped Grafana dashboard carried a `Service` variable that filtered one panel out of twenty-one, because no other metric had the dimension. Selecting a service left `Critical findings` unchanged, which is how this was found on a production install. The label value is the finding's emitting service (`Finding.service`, `service.name` on the span), and the cardinality rule this repo enforces still holds through a per-run cap on the analysis side: 128 distinct services on findings, 64 on the histogram (14 series per (type, service) pair), services past a cap folding into `service="_other"` rather than being dropped, so `sum` over the label always equals the old unlabeled series and the global p95 stays exact. A query like `perf_sentinel_findings_total{severity="critical"}` now returns one series per service and an unaggregated alert on it fires per service, wrap it in `sum()` to keep the old behavior. OpenMetrics exemplars follow the split: the worst-trace map is keyed on (type, severity, service), so each service's series links to a trace of its own instead of another service's. They now expire 15 minutes after the batch that recorded them, so a service that goes quiet stops pointing its series at a trace the tracing backend has already dropped. `[daemon] per_service_labels = false` restores the pre-0.18 shape (the label stays declared but empty on every series, which PromQL treats as absent). On Kubernetes the chart's `serviceMonitor.honorLabels: true` (chart 0.17.1) is what keeps these labels over the operator's target relabeling, same as `service_io_ops_total` before them.

- A span carrying no service name resolves to `"unknown"` on every ingestion path, settled once in `sanitize_span_event`. The Zipkin and Jaeger readers left the field empty where OTLP already fell back, so the same anonymous trace produced a blank component in its ack signature and, since this release labels metrics by service, an empty `service` label, which an honor-labels scrape drops (a series with no service at all) and a plain scrape overwrites with its own target name. `Finding.service`, the report payloads, the query API and the metric label now agree on one value. An acknowledgment recorded against a Zipkin or Jaeger finding with a blank service must be re-taken, and a persisted report baseline (`diff`, `report --before`) built from such spans must be re-captured, since the diff identity is keyed on the service. The `unmatched_acknowledgment` warning now names the successor when one exists: a current finding sharing the detector and the template hash under a different service or endpoint means the attribution moved, not the query, so the message stops reading as "the problem looks fixed and the entry can be removed" on a signature that only changed with the upgrade.
- The service meters share one admission policy (`CappedServices`), and `_other` is reserved everywhere: a real service named `_other` merges into the fold bucket without taking a cap slot. `GreenSummary` gains an in-process `avoidable_per_service` field (`serde(skip)`, like `accounted_io_ops`), the per-service split of the deduped avoidable ops, as a `BTreeMap` so cap admission does not depend on hash iteration order. Ordering at the source also drops the daemon's re-collect, and the ordered map does not cost anything on the way in: over a synthetic corpus the scoring pass measures no slower with a `BTreeMap` at 16 distinct services or at 128, the analysis cap, where its probe is deepest, and two runs out of three put it a few percent faster, a probe over a bounded set of service names costing less than hashing each one. Run-to-run variance is of the same order as that gain, so `bench_score_green_findings` is there to catch a real regression later rather than to claim a win.
- `disclose` credits per-application `avoidable_io_ops`, hence `efficiency_score` and the G1 `estimated_waste_*`, to the services whose spans a shared finding contains, the finding's service one less for the necessary call, on windows archived by 0.18.0 or later. Older archives carry no split and keep first-span attribution. `occurrences` and `anti_patterns_detected_count` stay on the owning service, so the global count is unchanged, and a service can show waste with zero occurrences for a pattern it took part in under another service's finding.

### Added

- `[detection] sanitizer_aware_min_cv`, the coefficient-of-variation threshold the sanitizer heuristic compares per-span durations against, default `0.5`, the value it hard-wired until now. The simulation lab supplied the case the design note had been waiting for: under `"strict"`, ten identical Doctrine lookups served from cache on a PHP-FPM worker measured a CV near 0.75 once the runner was loaded, which crossed the bar, reclassified a `redundant_sql` finding as `n_plus_one_sql`, and attached a `leftJoin` hint to a query that needs memoizing. None of the four modes separates that case from a real N+1, `strict` being the only one that separates them at all and only while the variance stays under the bar, so the threshold had to move rather than the mode. Validated to a finite `(0, 10]`. Recorded in `detection_config`, and a report written before the knob reads back as `0.5`, the value its run used. The same threshold feeds the HTTP heuristic, which reads a repeated outbound call with few distinct parameters as `n_plus_one_http` or `redundant_http` by timing spread alone, so raising it moves both verdicts.

- `perf_sentinel_service_avoidable_io_ops_total{service}`,
 the per-service share of `perf_sentinel_avoidable_io_ops`, derived in the same scoring pass and with the same dedup rule ((`trace_id`, template, `source_endpoint`), max) so the two cannot diverge. A finding whose spans come from more than one service is split between them, each service charged its own repeats and the finding's service one less for the necessary call, carried by a new optional `pattern.occurrences_by_service` field on findings, present only when it applies. The credit costs nothing visible on the scoring pass: against the commit before it, both the single-service synthetic corpus and `bench_score_green_findings_mixed`, where every group is shared by two services, stay inside run-to-run noise at 16 and at 128 services (from minus 1.3 to plus 0.4 percent, where the same binary drifts by up to 1.9 percent between two runs). The sum across services, `_other` included, equals the global counter as long as green scoring is on, and the counter is silent when it is off, matching the global one. Its denominator ships beside it: `perf_sentinel_service_analyzed_io_ops_total{service}`, the per-service I/O ops of analysed traces, same pass, same 128-service cap, same green gating, so the per-service waste ratio the dashboard's `I/O waste ratio` panel plots never mixes populations (the ingest-side `service_io_ops_total` counts before analysis, drops rather than folds past its own 1024 cap, and includes batches later shed). Both ignore `per_service_labels`: per-service is their only shape. Two overflow counters keep the caps observable, `perf_sentinel_analysis_service_overflow_total` and `perf_sentinel_slow_duration_service_overflow_total`, both counting attributions folded into `_other`, not data lost. Neither ships as an alert, for the same reason `PerfSentinelServiceCardinalityOverflow` was removed in 0.17.0.
...
- `PerfSentinelArchiveDropping` in the chart's `PrometheusRule`, on `rate(perf_sentinel_archive_windows_dropped_total[15m]) > 0`. `docs/METRICS.md` had carried the hint since the counter shipped and nothing acted on it. A dropped disclosure window leaves the archive hash chain contiguous, so `hash-bake` stamps and `verify-hash` recomputes a `content_hash` that validates cleanly over a record that is short, and this counter with its paired warn log are the only witnesses. It carries no rate floor, unlike the two rules that gained one in the same pass, because the unit is a whole scoring window's `Report` rather than one request, so losing a single one is material. Inert on an install without `[daemon.archive]`, where every `ArchiveDropReason` child is pre-warmed to zero and never advances.
- An `Alerting` section in `docs/METRICS.md` and its French mirror, carrying the five shipped rules as a plain `groups:` block. `PrometheusRule` is a Prometheus Operator CRD, so until now an operator outside Kubernetes could read every metric definition in that file and still had nowhere to get the expressions. It also documents the one rule deliberately left off, ingest arriving while zero events come out, which is the failure where every shipped rule reads green: instrumentation that strips `db.statement` or `http.url` converts every request into zero events while still returning success. It stays a snippet rather than a default because a fleet legitimately emitting no I/O spans would fire it truthfully on install day, and a rule that is wrong on day one gets muted forever.
...
- **The chart's `PrometheusRule` drops from seven always-on alerts to five, and three survivors change shape. Breaking for anyone running `prometheusRule.enabled: true`.** The shipped set alerted on conditions that were routine, already predicted by a second rule, or already a panel on the dashboard the same repo ships, and two of them carried `severity: info`, which routes to nobody. What remains fires only on data the daemon lost irrecoverably.
- `PerfSentinelDown` becomes `up{job="<release fullname>"} == 0`, held 15 minutes, at `severity: warning` rather than `critical`. `absent(perf_sentinel_active_traces)` is aggregation-blind and carries no labels: on the sharded topology this repo documents in `examples/docker-compose-sharded.yml`, killing one of two replicas left the survivor publishing the metric, `absent()` stayed empty, and the alert was silent while half the spans went into a hole. It was equally silent when one Prometheus scraped a staging and a production install, since either one alone kept it false. `up == 0` is generated per target and carries `pod` and `namespace`, so it says which install died, and the dashboard's own `Daemon health` panel had been using that idiom all along. At the chart's 15 second scrape interval the 15 minute hold is 60 consecutive failures, past any rolling update, whose `maxSurge: 1` / `maxUnavailable: 0` strategy retires the old target from service discovery rather than letting it report down. The severity drops because the daemon sits in no request path, so nothing done at 03:00 recovers spans already lost.
- `PerfSentinelIngestRejecting` narrows to `reason="channel_full"` with a `> 0.05` floor over `[10m]`, held 15 minutes. It matched `reason!="memory_pressure"`, which swept in `parse_error` and `unsupported_media_type`, raised by any malformed HTTP request reaching `:4318`, a port whose declared threat model is an unauthenticated cluster network. Nothing the daemon meant to analyse was lost in either case and the fix lives in another team's exporter, so both belong on the `OTLP span intake` panel. A single increment held `rate([5m])` non-zero for five minutes, so the shipped `> 0` with `for: 10m` was satisfied by roughly three rejected requests spread across ten, around 0.005 per second. The floor now needs about 45 rejected requests before anyone is told, and burst immunity comes from the 15 minute hold exceeding the 10 minute rate window, so a collector-restart flush of any size decays before it can fire.
- `PerfSentinelAnalysisShedding` gains a floor of one trace per second over `[10m]`, held 15 minutes, so it fires past roughly 900 traces whose findings will never exist. At `> 0` it fired on about three shed traces in ten minutes, a 0.01 percent loss rate on a daemon doing 50 traces per second, and a rule that fires at 0.01 percent teaches its audience to ignore it. The metric stays `analysis_shed_traces_total` rather than `shed_batches_total`, a shed batch holding anywhere from one trace to a thousand.
...
- `PerfSentinelAnalysisQueueSaturated` from the chart's `PrometheusRule`. It predicted `PerfSentinelAnalysisShedding`, said so in its own description, and shares that alert's remedy, so one incident produced two notifications of which the first carried nothing actionable the second did not. It is also already the `Runtime headroom and shedding` panel under the identical division. Worse as a rule than as a panel: `analysis_queue_depth` is a live gauge read once per 15 second scrape, so a daemon running healthily at sustained throughput sits above 80 percent indefinitely while dropping nothing, and `analysis_queue_capacity` is set once at startup, so any window where depth is non-zero before capacity lands evaluates to `+Inf` and fires at once.
- `PerfSentinelCorrelatorEvicting`. It carried `severity: info`, which routes to nobody, and it is inert unless `[daemon.correlation]` is enabled, at which point eviction at `max_tracked_pairs` is the documented steady state rather than a fault. The tuning advisor already names that knob inside the `/api/export/report` the operator is reading, the daemon logs a warn the first time, and the count it fired on is deliberately approximate above 8192 distinct pairs per batch, which is exactly the wide topology where it would fire.
- `PerfSentinelServiceCardinalityOverflow`. It carried `severity: info` and its own description ended `Expected on very wide topologies`, so it shipped pre-labelled as a rule that fires on a routine state. `MAX_SERVICE_CARDINALITY` is a compile-time constant, so it named no knob an operator could turn, and what overflows is per-service attribution granularity on a bounded label set: every trace is still correlated, every detector still runs, every finding is still produced and `total_io_ops` stays correct. The cap firing means the cardinality guard is doing its job.

### Fixed

- The cross-trace correlator no longer OOM-kills a memory-limited daemon on a wide topology. `ingest` collected every pair it refused at the `max_tracked_pairs` cap into a `HashSet` whose only outputs are a length and a boolean, and that set is sized by the batch's findings times the occurrences inside `lag_threshold_ms`, never by the cap. Measured in the simulation lab at 1500 services and 80 traces per second against the daemon's own 15 second batch cadence, that is roughly 1.6 million distinct keys per batch, a 34 MiB table and 51 MiB live across the doubling rehash, inside a 256 MiB container. Lowering the cap tenfold made the peak worse rather than better, because a refused pair is by definition one the cap did not admit, so the cap can remove at most its own size from a set of millions. Refusals are now deduplicated up to 8192 distinct pairs per batch and counted past that, which holds the structure near 140 KiB while `perf_sentinel_correlator_pairs_evicted_total` stays exact for every topology small enough for an exact number to tell an operator anything.
- `docs/CONFIGURATION.md` no longer describes `trace_ttl_ms` as a flat deadline. A trace goes stale once no span has arrived for that long, but the sweep that evicts and analyses it runs on a ticker at half the TTL, so the effective deadline is the TTL plus up to one tick, and a span arriving in that gap refreshes the stale trace instead of opening a new one. Measured on an isolated daemon: two identical emissions 33 s apart against a 30 s TTL merged into one generation of 20 traces analysed once, where 50 s apart produced the two the old wording implies. The tick appeared in the docs only in passing, in `docs/QUERY-API.md` and about a different subject.
- `docs/CONFIGURATION.md` no longer claims `max_tracked_pairs` prevents unbounded memory growth. It bounds what the correlator retains, not what one batch walks, which is the distinction the failure above turned on. A doc comment crediting `Arc` interning to `intern_endpoint` is gone with it: no such function exists anywhere in the tree, and `ingest` allocates a fresh `Arc` per finding.
- The dashboard's `--brand-text` moves from `#11845d` to `#107a56` in the light theme. The brand green fell to 4.09 against `--accent-bg`, 4.12 on `--surface-3` and 4.37 on `--surface`, under the 4.5 WCAG AA asks of text that size, which the active nav item and every brand-coloured link use. The new value's worst case is 4.65 and the perceived shift is one step of lightness on the same green.
- The dashboard's `--text-3` moves from `#7d8c83` to `#95a199` in the dark theme. It fell to 3.97 against `--surface-2` and 3.50 against `--surface-3`, under the 4.5 WCAG AA asks of text that size, which every overline, gloss and metric label uses. The light theme already carried a documented AA floor for the same token. This is the other half of that pass, and the dark block now carries the same note. Measured on the rendered dashboard with translucent layers composited, not on the token pairs alone.
- `--sort` produced a wrong order on any input whose sort permutation holds a cycle of three or more: the decorate-and-permute rewrite applied the inverse permutation, so `--sort impact` could open on the LOWEST-impact findings and hand the trace embed the wrong rows. Caught one commit later by review, the pinning test asserted only the first element, which an inverse rotation also satisfies. The application now inverts into the destination convention first, and two tests pin the full order, one of them a three-cycle that fails on exactly this mistake.
- The trace-selection rule lives once, `first_reference_rank` in `report::embedded`, shared by the embed budget and both of the HTML sink's selection paths instead of three hand-rolled copies.

- One trace-selection rule on every render path. A report rendered from raw traces picked its embedded trees by per-endpoint IIS, which silently degenerated to input order whenever `[green]` was disabled, and ignored `--sort` entirely, so the page could claim trees follow the ranking while they followed nothing. Both paths now rank each candidate trace by the first finding that references it, and `report` forwards the chosen sort key to the dashboard via `initial_sort` in the payload, so `report --sort severity` opens the page on severity rather than stranding its top rows without their trees.
- The embed moved into the shared emit seam, after the sort: budget-trimmed before sorting, it kept the detector-order head and could strand the top rows of the sorted list. Only the JSON sink pays it, and `analyze` deliberately does not embed at all: its input is a local file the user still holds and `report --input <that file>` draws the trees from it directly, where the backend queries' JSON is the only carrier of the spans they fetched. Embedding in `analyze` would also have fattened every `--ci` pipeline's stdout on a version bump, for output nothing in the documented CI flow reads. The embed's safety ceiling is 192 MiB, under the tightest known consumer cap (a 256 MiB stdout reader), keeps the traces of the top-ranked findings, measures through a counting writer rather than throwaway strings, and logs the drop.
- The body-overrun error names the remedy of the path that failed: a search overrun says `lower --max-traces`, a single trace past the per-trace cap says the flag cannot shrink it. `jaeger-query` gains the same classified error for its 256 MiB cap, which still surfaced as the generic `failed to read response body: length limit exceeded` the Tempo fix had just retired, and the `LengthLimitError` walker now lives once in `http_client` instead of twice.
- The stderr trim notice distinguishes the operator's own `--max-traces-embedded` from the size budget instead of prescribing the flag that caused the trim, and `--max-traces` on `tempo` and `jaeger-query` reads its shared 1..=10000 ceiling from one constant instead of one constant and one magic literal.
...
- `report` accepts `--sort`, the key the other three subcommands already took. It is what makes the line above actionable: `report --input x.json --sort impact --max-traces-embedded 50` keeps the fifty trees behind the fifty highest-impact findings. Measured on a mixed corpus, the five embedded trees move from five low-impact N+1 SQL traces to the five N+1 HTTP traces that carry eight times the avoidable I/O.
- The two trim messages no longer name a size budget that an explicit `--max-traces-embedded` has switched off. The page cannot tell a cap from a budget, and naming the wrong one is what the four-way message split exists to avoid.

- `tempo --max-traces` is bounded to 1..=10000, the range `jaeger-query` already enforced. It was a bare `usize`, so `--max-traces 999999999` parsed, reached the backend, and failed somewhere downstream on a limit nobody had named.
- The search-response cap on the Tempo path goes from 1 MiB to 16 MiB. It was sized for the `limit=500` its own comment cites and then applied to every value, so a legitimate `--max-traces 5000` died on a limit of this client's own making. Measured: a realistic Tempo summary, carrying the root service, name, duration and span set, runs about 210 bytes, which puts the old cap's ceiling near 4700 traces.
- A response body that outgrows one of those caps is now told apart from a transfer that broke, and says whose limit it hit: `response body exceeded the N byte cap perf-sentinel applies to it, which is a limit of this client and not of the backend, lower --max-traces`. It previously surfaced as `failed to read response body: length limit exceeded`, which reads as a network or backend fault and sent operators to check a healthy Tempo.
...
- The two trim messages no longer prescribe a flag as the only way out. The one on a finding names the figures it has, `526 of 2000 span trees are in this report, kept for the findings ranked highest`, then points at the trace id sitting one line above it with its copy button: rerunning on that id returns the one tree the reader is looking at, where a narrower window means guessing which one contains it and running the whole sweep again. The banner says the same, both silent on which cap cut because the page cannot tell an explicit `--max-traces-embedded` from the size budget, and naming the wrong one is the defect the rewrite removes.
- The dashboard's four "no span tree" messages render their flags, config keys and commands as code chips rather than as bare prose, the way suggestions and templates already do, and the chips stay on one line inside the detail pane.

## [0.16.0] - 2026-08-28

### Added

- `tempo` and `jaeger-query` accept an absolute search window, `--from` and `--to` in ISO 8601 UTC, as an alternative to the relative `--lookback`. The two forms are mutually exclusive, the pair must be given together, and neither joins `--trace-id`, all three enforced by the argument parser rather than silently ignored: a trace ID resolves to exactly one trace, so the search path that reads a window is never entered. A relative window is resolved at the moment the request is issued, which is fine interactively and useless the moment a caller queues the request before sending it, or wants to re-read the exact hour an incident happened in three days ago. Both flags parse through `crate::time`, the single source of civil-calendar arithmetic, and an empty or inverted window is refused at the flag rather than at the backend, which would answer it with an empty result set that reads as "nothing happened".
...
- `jaeger-query --service` no longer searches from the Unix epoch. It sent the window as `lookback`, which Victoria Traces reads only on its service-graph endpoint and never on `/api/traces`, so the parameter was dropped and the search ran against that handler's own defaults, `StartTimeMin: time.Unix(0, 0)` to now, bounded by nothing but `--max-traces`. Every relative query against Victoria Traces returned whatever the store handed back over all of history rather than the requested window. Both window forms now send explicit `start` and `end`, which Victoria Traces parses with `time.UnixMicro` and Jaeger with `time.Microsecond`.
...
- **Breaking, `perf-sentinel-core` only.** `ingest_from_tempo`, `ingest_from_tempo_with_grouping`, `search_traces`, `ingest_from_jaeger_query`, `ingest_from_jaeger_query_with_grouping` and `search_and_fetch_traces` take a `SearchWindow` where they took a `Duration`. Callers passing a lookback wrap it: `SearchWindow::Lookback(duration)`. The new type carries the relative and absolute forms as two arms because they are two different things to ask for, not because the wire differs: both resolve to explicit bounds in epoch milliseconds, which each backend then scales to the unit its own API counts in, whole seconds for Tempo and microseconds for the Jaeger query API. Milliseconds rather than seconds so a sub-second window survives to the backend that can use it. The CLI surface is unaffected.

## [0.15.0] - 2026-08-27

### Added

- The diff pairs template mutations instead of reporting them as one resolved plus one new finding: leftovers sharing the same detector, service, endpoint and grouping (arbitrated by the `code.*` anchor when several candidates remain) land in `mutated_findings`, counted neither as new nor as resolved, so a schema refactor no longer fails a CI gate fed by `new_findings`. The pairing never guesses: anything ambiguous stays reported as resolved plus new.
- An `unmatched_acknowledgment` warning now names the candidate successor when exactly one current finding shares the acked signature's detector, service and endpoint with a different template hash: the template drifted rather than the problem being fixed. The ack is never transferred automatically, a signature is a suppression boundary.
- `pg-stat` and `mysql-stat` cross-reference statements against every SQL template observed in the traces instead of only the templates that produced a finding, so a healthy traced query finally gets its `[seen in traces]` marker, and both report the matched share (`trace_match`: templates and calls-weighted percentage, also shown on the dashboard panels). Both surfaces spell the figure out rather than labelling it, because the share of statements and the share of calls are two different numbers and a bare "trace-matched: 1/15 templates, 15.4% of calls" gives a reader nothing to anchor either to. Never called "coverage" either, and the reason now ships next to the number instead of living in a code comment: the database counters are cumulative since the last statistics reset while the traces cover one capture window, so the figure is a floor on how much is traced, not a sampling rate.
- `pg-stat --baseline <earlier-export>` (with `--traces`) compares the call delta between two `pg_stat_statements` snapshots framing the trace window to the traced span counts on the same templates, and reports an empirical tracing coverage (`trace_coverage`). Counter resets between the snapshots are detected, skipped, and surfaced as a warning.
- Each daemon archive line now carries the cumulative `archive_windows_dropped_total` value at write time, and the periodic disclosure schema moves to `perf-sentinel-report/v1.7`: `integrity.trace_integrity_chain` gains `windows_dropped` (windows produced but not archived over the period, derived from the counter deltas) and `drop_counter_resets` (daemon restarts inside the period, each making the figure a lower bound). Both absent on archives written before the counter, so an old archive never reads as zero drops. Re-hashing a v1.6 report on this binary keeps its `content_hash`.
- `perf_sentinel_archive_windows_dropped_total` counts the per-window disclosure archive entries dropped instead of written, by `reason` (`channel_full`, `writer_exited`, `serialize_error`, `write_error`). The drop-on-full policy is deliberate, a stalled filesystem must never block the analysis path, but until now every loss was a warn log and nothing else: the archive hash chain only advances on a successful write, so a dropped window leaves no hole in `seq` and `disclose verify-hash` cannot see the gap after the fact. This counter is the scrape-visible witness, pre-warmed to zero for all four reasons so a healthy archive and a missing metric stay distinguishable. Two blind spots remain that no counter can close: a writer task that dies discards its queued backlog uncounted, and a hard kill before the shutdown drain finishes does the same. Alert on `rate(...) > 0` and read a nonzero rate as "the archive under-reports the period, at least by this much".

### Fixed

- The disclosure archive can no longer be truncated to nothing by a rotation that happens while the daemon is writing. The writer tracks how many bytes it has appended so it can stop before the configured cap, and it resynced that count by stat-ing the archive **path**. A rotation moves the file out from under the open descriptor, so the path then names a file that does not exist yet and stats as zero bytes, after which the writer believed it had room it did not have and a later `set_len` cut the archive back to empty. It reads the count from the descriptor it actually holds now, which follows the file through a rename, and keeps the previous count when the stat itself fails rather than assuming zero, since the previous count is the offset of the last complete window. The two drop paths also name `perf_sentinel_archive_windows_dropped_total` in their warning, so a log line points at the counter that measures how much was lost.
...
- 65 Scaleway instance types get an embedded power profile, and `[green.cloud]` accepts `provider = "scaleway"`. Cloud Carbon Footprint publishes coefficients for the three hyperscalers only, which is why this table had held nothing else, but the blocker turned out to be narrower than "no data exists": Scaleway publishes no wattage, yet its Product Catalog API names the **exact CPU** of every offer without authentication, `AMD EPYC 7543` rather than a generic family, alongside the vCPU count and whether they are shared. Each offer is therefore priced by the CCF coefficient for that CPU's architecture, the same vCPU-times-coefficient arithmetic the AWS, GCP and Azure rows already use, adding one assumption rather than a measurement: that a coefficient computed on hyperscaler fleets transfers to comparable silicon elsewhere, which is what CCF already assumes between AWS and GCP. 35 POP2 rows come from the AWS CSV (EPYC 3rd Gen), and the 30 COMPUTE3, MEMORY3 and STANDARD3 rows ride the same SPECpower-direct figure as GCP `c4d`, being the same EPYC 9005 generation, since the CCF EPYC 5th Gen row reads 3.68 idle watts per vCPU against 0.74 for the neighbouring generation, the upstream error the `m8a` family already works around. Three groups are excluded rather than approximated, each named in the docs: shared-vCPU ranges (`DEV1`, `PLAY2`, `BASIC2`, `BASIC3`), where attributing a whole vCPU to one tenant overstates what it draws; GPU offers (`H100`, `L4`, `L40S`, `RENDER`), because the table models no accelerator for any provider and one H100 outweighs the entire CPU budget; and the AmpereOne and Granite Rapids ranges, absent from the CSVs. An excluded type still behaves as any unlisted type does, warning once and falling back to the provider average. The offer list is read live at refresh time instead of pinned by hand, so a new Scaleway size lands in the table on the next `refresh-datasets` run, and only the CPU-to-architecture mapping is hand-maintained. OVHcloud and OUTSCALE stay out: OVHcloud documents no CPU per range and the only third-party attribution, Boavizta's, is flagged `CPU not verified` by Boavizta itself on all 12 of its OVHcloud rows, one of which credits a two-socket rack server with a mobile `Intel Core i7-4940MX`; OUTSCALE stops at (Region, service category) and appears nowhere in Boavizta.

- OVHcloud, Scaleway and 3DS OUTSCALE regions carry grid intensity and PUE, next to AWS, GCP and Azure. Until now a workload on any of the three landed in the synthetic `unknown` bucket and contributed **zero operational CO2**: not an absent figure, a wrong one, on three European sovereign clouds. 42 keys join the table, and 36 of them are not hand-entered, they come from the same Ember refresh by ISO3 country code that already feeds the other providers, so the script stays the single source. The remaining 6 sit on subnational grids the country average would misprice (Quebec hydro, PJM, Oregon, CAISO) and join the existing hand-kept rows in `MANUAL_CARBON_ROWS`, exactly as the AWS and Azure US regions already do. OVHcloud names one datacenter three ways depending on which API answers (`GRA11` for the OpenStack Public Cloud region, `GRA` for the zone code, `gra` for the S3 location string) and all three are keyed, since which one reaches `cloud.region` depends on the SDK the caller instruments. OUTSCALE keys carry an `outscale-` prefix, because OUTSCALE reuses AWS region identifiers for different places: its `eu-west-2` is Paris where the AWS one is London, 42 against 221 gCO2/kWh on the same string, so an unprefixed row would have silently scored a French deployment on the British grid. That deployment declares the prefixed key in `[green] default_region`. PUE is per provider as it already was for the other three: 1.24 for OVHcloud (FY25 group average, measured over twelve months and audited by Apave against ISO 30134-2) and 1.375 for Scaleway (2024 average over ten datacenters). OUTSCALE publishes none, and rents capacity rather than operating its own datacenters, so its rows ride the generic prior rather than a number invented for them; the 1.2 its partner advertises is a design figure for one site, and the 1.64 that circulates traces back to a 2021 infographic that does not state it. Each region also gets its own hourly-profile alias, without which it would have resolved to the flat annual value while looking supported. The datacenter-level PUE both OVHcloud and Scaleway publish is deliberately not modeled: an 8% spread around a group average sits far inside the 2x bracket the whole estimate carries. The SPECpower path (`[green.cloud]`) gains `scaleway` (see above) but still refuses OVHcloud and OUTSCALE, since neither documents the CPU an offer runs on and a fallback would be a fabricated wattage; those machines stay covered by Alumet or Scaphandre.

- The interactive TUI catches up with the dashboard on exploring findings. An audit of the two surfaces found that the terminal browser had quietly fallen behind: the dashboard gained sorting, filtering and per-finding statistics across several releases while `inspect` kept the shape it shipped with, and no CHANGELOG entry ever claimed a terminal port for them, where recurrence (0.10.0) and grouping (0.11.0) explicitly did. The Detail panel now shows the per-span timing (`p50`, `p99`, `CV`), how an n+1 was classified, the observation window and the finding's confidence, all announced in 0.11.0 and 0.5.4 and all HTML-only until now; the first two also reach the text report, which was missing them as well. `s` sorts the findings inside the selected trace, not just the trace list, on the comparator `analyze --sort` and the dashboard already share, and `query inspect` accepts `--sort` where it had no ordering control at all. `f` cycles the Findings panel through the severities, the family the dashboard puts on pills. The Analyze view leads with the report's warnings, which say what the figures below them do not cover and which every other surface has always printed, and it ranks the worst findings by impact next to the top offenders it already listed, two cards the Overview has had since 0.9.10. The operator monitor (`query monitor`) is a separate surface with a separate audience and is deliberately untouched. `g m` opens the `mysql_stat` tab on the dashboard, the only tab left without a shortcut since it shipped in 0.9.6.

### Changed

- The HTML dashboard's severity pills combine instead of replacing each other, and the `All` pill is gone. They were the last family still behaving as a radiogroup: a click on `warnings` dropped the `critical` selection, so "show me everything but the infos" was not expressible, while the three other families had accepted several values at once since the toolbar was reorganised. They are toggles now, OR'd like the rest, and no pill pressed already means no restriction, so nothing has to be pressed to see everything. A `Clear filters` button closes the row instead, since `All` had been doing two jobs and only one of them survives multi-select: it emptied the whole toolbar, severities, services, types and grouping together, and dropping it left a pointer user un-pressing every pill and unticking every box one at a time. It sits at the end of the row, shows up only once something is filtered, and carries no fill or border so it never reads as one more pill waiting to be pressed. `Escape` is its keyboard equivalent, as before. The Overview severity cards now set the row rather than adding to it, so the list they open still holds the number printed on the card. The URL carries the selection as a list (`severity=critical,info`), on the same encoding as the other families, and a link written before this change with a single value restores as it did. A severity naming no finding is dropped on restore rather than applied, matching how the service, type and grouping values already behave, since applying it would open an empty list with no pressed pill to explain it.

- The Overview's `view all` and `open diff` buttons drop their underline. They are pills with a border, a fill and a hover state, and they carry a trailing arrow, so the underline was a third affordance stacked on two that already read as clickable, and it borrowed a hyperlink's signal for a control that navigates a tab rather than a document. The arrow stays.
...
- `[daemon] max_export_findings` sizes the `/api/export/report` slice, and `watch --max-export-findings` overrides it for one run. `/api/config` and the Config tab of `query monitor` report the effective value, since the CLI override means the file is not authoritative. The export was pinned to `MAX_FINDINGS_LIMIT`, the constant bounding `/api/findings`, so a daemon retaining tens of thousands of findings shipped 1000 of them with no way to widen the snapshot: the two caps answer different questions, one paginates a browsing API where the other sizes a deliberate export, and sharing a constant meant widening every page of the first to widen the second. The default is the previous hardcoded value, so an operator who sets nothing sees the snapshot they saw before. The ceiling is 100,000 rather than the store's 10,000,000: a finding serializes to a few KB, so the knob bounds a response body and the HTML rendered from it, not retention, and the two run out at very different orders of magnitude. Above roughly 2000 the snapshot outgrows the 8 MiB body limit `query inspect` and `query monitor` fetch it with, so the daemon logs an advisory there rather than only at the hard ceiling. `0` exports the envelope alone, and because the gate counts findings from the exported slice its three finding-count rules then pass whatever the daemon detected: an advisory names that too, since a probe polling that shape reads half a verdict. The fourth rule, `io_waste_ratio_max`, reads `green_summary`, which no cap empties, so the verdict itself does not become a constant, it stops reflecting the findings. The CLI override goes through the same validation as the file, so a typo past the ceiling is refused at startup rather than at the first request.

### Changed

- The HTML dashboard's two crowded bars are reorganised. The findings toolbar listed every filter as a flat pill, 26 of them across two wrapping rows on the demo dataset, mixing three families that do not combine the same way and giving no hint of which was which. Service, type and the grouping attribute each collapse into one menu now, ordered `Type`, `Service`, then the grouping attribute, named after its own key when the report has a single one, and the collapsed trigger keeps the pill's look so the row reads as it did. The severity row and its count are untouched, deliberately: they were already the one part that scanned well. Those three families also accept several values at once, where a click used to replace the previous one, OR inside a menu and AND between menus, with the count on the trigger (`Type · 2`) and both carried in the URL so `Copy link` still reproduces the view. A value the report no longer holds is dropped on restore rather than applied, as before. Sort moves out of that row onto the one above the findings column it orders, since it ranks rather than narrows, and `Export CSV` and `Copy link` join it at the far end instead of taking a row of their own. In the top bar the search box is centred rather than trailing the actions, the density and theme buttons fold behind one gear so the bar carries report state rather than preferences, and the `HTML REPORT` line under the logo is gone, the page having never been anything else. The gear and the filter menus are native `<details>`, and they take their own tier in the Escape ladder: without it, closing a menu with Escape fell through to the tier that clears the filters and threw away a selection the user only meant to stop editing.

### Fixed

- A snapshot too large for the query clients says so instead of reading as an unreachable daemon, and the two knobs that size it are advised on together. `max_export_findings` and `max_retained_traces` fill the same response body, but each was comfort-checked alone, so a pair sitting inside both zones (2000 findings, 400 traces) projected roughly 11 MiB against the 8 MiB `query inspect` and `query monitor` fetch `/api/export/report` with. Past that limit `fetch_json` reduced the read to `None`, indistinguishable from a daemon that never answered, and the operator went looking at the network for a configuration problem. Startup now warns on the projected total, naming both knobs, from per-item weights measured on a production daemon, with the span-tree term clamped at the byte budget `snapshot_for` already enforces so the advisory never blames a knob that cannot overflow on its own. The overrun is no longer flattened either: `FetchError::BodyTooLarge` separates it from a broken transfer, since it is the one body failure whose fix is a setting on the peer, and `query monitor` prints the reason beside its `[STALE]` marker rather than the bare marker. The energy scrapers map the new variant onto the body-read outcome they already report, keeping their Prometheus label sets bounded. Config validation carries its own copy of the limit, because it compiles without the features gating `http_client`, with a test that fails if the two drift.

- Long SQL statements wrap in the `pg_stat` and `mysql_stat` tables instead of scrolling their card sideways. The template cell carried a `ps-pg-template` class that no rule ever defined, so the column sized to its content: a statement is routinely past 100 characters and often a comma-separated column list with nowhere to break, which set the table's minimum width and pushed the figures out of view behind a horizontal scrollbar. Measured on a production `pg_stat_statements` export, a 760px card overflowed by 416px on the PostgreSQL table and 798px on the MySQL one, both now at zero. `overflow-wrap: anywhere` rather than `break-word`, because only `anywhere` shrinks the cell's intrinsic minimum width, which is the property that stops the overflow rather than merely wrapping once the column is already too wide. The breaks land after commas, where `addCommaBreaks` now marks the cell as it already did for the Explain code block, and fall back to mid-token only on a list written without spaces. Both tables label that cell with the same class, so the two tabs are fixed by the one rule.

- `/api/export/report` states what its snapshot covers, in `warning_details` under the `snapshot_scope` kind. Two populations coexist in that payload and nothing said so: `findings` is capped at `[daemon] max_export_findings`, the most recent ones, and `green_summary` is not an aggregate over them but the latest per-batch summary the event loop wrote. On a daemon retaining 46 000 findings the export carried 2% of the store, covering the last few minutes rather than the daemon's uptime, next to carbon totals describing a single batch, and a reader took the two for one picture of the whole deployment. The absolute green numbers (`total_io_ops`, `co2`, `energy_kwh`) are wrong by orders of magnitude read that way, while the ratios stay representative, so the fix names the reach rather than recomputing: the findings store keeps findings, not the events behind them, and `total_io_ops` counts operations no finding points at, so a summary rebuilt over the exported slice would be a different wrong number rather than the right one. The HTML dashboard already renders `warning_details` in a banner on every tab, so a snapshot fed to `perf-sentinel report` carries the caveat with it. The truncation line is omitted when the store fits under the cap, since claiming one would be noise. Batch output is unchanged and carries neither warning: there every number comes from the same pass over the input.

## [0.13.0] - 2026-08-14

### Fixed

- The TOML acknowledgments file is re-read every minute instead of only at startup, and a symlink pointing inside its own directory is no longer refused. The two go together, and both surfaced from the same deployment: shipping that file as a Kubernetes ConfigMap, which is the obvious way to hand a daemon a text file reviewed in a PR. A mounted ConfigMap is projected through a `..data/` indirection, so the configured path *is* a symlink and the daemon refused to start at all. Following it grants no reach the caller did not already have by naming that directory, so a link resolving under its own directory is now accepted; one resolving anywhere else stays refused, which is the case the check was written for. And once it loads, an edit has to apply: telling an operator to restart a daemon so it notices a text file, when that file is the sanctioned way to record a team decision, is not an answer. The map moves behind an `ArcSwap`, matching the energy cache already in the tree, so reads stay lock-free and a query in flight keeps a coherent view rather than one spanning two revisions. A failed re-read keeps the previous acks and logs a warning, and so does a file that has disappeared, since an unmounted volume or a deleted `ConfigMap` must not un-acknowledge what the team decided to accept. That warning is latched, so a volume that stays unmounted logs once rather than once a minute forever. A configured `toml_path` that is not there at startup now warns too, naming the path, instead of the line that advises setting the option the operator already set. `[daemon.ack] enabled = false` still holds no acks at all: it stops the poll rather than letting it put them back a minute later. Only an explicitly configured `[daemon.ack] toml_path` is polled, never the CWD-relative default, and the interval matches the kubelet's ConfigMap sync period since polling faster would only re-read a file the platform has not refreshed. A path with no directory component (the CWD-relative default, `--acknowledgments acks.toml`) resolves against the current directory rather than being refused outright.

### Added

- `report --pg-stat-calls-metric` and `--mysql-stat-calls-metric` fetch the call counter from a series of its own and join it on `queryid` and `digest` respectively, and `--pg-stat-unit` states whether the ranked time series counts seconds or milliseconds. The standalone `pg-stat --calls-metric` / `--unit` and `mysql-stat --calls-metric` carry the same knobs, since they run the same scrape. The scrape used to read `calls` as a *label*, which no exporter publishes, the `postgres_exporter` built-in included: it exposes `pg_stat_statements_calls_total` alongside the time series, as a series. So the count silently fell back to zero on every deployment, which emptied the ranking by calls and collapsed the mean onto the total, and no test caught it because the fixture had no `calls` either. The join is keyed on `queryid` only, never on the statement text, since exporters commonly truncate it and a wrong join is worse than a missing count. Both queries fold on the statement identity before ranking (`sum by (queryid, <query label>)`, and `sum by (digest, schema)` on MySQL), because the exporters label one statement once per database, user or schema: ranking the raw series listed the same statement several times, split its execution time across those rows, and left the call count describing a different set of rows than the time it divides. The call-counter series name goes through the same `PromQL`-metric-name guard as the timing one, since both land unencoded in the query string, `--query-label` goes through the label-name guard now that it lands inside the `by (...)` clause, and the second query is intersected with the ranked set (`and on(queryid)`) so it stays bounded by `--top-n`: pulling the whole counter series would overrun the client's body cap on a large `pg_stat_statements.max` and lose the counts it went to fetch. A failed second query is not fatal: the timings still produce a report and a `WARN` names what the rankings will be missing. The MySQL scrape had the same hole, documented as a limitation when it shipped: `mysqld_exporter` publishes `COUNT_STAR` as `mysql_perf_schema_events_statements_total`, again a series and not a label, so the join closes it there too and the two scrapes now share one guard rather than each carrying its own. The unit knob exists because `pg_stat_statements` counts milliseconds while the exporter built-in converts to seconds, and a hand-written exporter query usually forwards the column untouched, so reading one for the other is off by a factor of a thousand. `--mysql-stat-unit` and `mysql-stat --unit` are its counterpart and add `picoseconds`, the unit Performance Schema counts `SUM_TIMER_WAIT` in, where reading it as seconds is off by a factor of 10^12. The MySQL scrape also joins `SUM_ROWS_SENT` and `SUM_ROWS_EXAMINED`, published as series of their own like `COUNT_STAR`: the ranking by rows examined used to sort an all-zero column and read as a placeholder. `--mysql-stat-rows-sent-metric` and `--mysql-stat-rows-examined-metric` name them, on the same empty-value-opts-out rule as the call counter, so every series the scrape queries is nameable and skippable. The identity those queries fold on keeps `instance` and `job`, so a Prometheus scraping several database servers is not summed into one row that names no server. And since the identity those queries fold on is `digest` plus the schema label, `--mysql-stat-schema-label` names that label, because a recording rule renaming it would merge two schemas into one row with their execution times summed. Defaults keep the same series names, and a statement the exporter reports once per database now occupies one ranked row instead of one per database.
...
- The Hub exporter is drained on shutdown instead of being aborted. Its task was killed in `Drop`, so a SIGTERM discarded every signature still pending plus the request in flight: a rolling upgrade lost each finding discovered since the last flush, silently and with no counter moving, while the archive writer beside it was already drained properly. Shutdown now signals the exporter, which stops waiting for its flush interval and empties the buffer, bounded to 10 seconds. The bound matters as much as the drain: an unreachable Hub must not hold the daemon past the orchestrator's grace period, where SIGKILL follows and nothing is flushed at all. An expired budget logs a `WARN` naming how many findings are lost, and `terminationGracePeriodSeconds` should sit above that budget.
...
- A span the instrumentation marked SERVER no longer becomes an outbound HTTP call. Jaeger, Zipkin and OTLP ingest all built an `HttpOut` event from any span carrying a URL, so a service's own inbound request was read as a call it had made, and a reverse proxy, which does nothing but receive, became the busiest caller in the trace: on the OpenTelemetry demo under load, `frontend-proxy` alone accounted for three of the four largest `redundant_http` groups, under a `source_endpoint` of `unknown` and a template naming its own inbound route. The whole corpus went from 70 distinct `redundant_http` identities to 5, 279 findings to 57, because the duplicates were one hop counted twice, once as the caller's real client span and once as the callee's inbound span. `traces_analyzed` falls with it, since a trace made only of proxy SERVER spans has no analyzable work left once they stop being counted. Spans of unspecified kind stay eligible, so instrumentations that never set `span.kind` are unaffected. This entry is written after the fact: the change shipped in 0.12.0 undocumented, and the simulation lab's replay gate, which pins a finding census against a captured chaos corpus, is what surfaced the omission three releases later.
- Connection and session management statements no longer surface as `n_plus_one_sql` or `redundant_sql`. A pooled driver emits one per connection checkout, so a request borrowing N connections showed N identical statements and tripped the repetition detectors, with a remediation (`WHERE ... IN (?)`, "cache result") that cannot apply to a `SET` or a `COMMIT`. On one production trace, 39 `SELECT set_config(?, ?, false)` outranked the three genuine Hibernate N+1 of the same trace, and the pattern accounted for 10.8% of all findings emitted over a 30-minute window, enough to keep `n_plus_one_sql_critical_max = 0` permanently red on a gate that had nothing actionable behind it. The detectors now skip these statements via `normalize::sql::is_session_command`, which is vendor-neutral: transaction control everywhere (a trailing `;` and the `BEGIN TRANSACTION`/`START TRANSACTION` spellings included), `SET`/`RESET`/`DISCARD`/`set_config()`/`current_setting()` on PostgreSQL, `SET NAMES`/`USE` on MySQL, `ALTER SESSION` on Oracle, `VALUES(?)` on DB2 and H2, and `sp_reset_connection` on SQL Server. Four shapes are deliberately left visible because the keyword alone does not identify them, or because a remediation does apply to them: `SELECT 1`, which normalizes to the same template as a select of any bare literal, Oracle's `BEGIN pkg.proc(?); END;` anonymous block, which is a stored-procedure call and can legitimately be an N+1, `SHOW COLUMNS FROM t`, which is the schema introspection a dynamic-schema ORM runs once per entity and which both batching and caching can fix, and `SELECT @@…`, because `@@IDENTITY` and `@@ROWCOUNT` are emitted once per statement to fetch generated keys rather than once per checkout. The statements keep counting as I/O operations, so they stay in `total_io_ops`, in per-service I/O and in the total carbon figure, since that work does happen. Their operations do leave `avoidable_io_ops`, which is derived from the findings: `io_waste_ratio` and the disclosed avoidable energy both fall on upgrade, and a `[thresholds] io_waste_ratio_max` calibrated on a 0.11.2 run is now looser than intended. Deployments seeing these findings will lose them, which lowers finding counts without any change in application behavior.
- `serialized_calls` no longer counts session commands as links of a parallelizable chain. A `BEGIN` → reads → `COMMIT` sequence under one parent used to report every statement, so the suggestion advised parallelizing transaction control, the one thing that cannot move off the chain. Their durations also inflated the "total sequential" figure the finding advertises as recoverable. Both the link count and that total now cover application work only, so a chain that owed its length to transaction control drops below `serialized_min_sequential` and stops being reported.
...
- The `diff` terminal block sanitizes warning text like the report block does. A baseline JSON is operator input, so an escape sequence could reach the terminal unfiltered.
...
- `/metrics` no longer serves OpenMetrics exemplars to a scraper that did not ask for them. A `*/*` wildcard used to count as an opt-in, so vmagent received an exemplar suffix it cannot parse and read `perf_sentinel_io_waste_ratio 0.60 # {trace_id="..."} 1.0` as a metric *name*, minting one dead series per scrape. A real cluster accumulated 6,216 junk metric names against 41 real ones in a few hours. Exemplars now require an explicit, non-refused `application/openmetrics-text` in `Accept`; every valid zero quality form from `q=0` through `q=0.000` is honored.
- Native `SpanEvent` JSON cannot bypass the configured eight-attribute grouping cap. Oversized arrays are truncated at the shared sanitization boundary before they can multiply retained memory.

### Added

- `[detection] grouping_attributes` chooses which resource or span attributes separate one deployment from another, defaulting to `["k8s.namespace.name", "service.namespace"]`. The first one present on a span decides finding identity, every present one is captured and displayed, and each surface labels the value with the attribute it came from. A shared cluster can group by `tenant.id` with no code change. The configured order now reaches batch JSON, daemon OTLP gRPC and HTTP, Tempo, and Jaeger Query ingestion alike. `SpanEvent` and `Finding` carry the captured attributes. The HTML report can filter exact finding types and the effective `key=value` grouping, and its detail and CSV views expose the captured attributes, classification, observation window, timing statistics and masked evidence. Exact culprit spans are listed individually. Cross-trace findings state how much of the total is present in the representative trace, while older reports that cannot rebuild exact ids keep matching spans grouped.

### Changed

- Recurring findings are grouped by the effective grouping key and value as well as acknowledgment signature, so identical problems in separate deployments remain distinct without changing acknowledgment signatures. Older JSON without grouping fields remains readable. Adding the public fields is an accepted Rust source-compatibility break, so this release is a minor bump rather than a patch. Acknowledgment stays grouping-blind on purpose: grouping answers who is affected and where, an acknowledgment answers whether the code is accepted debt, and that decision does not vary by deployment. One ack therefore covers every grouping, and reordering the list never invalidates one. The acknowledgment dialog names every grouping the acknowledgment will cover, so a row-level Ack never silences a sibling row silently.
...
- `query findings` prints the daemon's own tally per problem instead of losing it in display: its rows arrive folded with `seen_count`, and the shared renderer now receives that count rather than recounting single rows to one, so each block carries the same `Recurrence:` line as `analyze`, with the `seen_count` x representative-ops estimate. `explain --input <snapshot> <trace_id>` works on a daemon Report JSON, rebuilding the tree from its embedded masked spans exactly as the dashboard and the TUI do, and says which knob matters when the snapshot carries no trees. The TUI's Inspect list gained the `s` key, cycling id, worst-severity-first and aggregate-impact-first order with the active mode in the footer, and its detail panel prints the recurrence line. The Grafana findings table gained `Ops/trace` and a computed `Est. impact (ops)` column (`Traces x Ops/trace`) to click on, the default order staying `Traces` descending since a finding that wastes no I/O has no impact value to sort on. Every triage surface can now rank by the same aggregate-impact estimate.
- `max_retained_traces` is visible where its sibling already was: `/api/config` exposes it and `query monitor`'s Config tab lists it, so an operator can read the effective trace retention instead of inferring it. The TUI also stopped claiming that "reports do not carry raw spans": a batch report still does not, but a daemon snapshot carries the traces its retention held.
- The terminal, the TUI and `query findings` catch up with the dashboard on recurrence. `analyze` folds its output by signature: the first detection prints in full with a `Recurrence:` line (trace count and total avoidable ops), the repeats compact to one line each carrying only what differs, their trace id, so a recurring problem reads as one block instead of burying the report under near-identical ones. `--sort severity|impact` orders the findings on `analyze` (any output format) and on `query findings` (text output), with the same semantics as the dashboard control: descending primary key, the other axis as the tie-break. On the folded `query findings` data the aggregate is estimated as `seen_count` times the representative's avoidable ops, since the store keeps one detection per signature. And the TUI rebuilds span trees from a snapshot's `embedded_traces`, so a daemon export opened with `analyze --tui` draws the same masked trees as the HTML dashboard instead of empty panels, through a public `EmbeddedTrace::to_trace` whose spans carry the template in place of the never-embedded raw statement.

- The findings list sorts by aggregate impact, not only by unitary severity. Severity measures how bad one request is, never how much a problem costs in total: an info-level duplicate query detected on two hundred traces wastes more I/O than a critical seen three times, and the severity-first list buried it. A sort control on the Findings tab picks the primary key, severity or impact, and its direction, with the other axis as the automatic descending tie-break, so "highest total waste first, worst severity among equals" is one click. Impact is the sum of a signature's avoidable operations across every detection the report holds, computed in the page from what it already embeds, and shown on each row in impact mode ("~82 ops", the detection count and total in the tooltip). Slow findings have nothing avoidable and rank at zero, ordered among themselves by the severity tie-break. The CSV export follows the active order, and Copy link carries a non-default sort so a shared view opens sorted the same way. Severity stays untouched everywhere it is load-bearing: the quality gate, SARIF levels and acknowledgment folding read the same values as before.

- A daemon snapshot carries its span trees. `/api/export/report` handed over findings with no way to see what happened around them: the correlation window drops a trace's spans seconds after it completes, so `query explain <trace_id>` only answers while the trace is still live, and a report exported afterwards showed an empty Explain tab for every finding. The daemon now keeps the masked spans of the traces its retained findings point at, in a ring buffer sized by `[daemon] max_retained_traces` (default 50, `0` to keep none), and exports them under the report's new `embedded_traces`, capped in bytes to half the CLI's fetch limit so a busy daemon cannot grow the export past what `query monitor` and the TUI can read. A trace flushed twice by the window (TTL split) merges its flushes' spans, bounded by `max_events_per_trace`, since each flush carries only its own spans and findings from either can point at the other's, and the reflush also refreshes its eviction slot so a hot trace is not shed as the oldest. The byte cap skips a tree too large for the remaining budget rather than stopping at it, so one oversized trace cannot empty the export, and sizes are measured on the stored value with only the kept traces paying a clone. The key is range-validated like every sibling daemon cap, and retention switches off, with a startup warning naming the cause, when nothing can serve it (`api_enabled = false` or a disabled findings store). `perf-sentinel report --input <snapshot>` draws the tree from them, filtered to the findings the page shows, an acknowledged finding's tree does not ship, ranked worst-severity-first so the trees kept are the ones the page's top rows point at, and bounded by the `--max-traces-embedded` cap or by what the size target measurably leaves after the rest of the payload. The empty-tree message follows: the trim advice only shows when this render actually trimmed. Only masked fields travel, the normalized template and never the raw statement, the same shape the HTML sink has always embedded. A finding whose trace has aged out is exported as before, and the dashboard names the real knob for each empty-tree case: the retention buffer on a daemon snapshot, the embed cap on a batch render, the spanless Report JSON otherwise.

### Fixed

- The finding detail foot says how many traces a problem was detected in, and over which window. Detection is per trace, so a recurring problem is many findings that each carry the timestamps of their own trace: the foot read `seen first 14:32:01 · last 14:32:01`, which looks like a one-off whatever the report holds. The count and the window now come from every detection sharing the signature, and the date joins the time when the window straddles days. Derived in the page from the findings already embedded, so nothing new travels, and a report trimmed to fit its size budget counts what it kept.
- The finding detail panel stopped hiding everything behind a missing span tree. Only the tree needs the trace, but the whole panel was skipped without one, so a daemon snapshot showed no suggested fix, no pg_stat cross-reference, no source location and no first/last seen, on findings that carried all four. Each of them reads the finding, so each now renders whether or not the trace made it into the report. The `suggestion` field also reached the HTML for the first time: every other sink printed it, the dashboard never did. It is a CSV column too.
- Long templates no longer widen the page. A forty-column `UPDATE` in the span tree pushed the detail panel past the viewport and put a horizontal scrollbar under the whole report, because `text-overflow: ellipsis` cannot fire on a flex child that defaults to `min-width: auto`, and the panel itself sized to its content as a grid child. Both now set `min-width: 0`, so the tree row ellipsizes with the full operation on its tooltip. In the normalized-template block, which wraps rather than truncates, the break points sit after commas, a boundary a reader of SQL expects, marked with `<wbr>` so the text copied out is still the exact template.
- Cross-trace correlation confidence stays inside the `[0, 1]` the field documents, and keeps discriminating. Two counting bugs stacked: the correlator scored one co-occurrence per (source occurrence, target finding) couple, so a source seen twice followed by three targets counted six against a denominator of two and a live report showed `conf 150%`, and the pair counter accumulated for the pair's whole life while the source total only spans the rolling window, so any long-lived pair would eventually saturate whatever its real strength. Each source occurrence now counts once per pair, tracked by a monotonic sequence number since every finding in a batch shares one timestamp, and the count is window-scoped through two half-window buckets, rotated on every batch (not only when a pair records, which froze a quiesced pair's count at full strength while its denominator aged away) and stepped on aligned boundaries (re-anchoring to the current time stretched the estimate toward two windows), so `co_occurrence_count` finally means what its doc says and `min_confidence` filters mature pairs again. The correlation's sample trace id also refreshes on every matching target rather than freezing on the oldest one, which usually had already expired when an operator jumped through it to `explain`.
- A daemon snapshot no longer divides a finding's avoidable I/O by the wrong total. `/api/export/report` pairs findings retained over the store's whole window with a `green_summary` covering the last batch only, so nine avoidable operations over a two-operation batch rendered as `450% of I/O` on both the Overview list and the finding detail. Neither now shows a share when the report is a daemon snapshot, falling back to the IIS, which needs no report-wide denominator. Batch reports, where the two counts do come from the same run, are unchanged.
...
- `GET /api/findings` folds recurring findings into one row per problem. Detection is per trace, deliberately, so a recurring pattern re-emits an identical finding for every request that exhibits it: an operator polling the endpoint on a busy service read the same `redundant_sql` nineteen times in as many traces and reasonably concluded deduplication was broken, and the very first field tester did. The fold happens on the read path, keyed by the canonical acknowledgment signature, the product's existing "same problem" key. The stored buffer still holds every per-trace detection, which matters more than it looks: those instances carry the per-trace severity, they are what `/api/findings/{trace_id}` answers with for a trace whose spans have already aged out of the window, and FIFO pressure on them is the only thing that ever expires a fixed problem. Folding them away in the store instead would have downgraded a critical occurrence to whatever the next quiet trace produced, emptied that triage endpoint, and turned every count-based quality-gate rule into a distinct-problems rule. A folded entry carries two additive fields, `seen_count` and `first_seen_ms`, and the worst severity of the fold; `stored_at_ms` keeps its meaning. `limit` applies after the fold, so a pattern recurring on a hundred traces can no longer consume a whole page, and the severity filter applies to the fold's worst severity rather than to instances, so one problem cannot report two different counts depending on the filter passed. The representative row is the detection that earned the worst severity, evidence included, so a critical row never points at the quiet trace. `/api/findings/{trace_id}` and `/api/export/report` deliberately read the unfolded instances: the export is documented as shape-identical to `analyze --format json`, and every consumer that reloads it re-derives the quality gate from `findings`, so folding it would make a reload contradict the verdict shipped in the same payload, turning a red daemon into a green dashboard. The repo's own RUNBOOK triage recipe was updated with it: ranking services by row count now ranks by variety, so it ranks on `seen_count` instead.

### Added

- Batch runs count the spans they filter, so a thin report can be told apart from unusable instrumentation. The daemon has counted its OTLP drops since 0.8.7, but `analyze` and `report` threw the same tally away, and the CI path, the one place a false green costs the most, is a batch path: a SQL span shipped without `db.statement` (the .NET default until `SetDbStatementForText` is on) produced no finding, no counter and no warning, and the gate passed on nothing. On OTLP input the JSON report now carries `analysis.ingest`, spans received and filtered per stable reason, and the HTML report states the filtered count in its header rather than letting one retained span read like a clean run. The usable-span ratio beside it is computed per I/O kind and reported as the worst of them, not as a pooled share: a service emitting 900 usable HTTP spans and 100 SQL spans that all lack their statement pools to 0.90, which would slip past a 0.9 threshold while every SQL detector is blind. Internal spans and the deliberate drops (`non_sql_datastore`, `merged_db_span`) stay out of both ratios, the same exclusions the daemon's zero-retention warning applies. An optional `[thresholds] min_usable_span_ratio` turns the ratio into a fifth quality-gate rule that fails the gate below the threshold even with zero findings. It applies to batch runs only, and is skipped rather than passed when the input carries no tally (native, Jaeger, Zipkin) or no I/O-shaped span, so its absence stays visible. Two blind spots are documented rather than fixed, both in `docs/LIMITATIONS.md`: a statement-less span carrying no `db.system` is indistinguishable from an internal span and leaves the ratio entirely, and connection or transaction spans look exactly like a statement gap, so a threshold set at 1.0 will fail on healthy instrumentation. `diff` is not covered either, a `DiffReport` has no `analysis` block.

## [0.9.28] - 2026-08-04

Version `0.9.27` was a chart-only release, so the application line goes from
`0.9.26` to `0.9.28` and the two are aligned again.

### Added

- An acknowledgment that is still active but suppressed nothing in a run is reported, under the stable `unmatched_acknowledgment` warning kind. The file was write-mostly: nothing ever said an entry had become dead weight, so a finding accepted once stayed accepted long after it was fixed, and the accepted-problem list only grew. It also closes a hole no baseline comparison can: acknowledging a finding removes it from a baseline produced with filtering on, so the pull request that genuinely fixes it later has nothing left to make disappear and its Diff shows no resolution at all. The warning answers that directly, without a baseline and without needing both sides to have covered the same endpoints. What it can say depends on the entry. Two optional fields, `service` and `source_endpoint`, copied verbatim from the finding, let the warning check the run's per-endpoint I/O op counts: an endpoint that did I/O with no finding reads as "looks fixed, the entry can be removed", one that emitted no I/O as "proves nothing, keep the entry", the counts cannot tell an unexercised endpoint from a fix that removed the I/O outright and the message says so. An entry without the fields keeps a deliberate double reading, fixed or simply not run, since its opaque signature cannot tell the two apart and claiming a fix would invite deleting an entry that still protects the gate. An expired entry is inactive and is never reported this way, its finding is already back in the gate. The warning is derived only from a fresh analysis: re-rendering a saved report or a daemon snapshot never emits it, since such a report may already be ack-filtered and its I/O op counts describe another run, and the `diff` subcommand carries the after run's warnings in its output under `warning_details` so a diff-only CI surface sees them.

### Changed

- A daemon that refuses to start prints the cause under the error, not only the resource that failed. The startup errors name what could not be loaded and carry the underlying cause as an error source, which nothing printed, so `failed to load acknowledgments TOML at '<path>'` looked identical whether the file was missing, unreadable or refused. The refusal case is the one that matters: the loader declines to follow a symlink, a hardening against a link pointing at a sensitive file, and a Kubernetes ConfigMap projects every key as exactly that kind of symlink. Mounting a CI ack baseline the obvious way therefore produced a `CrashLoopBackOff` whose message named no cause, on an image with no shell to investigate from. The chain is now walked and printed as `caused by:` lines, and `docs/HELM-DEPLOYMENT.md` documents the `subPath` mount that materialises a real file. Behaviour is unchanged, only the diagnosis.

- The CI templates gate the merged state, not only pull requests. The GitHub baseline workflow and the GitLab default-branch job produced `baseline.json` and stopped there, so the branch every comparison is made against was the one branch nothing was ever enforced on. The two questions differ: the Diff tab asks what a pull request changed, and a finding present on both sides sits in neither column, so no author is ever blamed for a regression they inherited, while the quality gate asks whether a state is acceptable, on absolute thresholds and with no baseline at all. A regression merged into the default branch was therefore invisible in every Diff and surfaced as the failing gate of the next unrelated pull request whose integration tests happened to exercise the same endpoint. Both templates now end on `analyze --ci`, placed after the publish rather than before it, because a red default branch must still refresh the baseline or every pull request that follows loses its Diff tab on top of being blocked. It re-reads the trace file the publish step already produced, so it costs one process and not a second integration run. While a known trunk finding is being fixed, acknowledging it with an `expires_at` unblocks the queue without touching a threshold. Two diagrams were added to the CI guide and its FR mirror, one for where the baseline comes from and where each gate sits, one for how a finding lands in New, in Resolved or in neither, since the columns only mean what the two sides both covered.

### Fixed

- The correlation cards wrap instead of overflowing their panel on a narrow viewport. The meta column beside each card cannot shrink, so below roughly a phone width it pushed the card past the panel's right edge rather than reflowing. It now drops to its own line.

- The diff zones are readable in dark mode. They tint a whole column rather than a badge, so the severity backgrounds they share with the findings list read far too bright against a dark surface. Each zone now darkens its own tint, and light mode is unchanged since it needed no correction.

- The OTLP gRPC listener accepts gzip and deflate compressed exports. The OTel Collector's OTLP exporter compresses with gzip by default, and the listener declared no accepted encoding, so tonic answered ``Content is compressed with `gzip` which isn't supported``, which the Collector logs under `rpc error: code = Unimplemented`. It treats that status as permanent, which means it dropped every batch without retrying: a collector left at its defaults sent nothing through, while the pod stayed green, `/health` kept answering and every `/metrics` counter stayed at zero, so the failure only existed in the collector's own logs. The documented Collector recipe pointed at `:4317` and the instrumentation guide stated in the same breath that no `compression: none` override was needed, a claim that only ever held for the OTLP HTTP endpoint, which has decompressed gzip since 0.5.5 and is unaffected here. That endpoint accepted gzip only, it now takes deflate as well, so the two endpoints accept the same encodings. The decoding size cap keeps applying to the decompressed message, so `[daemon] max_payload_size` (16 MiB by default) still bounds a compressed payload and no window opens for a compression bomb, which a test now pins rather than a comment. What that limit bounds has changed, though: it used to cap buffers a client had genuinely uploaded, and it now caps what a much smaller upload expands to, so `[daemon] memory_high_water_pct` is the setting to raise off its default on a memory-capped pod. `perf-sentinel capture`, which receives from the same collectors, gets the same two encodings, plus the concurrency and timeout bounds the daemon listener already had. `zstd` and `snappy` are deliberately left out: a collector only sends them when explicitly configured, and zstd would pull C bindings into a static musl binary. On 0.9.26 and earlier the workaround stays `compression: none` on the exporter, or pointing it at the HTTP endpoint.
...
- The findings list is sorted by descending severity, so the first page carries the worst rows rather than whichever type sorts first. The sort is stable, so the report's own type and trace order survives inside each severity band, and it happens on the filtered data, so the paginated list, the counter, the sidebar badge and the CSV export all agree. The Diff columns and the correlations already sorted this way.
...
- The published carbon total is split into the three terms it is made of, under `aggregate.carbon_breakdown`. A disclosure exists to state avoidable waste, and the single total mixed what an anti-pattern fix can reduce with what it cannot: only the operational term carries the avoidable share the waste tiers quantify, while amortised hardware and cross-region network transport are irreducible by construction. Reading `total_carbon_kgco2eq` alone therefore overstated the addressable footprint by whatever those two terms weigh, with no way to tell. The three sum back to the total, an absent term counting as zero, and every term is omitted rather than zeroed when no window counted any (transport also drops its low/high bracket when a window of the period was scored with a pre-0.9.25 custom coefficient, since the sourced bracket only frames the fixed one): a zero would not distinguish "not measured" from "measured at zero", and for transport specifically the setting being off from it being on with no cross-region traffic. Network transport is flagged as the least certain of the three rather than given a fabricated interval: published figures span two orders of magnitude, so beside the mid value the term is published under a sourced coefficient bracket, `transport_kgco2eq_low` (0.001 kWh/GB, the Cloud Carbon Footprint networking coefficient) and `transport_kgco2eq_high` (0.059 kWh/GB, the Sustainable Web Design Model v4 network segment, a ceiling), which is written next to the fields in both languages.

- Archived windows are hash-chained as the daemon writes them, and `disclose` verifies the chain while aggregating. Until now the integrity story stopped at the published document: the `content_hash` and the cosign signature proved a report had not moved since generation, while the NDJSON windows it was built from carried nothing at all, so editing one before running `disclose` produced a perfectly coherent report with a valid hash. Each line now records the hash of `{ts, report, prev, seq}`, the hash of its predecessor and its position in the file, which makes an edited, removed or reordered window detectable. A line whose `hash` field is deleted counts as a break rather than as a pre-chaining line, once the file's chain has started, otherwise stripping the field would have been a way to rewrite a window and still publish zero breaks. The one edit no chain can see on its own is a clean truncation of the file's tail: what remains is a shorter but self-consistent chain, and `seq` cannot close that either since the reader counts the lines it was given. Detecting it needs a record of the chain head kept outside the file, which is what `integrity.cross_period_log` stays reserved for, and the schema says so next to the field. Each line is written unbuffered and the file is cut back to its last complete window when a write fails, so a disk error or an ungraceful kill surfaces as a dropped window rather than as an accusation of tampering. Startup scans the archive line by line to resume the chain from the last chained window, a linear read chosen so no record-size ceiling can truncate a window, whose cost only shows on a multi-gigabyte unrotated archive. `integrity.trace_integrity_chain`, reserved and always null since v1.0, carries the verdict: windows verified, windows written before chaining existed, breaks found inside the period and breaks found in the same files outside it, since one rolling archive can span several periods and a 2024 edit must not be published as this quarter's break. A break is published rather than fatal, so a truncated archive or an old rotation still yields an honest partial report, and the field sits under the `content_hash` precisely so a break count cannot be reset to zero under a valid signature. A restarted daemon reads the last hash back before appending, otherwise every restart would look like tampering. This is tamper-evidence after the fact, not proof of a sincere source: whoever controls the daemon and its files can regenerate a consistent chain, which is what `integrity.cross_period_log` stays reserved to address, and both documents say so.

- The disclosure schema moves to `perf-sentinel-report/v1.6` and publishes the carbon parameters that scale its own figures. Two `[green]` settings changed what a report says without appearing anywhere in it: `embodied_carbon_per_request_gco2` scales the SCI `M` term directly, and `include_network_transport` decided whether transport sat in the numerator until this release deprecated it entirely. Halving the first, or flipping the second, produced different published totals under an identical `methodology` block, so two reports could be non-comparable with nothing to show for it. `methodology.calibration_inputs` now carries `carbon_methodologies` (the SCI tags observed, legacy and current numerator tags can coexist when the period spans older windows), `embodied_gco2_total` (the `M` term summed over the period) and `embodied_gco2_per_request` (that total over `total_requests`, comparable across reports since it does not follow traffic volume). All three are folded from the archived windows rather than read from the configuration of whichever machine runs `disclose`, which is the only correct source when the period was measured elsewhere. Additive as always, with one exception that matters: `integrity.trace_integrity_chain` has been emitted since v1.0 and keeps being emitted, `null` when there is no chain to describe. Dropping the key when absent would have changed the canonical bytes of every already-published report and made each one fail `verify-hash`. The `serde(default)` plus `skip_serializing_if` rule applies to fields a schema adds, never to one it already emitted, and a test now recomputes a published example rather than a freshly built struct, which is what a struct-built baseline cannot catch. A test also pins both halves of the hashing contract, since a field outside the hash could be rewritten without breaking a signature.

- `docs/INSTANCE-TYPES.md` lists the 356 instance types carrying an embedded power profile, with their idle and max wattage, per provider and with the table vintage. The reference doc named a handful of families and the configuration file pointed at nothing, so an operator had no way to know whether their own type resolved to a measured profile or to a provider average. Generated from the embedded table by `scripts/generate-instance-types-doc.py`, because that table is itself regenerated from upstream coefficients and a hand-written list would go stale on the first refresh, and pinned by a test that fails when the page and the table disagree in either direction. The page also states what nothing said before: an unlisted type is not an error, it warns once at startup naming the type and falls back to an average, and that warning is how you check yours. FR mirror included.

- Configuration can now be split into deterministic fragments under `.perf-sentinel.d/`. Names must follow `NN-lowercase-name.toml`, priorities must be unique, tables merge recursively, later values win, and the main `.perf-sentinel.toml` loads last. The ready-to-copy GreenOps fragments are standardized as `30-green-alumet.toml`, `31-green-cloud.toml`, `32-green-scaphandre.toml`, `33-green-kepler.toml`, `34-green-redfish.toml` and `40-green-electricity-maps.toml`. `60-daemon-docker.toml` remains a standalone main config because its non-loopback bind must not be overwritten by the reference config. Invalid discovered files now stop with exit code 75 rather than discarding the valid files around them and continuing with defaults. The same rule now covers the implicit `.perf-sentinel.toml` found in the working directory, which used to warn and continue on defaults: a config that fails to read or parse stops with exit 75 whether it was passed with `--config` or discovered.

- The HTML report says why it is blank instead of showing nothing. Its content is built at load time from the embedded JSON, so a server that blocks the script leaves a page with no tab, no row and no value, and nothing anywhere saying so: the first report of this cost a user a trip through the build logs. The report now opens with a plain unstyled notice naming both causes, JavaScript turned off in the browser and a policy sent by the server, and pointing at the CI guide, and a script placed immediately after it removes it during parsing, so a normal load never paints it. The mechanism detects nothing and names no vendor: it is what remains when the script does not run, whatever the reason. The text mentions Jenkins only because its default policy is the one that blocks, and says in the same breath that GitHub Pages and GitLab Pages do not. Measured both ways on the real report, served under the policy Jenkins sends and served without one. It is not a rendered dashboard on a locked-down server, which no shape of this file can be, it is a page that explains itself rather than a blank one.

### Fixed

- `perf-sentinel capture` fails when the trace file it opened has been deleted under it by the time the wrapped command exits, instead of reporting the spans it wrote to an unlinked inode. `capture --output target/traces.json -- mvn clean verify` is the case: `clean` removes `target/` after the file is open, the writer keeps filling an inode no path points at, and the run used to print a span count and exit 0 while nothing existed at the path the next step reads. Creating the output directory, below, is what removed the start-up failure that used to catch this by accident, so the end of the run now checks.

- `perf-sentinel capture` creates the directory of `--output` when it is missing, instead of refusing to start. The documented recipe is `capture --output target/traces.json -- mvn verify`, and a CI workspace starts clean: Maven is what creates `target/`, and in wrapper mode Maven is the command being wrapped, so it has not run yet. The failure was a first-use one and its cost was not the error message: `capture` binds its ports and opens the file before spawning, deliberately, so that a capture which cannot listen never leaves a test suite running detached. Refusing there meant the integration suite never executed at all and the job failed pointing at a trace file. The refusal itself stays for the cases that cannot be fixed by creating a directory, an unwritable path still fails before the command starts. Measured on a real Jenkins controller with the released 0.9.24 binary on a fresh workspace, which is where CI differs from a developer machine, and where the `java-ci-capture` validation of the same recipe had missed it by writing into a temporary directory that already existed.

### Changed

- `[green] network_energy_per_byte_kwh` is deprecated and ignored. A configurable coefficient let two disclosures scale the same cross-region traffic differently, and a zero erased the term with no trace, which defeats always counting transport. The coefficient is fixed at 0.04 kWh/GB and the disclosure publishes the sourced 0.001-0.059 kWh/GB bracket beside the term. The key still parses, with a warning, so existing configs keep loading. `GreenConfig::include_network_transport` and `GreenConfig::network_energy_per_byte_kwh` also stay on the public Rust API, marked deprecated: `0.9.24 -> 0.9.25` is a cargo-compatible upgrade and deleting a public field would break a downstream build on `cargo update`. Both now read what scoring applies rather than what the file asked for, so code still using them computes the same figure as the engine.

- `[green] embodied_carbon_per_request_gco2 = 0.0` is deprecated and falls back to the default, warned rather than refused: an upgrade must not turn a running daemon into a boot failure, the same treatment the two transport keys above receive. No hardware has zero embodied carbon, and a zeroed coefficient erased the SCI `M` term from the disclosure while looking like a measurement. The coefficient stays configurable, hardware fleets genuinely differ and the applied value is published under `calibration_inputs`, but zero stops being a way to opt a term out of a public figure. An absent `embodied_kgco2eq` therefore has a single meaning: no green-scored window in the period.

- `[green] include_network_transport` is deprecated and ignored. The transport term is computed, displayed and disclosed on every run. Two organisations whose reports disagreed on this setting were not comparable, the setting left no trace in the published figures, and once the term was always disclosed a toggle that only hid it in the dashboards had no remaining justification, so it went the way of the coefficient above. The key still parses, with a warning. The term stays inside `total_carbon_kgco2eq`, like the embodied term it resembles: irreducible, uncertain, and part of the perimeter a disclosure must not under-state. Consequence to expect once: a period scored with the setting off under 0.9.24 now reports a larger total than it did.

- The carbon of the database and of the broker is published beside the split, as `database_kgco2eq_out_of_total` and `messaging_kgco2eq_out_of_total`. Both blocks already reported their waste, never their total, so the only carbon figure a reader could see for those subsystems was the avoidable part. They stay outside the sum and outside `total_carbon_kgco2eq`, since a database cgroup and a broker are a different scope from the instrumented services and folding them in would mix perimeters, but a reader now finds every carbon figure a run produced in one place. Windows archived before this carry no such figure and the fields are simply absent.

- The coefficients that scale the published figures are recorded in every archived window and published as `calibration_inputs.scoring_coefficients`: the embodied gCO2 per request, the network kWh per byte, and the two model switches. None of them appeared in a report before, so halving one changed every number with nothing to show for it. They are collected from the windows rather than from the machine running `disclose`, which is the only correct source when a period was measured elsewhere, and a coefficient changed mid-period publishes both values instead of one.


- `examples/perf-sentinel.toml` documents the daemon surface in full. It carried 14 of the 31 keys `[daemon]` accepts, with `[daemon.correlation]`, `[daemon.ack]`, `[daemon.cors]` and `[daemon.archive]` absent entirely, and an unknown key under `[daemon]` is ignored in silence, so a name recalled from memory reads as a code bug rather than a config mistake. Every key now appears with its default, its hard bounds and the startup-warning band where one exists. Three additions beyond the list: a block for the settings that change what the daemon reports or exposes rather than how fast it runs (`sampling_rate`, `memory_high_water_pct`, TLS, CORS), a block on what a deployment topology changes, since runtime acks and the archive only survive on a StatefulSet with persistence and a DaemonSet splits traces unless the collector routes by trace ID, and links to the online reference. Every key was loaded through the binary to confirm it parses and warns as documented. Every line now names the default to restore and the accepted range, and the file departs from the code defaults on two points it calls out: `environment = "production"`, since that is what the daemon this file describes runs as, and cross-service correlation switched on. `[daemon.correlation]` also gains the parsing test it never had, which pins the one conversion that can break quietly, `window_minutes` in the file against `window_ms` in the code.

- The example config now covers every key the parser accepts, not only the daemon ones, and a test enforces it: adding a field to a config section without showing it there fails the build. The audit that test replaces found `detection.sanitizer_aware_classification` missing, the `auth_header` of all five energy scrapers, `redfish.ca_bundle_path`, the Electricity Maps `emission_factor_type` and `temporal_granularity`, the `[green.alumet.database]` and `[green.alumet.broker]` attribution tables, `[green.broker_static]`, and the entire `[reporting]` section that drives `disclose` and the supply-chain subcommands. The sanitizer entry states which value fits which stack, since the heuristic's strongest signal is an ORM instrumentation scope: the recognized ones are listed, and `strict` is called out as the wrong choice on a bare driver, where nothing corroborates and the lower occurrence counts fall back to `redundant_sql`.

- Three examples in that same file did not parse, which nothing had caught since no test loaded it: a Redfish endpoint written as a bare URL string where the parser wants `{ url, schema }`, a Scaphandre `process_map` written as service-to-string where it wants a `{ exe_contains, cmdline_contains }` table, and the `[tempo]` section documented in `docs/CONFIGURATION.md`, which the parser never reads at all since `--endpoint` is a required CLI flag. The two examples are fixed, the `[tempo]` section is replaced by what the subcommand actually accepts, in both language versions. The file is also reordered: what is on by default first, then the settings marked `/!\`, then everything that is off until enabled, energy scrapers by measurement directness (Redfish, Alumet, Kepler, Scaphandre, cloud SPECpower, declared broker), then carbon, then reporting. The layout note explains what could not move, since a bare TOML key belongs to the last table declared above it. Every credential line now names the environment variable the daemon prefers, `PERF_SENTINEL_*_AUTH_HEADER` and `PERF_SENTINEL_EMAPS_TOKEN`, which it warns about when the value sits in the file instead, and `disclose_output_path` is marked reserved since it has no effect today.

- Five `[reporting]` keys are marked reserved where they were presented as working settings, after checking every config field against its consumers: `confidentiality_level`, `disclose_output_path`, `disclose_period`, `rekor_url` and `fulcio_url` are parsed, sometimes validated, and read by nothing. `disclose` takes those values from its own flags, and `verify-hash` shells out to `cosign verify-blob` without ever passing `--rekor-url` or `--fulcio-url`, so a private Sigstore instance configured there was silently ignored. What the section still does is gate the daemon at startup, and that gate is now stated: `intent = "audited"` makes the daemon refuse to start, which the example previously offered as an ordinary third value. Same correction in both language versions.

- Six annotations in the reworked example were wrong against the code and are corrected: the Electricity Maps endpoint (the default is the v4 host, not the v3 one) and its poll interval (300, not 900, and the free tier needs 3600 or more), the cloud scrape interval (15, not 30), `default_region` (there is no default, and without one a service carries no carbon figure at all), and two credential lines that taught an `env:VAR` indirection no loader implements, so the literal string would have become the API key. `[green.redfish] ca_bundle_path` is marked reserved rather than offered as an option, since setting it stops the scraper before its first poll while the daemon stays otherwise healthy. The `strict` recommendation is corrected in both languages and in the example: it changes the verdict on moderate occurrence counts only, because past three times `n_plus_one_min_occurrences` a sanitized group is reported as N+1 under `strict` too, and `strict` does fire without an ORM scope through its sequential-siblings path. The test that guards the example was itself unable to guard it, matching field names as unanchored substrings, skipping `pub` structs and fields, and dropping any key containing a digit: `region` was covered by `default_region`, `path` by `storage_path`, and `embodied_carbon_per_request_gco2` was never checked at all. The correlation test now expects a window the default does not already produce.

- The sections that expose the daemon or change what it reports carry an explicit `/!\` warning rather than a neutral note, including one that was not stated anywhere: `[daemon.ack]` is enabled by default and its routes accept any caller until `api_key` is set, which is harmless on the default loopback bind and is not on `0.0.0.0`. `examples/60-daemon-docker.toml`, which does bind `0.0.0.0`, gets the same warning and moves to `environment = "production"`, with a note that `"prod"` is rejected at load time.

- The example config no longer states 1 MiB as the `max_payload_size` default, which has been 16 MiB, and the same stale figure is corrected in the `capture` source comment. `docs/CONFIGURATION.md` had it right.

- `docs/CI.md` no longer suggests that a future release might ship a CSP-friendly report with CSS and JavaScript split into sibling files. It would not render on the default Jenkins policy either, which blocks scripts twice over: it declares no `script-src`, so an external one falls back to `default-src 'none'`, and its `sandbox` directive omits `allow-scripts`, which turns scripting off for the document whatever the source. Moving the content into the static DOM would not be enough on its own either, since inline `<style>` and the `data:` URI fonts and logos are blocked the same way. Measured on a control page served under the policy Jenkins actually sends. The two configuration-side answers, a Resource Root URL or a relaxed `hudson.model.DirectoryBrowserSupport.CSP`, remain the ones that work. The section now also states that the constraint is specific to Jenkins: GitHub Pages and GitLab Pages serve the report with no policy of their own and it renders there with nothing to configure, both measured. Same correction in the FR mirror.

- The HTML report storage footprint quoted for all three providers in `docs/CI.md` moves from 80 to 150 KB to 450 to 550 KB, measured on the current template. The number of findings barely moves it, since the bulk is the embedded fonts and logos that make the file self-contained, which a report with nothing to show carries too. Same correction in the FR mirror.
...
- The Maven Failsafe recipe in `docs/INSTRUMENTATION.md` no longer claims an `otlp_file` exporter driven by `OTEL_EXPORTER_OTLP_FILE_PATH`. Neither exists: no supported Java exporter writes spans to a chosen path, and the declarative `otlp_file/development` exporter reports `output_stream` as not implemented in Java. Nor can the traces be picked up from a forked test JVM's stdout, which Surefire and Failsafe use as their command channel: the agent initialises in `premain` and captures that channel before the `redirectTestOutputToFile` wrapper is installed, so every export is classified as channel corruption and diverted to a `.dumpstream`. The section now documents the two shapes that do work, `forkCount=0` with the agent on `MAVEN_OPTS` and the console captured with `tee`, or a network export to a receiver with the fork kept, and states the cost of each. Same correction in the FR mirror, in `docs/CI.md` and its mirror, and in the GitHub Actions, GitLab CI and Jenkins templates under `docs/ci-templates/`.

## [0.9.23] - 2026-07-29

### Added

- OpenTelemetry messaging spans are now ingested. A span carrying `messaging.system` and `messaging.destination.name` (or the pre-1.21 `messaging.destination`) is admitted as an outbound call whose target is the destination, falling back to the span name. This is one convention for the whole family, so Kafka, RabbitMQ, Pulsar, SQS, NATS and JMS are covered at once. Until now those spans matched none of the three admission branches (SQL statement, outbound URL, RPC callee) and were dropped under `not_io` with no per-span signal, so an architecture built on a bus read as analyzed while half of its business path was silent. Only `SpanKind::Producer` is admitted: a `CONSUMER` span describes work done on a delivered message rather than a call the service made, and a polling consumer would flood the occurrence detectors with publishes it never issued. Only the OTLP path is covered, which also covers OTLP/JSON and Tempo. Jaeger and Zipkin imports are unchanged.
- Two dedicated finding types, `n_plus_one_messaging` and `slow_messaging`, backed by a new `EventType::Messaging`. There is no redundant counterpart on purpose: redundancy is decided on parameter equality, a publish carries no parameters, and two messages are never the same message, so publish spans are skipped by that detector. The dedicated variant is what keeps destinations away from the HTTP path normalizer, which would mask an SQS queue ARN's account id as `{id}` and merge two AWS accounts into one template. A publish carries no bind parameters, so the messaging N+1 rule is the occurrence count alone, over a single empty parameter set. `chatty_service` and `pool_saturation` are unaffected, they remain gated on HTTP and SQL respectively. Both messaging types carry a `suggested_fix` keyed by broker technology (Kafka, RabbitMQ, SQS, Pulsar, NATS, JMS) rather than by application framework: the remediation lives in the broker client's batching API (`linger.ms`, `SendMessageBatch`, a transacted JMS session), which the framework does not name. `activemq` maps to the JMS advice, an unlisted system keeps the generic suggestion.
- `SpanEvent` gains `link_trace_id`, the trace of the producer whose message triggered this span. A broker joins producer to consumer with an OpenTelemetry span link rather than a parent-child edge, and the consumer usually starts its own trace, so the chain was cut at the broker. Ingestion reads the first link of the nearest `CONSUMER` span, whether it sits on the ancestor chain or is a sibling under a shared parent, which is the shape the OTel Java and .NET Kafka instrumentations emit. Among several sibling `receive` spans it takes the last one that started before the work did. `explain` renders it as a `triggered by trace <id>` line in the CLI, `/api/explain/{trace_id}` and the TUI, not in the HTML dashboard. The two traces are deliberately not merged, see `docs/LIMITATIONS.md` for the reasoning. The gate on `CONSUMER` matters: batch span processors and follows-from relations emit links too, and a link back into the span's own trace is dropped.
- The periodic disclosure schema moves to `perf-sentinel-report/v1.5`, adding the two messaging pattern names to the `PatternName` enum.
- Broker energy can now be attributed to the messaging findings, through the same shape the database figure already used. `[green.alumet.broker]` measures a declared broker cgroup with Alumet. `[green.broker_static]` needs no agent at all: an operator declares `nodes` and an instance type, and the embedded SPECpower table turns that into watts, which is the only path available for a managed broker (Confluent Cloud, MSK, SQS, managed Pulsar). Either energy is multiplied by the messaging-only waste ratio and reported as `green_summary.messaging_waste`, plus `aggregate.messaging_waste` in the v1.5 disclosure. Informational like `database_waste`, never folded into `energy_kwh`, `co2` or the waste tiers. Visible in the CLI summary, the `monitor` TUI and the HTML dashboard.
- The three provenance tags do not err in the same direction, so read `model` before reading the number. `alumet_rapl` measures the cgroup through RAPL, CPU and DRAM only, so it under-counts. `broker_specpower` bounds the declared vCPUs at full load from a SPECpower table that excludes storage, network and PSU, so it is a ceiling on compute and not on the cluster's wall power, and it does not move when an application starts batching its publishes. `estimated` re-presents a share of the totals. The disclosure keeps the three apart, with `declared_windows` and `declared_energy_kwh` beside the measured pair. A measured cgroup always outranks a declaration when both are configured. Two counters back the ratio, `total_messaging_io_ops` and `avoidable_messaging_io_ops`, mirroring the SQL pair. Additive via serde defaults on every new field, so re-hashing an existing v1.4 report still yields its original `content_hash`, since the hashed form carries the report's own `schema_version`. A report generated by this build declares v1.5, so it is not byte-identical to one generated by 0.9.22.
...
- A fourth quality-gate rule, `[thresholds] n_plus_one_messaging_warning_max`, defaults to 3. It counts warning-or-higher `n_plus_one_messaging` findings, mirroring the HTTP rule rather than the SQL one: a Kafka client may already batch the publishes it buffers, so the occurrence count is an upper bound there and a zero tolerance would fire on work already done. Until now the messaging types had no dedicated rule and could only move the global `io_waste_ratio`. A bus-heavy service that already carries four or more such findings sees its gate turn red on this release without a configuration change, raise the threshold or acknowledge the findings.
- `analyze --ci` can start failing on a bus-heavy service with no configuration change, and not necessarily on a messaging finding. N+1 messaging findings feed both their own rule above and `io_waste_ratio`. But the effect measured on a real corpus arrived through `serialized_calls`: a service publishing one message per request grows no messaging anti-pattern at all, it simply adds I/O spans to its traces, and the detectors that count spans per trace cross their threshold more often. Expect the new findings to land on the topological detectors rather than on the messaging ones.

### Migration

- `EventType` gains a third variant and `FindingType` three more. Both enums are public and neither is `#[non_exhaustive]`, so a downstream crate matching exhaustively on either will not compile until it handles the new variants.
- `SpanEvent` and `explain::SpanNode` each gain a public field and neither derives `Default`, so every struct literal built outside this crate needs the new field. `SpanEvent` deserialization is unaffected, `link_trace_id` defaults when absent, so stored JSON traces keep parsing.

## [0.9.22]

### Added

- The daemon's `tuning` advisor gains a rule on `[daemon] sampling_rate`. Below 1.0 the daemon drops whole traces, so the absolute counts (findings, occurrences, the `perf_sentinel_*` totals) describe a fraction of the traffic and a rare pattern can be missed entirely, and nothing in `/api/export/report` said so. Ratios are left out of the message deliberately: the drop is a uniform hash over trace ids, so the I/O waste ratio samples numerator and denominator alike and stays readable, and rescaling it would produce a wrong number rather than a corrected one. A rate of exactly 0.0, which config validation accepts, gets its own message saying no trace is analyzed at all, and that message rides the cold-start envelope too, since at rate 0.0 the daemon never leaves cold start and `cold_start` alone points at the wrong cause. The rule reads the configuration rather than a metric, so it fires on an idle daemon as much as a busy one, and it is the only advisor entry that warns about how to read the report instead of about a knob the load outgrew. Sampling applied upstream degrades the same counts and raises no warning, because a kept trace is indistinguishable from a complete one, so `docs/HELM-DEPLOYMENT.md` now describes the collector pipeline layout that keeps the analysis branch unsampled.

### Changed

- `source_endpoint` now resolves the inbound HTTP route by walking the parent chain instead of inspecting the direct parent only. The single-level lookup missed the common layout where the route sits on the SERVER span two or more levels above the leaf (`tomcat -> hibernate -> jdbc`, or any controller/repository stack), so a `n_plus_one_sql` triggered by a real endpoint reported `"unknown"` and named no origin. Bounded by the same depth limit as the `code.*` walk. The parent index covers the whole export request rather than one `ResourceSpans` block, because the collector batch processor splits a single trace across blocks of the same service and a per-block index lost the endpoint at the boundary. A span carrying `http.url`/`url.full` on a CLIENT span is skipped, so a DB call nested under an outbound HTTP call stays attributed to the route being served, not to the third party the caller reached.
- `source_endpoint` falls back to the code frame (`code.namespace` + `code.function`, or the fully-qualified `code.function.name`) when no HTTP attribute is found anywhere up that chain, instead of reporting the literal `"unknown"`. Entry points reached outside an inbound HTTP request, scheduled jobs and message consumers, previously produced findings that named no origin. Worse, since the ack signature is `type : service : endpoint : hash(template)`, two distinct jobs in one service issuing the same statement shared a signature, so acknowledging one silently hid the other. All three ingestion paths walk the parent chain with the same rules and depth bound: OTLP through `parent_span_id`, Jaeger through the `CHILD_OF` reference, Zipkin through `parentId`, so one trace resolves to one endpoint whichever way it arrives and a single acknowledgment covers all three. The walk never crosses a `service.name` boundary, since a caller's route names another service's entry point and its code frame lives in another repository. It does not cross the export request either: a collector `batch` processor that flushes one trace as two exports leaves the second half on `"unknown"`, measured at 414 findings on a 245 MB lab capture and concentrated on services that instrument both sides of their own HTTP calls (see `docs/LIMITATIONS.md`). On Jaeger and Zipkin a DB span that carries an HTTP tag of its own still wins, that tag can only be the inbound route propagated onto it, but an outbound HTTP span no longer takes its own `http.target` as the route being served. Jaeger and Zipkin also learned the stable `code.function.name` / `code.file.path` / `code.line.number` spellings, and their empty-endpoint fallback becomes the documented `"unknown"`, an empty string used to leave an empty component in the ack signature. The namespace joins with the separator it already uses, `::` for PHP and Rust (`Slim\App::handle`, never the invalid `Slim\App\handle`), `.` otherwise, so the legacy attribute pair and the stable qualified name spell one origin identically and an agent upgrade does not re-key acknowledgments. A `#` inside a frame is rewritten to `.` because the endpoint secret-stripping applied at the ingestion boundary truncates there.
- The code frame is used only when it names an origin, and the walk keeps the outermost such frame rather than the nearest, so two entry points sharing one repository do not collapse onto the repository's frame. A blank attribute, one holding ASCII control characters, a bare unqualified function name such as `execute`, and a framework kernel frame (`Symfony\Component\HttpKernel\HttpKernel::handle`, `Illuminate\Foundation\Http\Kernel::handle`, `PDOStatement::execute`, Spring/servlet-container internals) all keep `"unknown"`, which is honest, rather than producing an endpoint that names no origin and collides exactly as `"unknown"` did while looking resolved. On PHP stacks the outermost frame is the framework's HTTP kernel, so without that filter thousands of findings would share one plausible-looking endpoint.

### Migration

- Findings whose endpoint was previously `"unknown"` change signature, and that is a wider set than scheduled jobs alone: it includes HTTP findings whose route lived two or more levels above the leaf span, common on Spring and Quarkus. Stacks whose route already resolved from the direct parent are untouched, and a lab run across 12 ecosystems measured roughly 4% of acknowledgment keys breaking, concentrated on PHP and on stacks that instrument both sides of their own HTTP calls. On the Jaeger and Zipkin paths the move is wider: findings whose endpoint was the empty string (the previous fallback) re-key to a route, a code frame or `"unknown"`, so acknowledgments recorded from those paths must be re-captured too. Any entry in `.perf-sentinel-acknowledgments.toml` recorded against an `unknown` endpoint stops matching and must be re-recorded. Findings whose endpoint was already a route are unaffected. Run `analyze` once and copy the new signatures from the report.
- Baselines that persist findings are affected the same way, and only those. `report --before baseline.json` (the documented CI-template path) compares stored old endpoints against fresh ones, so every finding that moves off `"unknown"` shows once as resolved and once as new on the first run after upgrade, with no application change behind it, and a CI job uploading SARIF raises a fresh code-scanning alert for each. Re-capture such baselines against this release. A pipeline whose baseline is a trace corpus fed to `diff --before` sees no upgrade churn at all, both sides are re-analyzed by the current binary.
...
- `green_summary.database_waste` now exists on every run: when no Alumet measurement of the database cgroup is available (batch runs, managed databases, no `[green.alumet.database]`), the figure is estimated from the modeled energy of the window's SQL spans (verb multipliers, calibration and regional intensities included) multiplied by the SQL waste ratio. A declared database with no delivered reading emits nothing (its energy carries over). The new `model` field carries the provenance (`alumet_rapl` = measured, `estimated` = modeled), surfaced on every display. Still excluded from `energy_kwh` and `co2`.
- The batch text report shows the SQL share of the I/O ops and, when a report carries it, the database waste line with its model tag. The HTML dashboard's Green panel gains a Database waste card and the SQL share on the I/O waste ratio card.
- Periodic disclosure schema v1.4: `aggregate.database_waste` sums the database-side avoidable energy and carbon over the period at both thresholds (operator and binary-pinned canonical, the same anti-manipulation construction as the waste tiers), with its provenance `models`, the measured/estimated split (`measured_energy_kwh`, `measured_windows`, `estimated_windows`) and the `windows_with_figure` / `windows_with_carbon` counts. An informational lower bound kept out of every total. The per-window archive carries the matching `disclosure_waste.database` block. See `docs/SCHEMA.md`.
...
- The `query monitor` Energy tab shows the database waste line next to the window energy.
- `docs/ENERGY.md` (and `docs/FR/ENERGY-FR.md`): a plain-language synthesis of the whole energy and carbon pipeline, what each configuration rung changes, and the external sources every figure rests on.

### Changed

- Unknown keys and subsection names under `[green.alumet]` are now rejected at config load instead of being silently ignored, so a typo like `[green.alumet.databse]` fails loudly. A config that previously carried a stray key in that section stops loading after the upgrade, remove the key.

...
- The two trim messages no longer prescribe a flag as the only way out. The one on a finding now names the figures it has, `526 of 2000 span trees fit this report's size budget`, and offers the action a reader without a shell can take, a narrower run, before the one that needs a terminal. The banner drops `--max-traces-embedded` entirely, it was addressed to a reader who may only have a browser.

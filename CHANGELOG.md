...
- The two trim messages no longer prescribe a flag as the only way out. The one on a finding names the figures it has, `526 of 2000 span trees fit this report's size budget`, then points at the trace id sitting one line above it with its copy button: rerunning on that id returns the one tree the reader is looking at, where a narrower window means guessing which one contains it and running the whole sweep again. The flag comes last, marked as needing a shell. The banner drops it entirely, it was addressed to a reader who may only have a browser.

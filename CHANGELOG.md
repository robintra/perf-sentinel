...
- The two incident panels of the findings dashboard page through the ring. They read `/api/incidents?limit=50`, so only the 50 most recent incidents were reachable. They keep pages of 50, since every incident carries its frozen findings and both panels fetch the page on each refresh, from the offset a new `Incident skip rows` variable picks (0 to 950, by 50), and `Incident findings` reads the same page so a clicked row is always found. Dashboard `version` 10. `docs/HELM-DEPLOYMENT.md` and its French mirror describe it.

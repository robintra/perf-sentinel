...
- `POST /api/incidents` receives an Alertmanager webhook and freezes the findings of the window that preceded the incident, `GET /api/incidents` returns them, both behind the opt-in `[daemon.incidents]` section and its required `api_key`. The point is not to record that an incident happened, it is to capture the findings before the FIFO ring evicts them, which on a busy fleet takes minutes: by the time anyone opens a terminal the window is usually gone. The window is `[at_ms - lookback_ms, at_ms]` with both bounds, so the frozen rows are folded over the window alone, and each incident carries the `oldest_finding_ms` of the moment it was captured, which is what separates a complete capture from one the ring had already eaten into. The Alertmanager envelope is the only accepted shape, because it is the only one an operator cannot produce otherwise (`webhook_config` has no body template) while any script emits it with curl. The service comes from a configurable label and an alert without it is refused, that label being the join key to the findings. The kind comes from a second label whose value must be one of five, never guessed from `alertname`. Reposting is idempotent on a content-derived id, and `perf_sentinel_incidents_total{kind}` therefore counts incidents rather than webhook deliveries. This is about observed services, not the daemon's own cgroup, which stays `perf_sentinel_ingest_memory_pressure`. `[daemon.incidents] archive_path` appends every accepted delivery to a newline-delimited JSON file, off the reactor and never failing the webhook, because the ring dies with the daemon and a node-level memory event that kills the observed service often takes a co-located daemon with it, destroying the very record that would explain the outage. Append-only with the last record of an id winning, created `0600` since `detail` and query templates are not world-readable, and deliberately without rotation: a few incidents a day against the analysis archive's several windows a second does not justify a bounded channel and a drop counter.

...
// it twice is a no-op.
// ponytail: a 4-rule string patch, NOT a reimplemented generator. When/if the
// generator is built, fold these into it and delete this file.
...
// Each fix: `from` is the exact broken substring the generator emits, `to` the fix.
// `from` never matches the already-fixed output, so the patch is idempotent.
const FIXES = [
  // 1. The outer IIFE of the inline script is never closed -> SyntaxError kills
  //    ALL inline JS (search, theme, scrollspy, sidebar, lightbox). Append `})();`.
  {
    name: 'iife-close',
...
// Self-check: the truncated IIFE (the one that breaks everything) must be gone.
const stillBroken = files.filter((f) => readFileSync(f, 'utf8').includes(FIXES[0].from));
if (stillBroken.length) {
  console.error(`FAIL: ${stillBroken.length} pages still carry the truncated IIFE`);
  process.exit(1);
}
console.log('self-check OK: no truncated IIFE remains');

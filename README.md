...
- **Datadog / dd-trace** (no OpenTelemetry SDK): bridge dd-trace traffic through an OTel Collector running the `datadogreceiver`, which re-exports OTLP to the daemon. perf-sentinel reads the SQL from the Datadog resource natively, no application change. See [docs/INTEGRATION.md#coming-from-datadog-dd-trace-no-opentelemetry](docs/INTEGRATION.md#coming-from-datadog-dd-trace-no-opentelemetry).

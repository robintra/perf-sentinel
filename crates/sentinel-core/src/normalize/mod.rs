...
pub fn normalize(mut event: SpanEvent) -> NormalizedEvent {
    ...
    match event.event_type {
        ...
        // A topic or queue name is already a template with no variable part
        // to extract. It is not routed through normalize_http, which would
        // mask numeric segments as {id} and strip an SQS account id.
        EventType::Messaging => {

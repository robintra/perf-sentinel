...
fn top_offenders_sorted_by_iis_desc() {
    ...
    assert!(
        summary.top_offenders[0].io_intensity_score >= summary.top_offenders[1].io_intensity_score
    );
}
...
fn hourly_row_intensity_is_time_weighted_mean() {
    ...
    .unwrap()
    .0;
    // 1e-9 tolerance: the time-weighted mean accumulates float
    // roundoff that exact equality would reject for values like 28.1.
    assert!(
        (row.grid_intensity_gco2_kwh - expected).abs() < 1e-9,
...
fn calibration_with_one_service(service: &str, factor: f64) -> crate::calibrate::CalibrationData {
...
fn realtime_intensity_estimation_metadata_round_trips_through_json() {
    ...
    assert!((round_tripped.grid_intensity_gco2_kwh - row.grid_intensity_gco2_kwh).abs() < 1e-12);

...
//! source-of-truth homogeneity. The CSV-derived bulk lives in
//! `table_data.rs` (regenerated by `scripts/refresh-instance-power.py`);
//! the remaining direct-compute modern entries (absent from the
//! provider CSV) are kept as-is in [`MANUAL_INSTANCE_ROWS`] below.
//!
